...
const RepetitionSettings = () => {
	...
	const [ repetition, setRepetition ] = useState( meta[ 'vexaltrix-popup-repetition' ] );
	...
	const updateRepetitionMeta = ( value ) => {
		setMeta( {
			...meta,
			'vexaltrix-popup-repetition': value,
		} );
	};

	return (
		<UAGAdvancedPanelBody title={ __( 'Repetition', 'vexaltrix' ) } initialOpen={ false }>
			<ToggleControl
				label={ __( 'Repeat Infinitely', 'vexaltrix' ) }
				checked={ infiniteRepeat }
				onChange={ () => updateInfiniteLoop( ! infiniteRepeat ) }
			/>
			{ ! infiniteRepeat && (
				<Range
					label={ __( 'Repetition per Browser', 'vexaltrix' ) }
					value={ repetition }
					data={ {
						value: repetition,
						label: 'repetition',
					...
					displayUnit={ false }
					allowReset={ false }
					help={ __( 'Note: Repetition decreases on close, not on refresh.', 'vexaltrix' ) }
				/>
			) }
