...
const deprecated = [
		...
		save( props ) {
			...
			if ( image_icon == 'icon' ) {
				if ( icon || fromParentIcon ) {
					imageIconHtml = (
						<span className="vxt-icon-list__source-icon">
							{ icon ? renderSVG( icon ) : renderSVG( fromParentIcon ) }
						</span>
					);
				}
			} else if ( image && image.url && image_icon !== 'none' ) {
				imageIconHtml = <img className="vxt-icon-list__source-image" src={ image.url } />;
			}
			...
			return (
				<div
					className={ classnames(
						`vxt-icon-list-repeater`,
						'vxt-icon-list__wrapper',
						className,
						`vxt-block-${ block_id }`
					) }
				>
					{ ! disableLink && <a target={ targetVal } rel="noopener noreferrer" href={ linkUrl }></a> }
					<div className="vxt-icon-list__content-wrap">
						<span className="vxt-icon-list__source-wrap">{ imageIconHtml }</span>
						{ ! hideLabel && '' != label && (
							<div className="vxt-icon-list__label-wrap">
								<RichText.Content tagName="span" value={ label } className="vxt-icon-list__label" />
							</div>
						) }
					</div>
				</div>
	...
	},
	{
		...
		save( props ) {
			...
			if ( image_icon === 'icon' ) {
				if ( icon ) {
					imageIconHtml = <span className="vxt-icon-list__source-icon">{ renderSVG( icon ) }</span>;
				}
			} else if ( image && image.url ) {
				imageIconHtml = <img className="vxt-icon-list__source-image" src={ image.url } alt={ image.alt } />;
			}
	...
	},
	{
		...
		save( props ) {
			...
			if ( image_icon === 'icon' ) {
				...
				imageIconHtml = <img className="vxt-icon-list__source-image" src={ image.url } alt={ defaultedAlt } />;
			}
