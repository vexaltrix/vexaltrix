	...
	const { latestPosts, taxonomyList, block } = useSelect( ( select ) => {
		...
		if ( true === postPagination && 'empty' === paginationMarkup ) {
			...
			const getApiFetchData = getApiData( {
				...
				action: 'vxt_ultimate_gutenberg_blocks_post_pagination',
				data: formData,
			} );
	...
	if ( ! ( Array.isArray( latestPosts ) && latestPosts.length ) ) {
		return (
			<>
				...
				<Placeholder icon="admin-post" label={ __( 'Post Grid', 'vexaltrix' ) }>
...
export default compose( addInitialAttr, AddStaticStyles )( PostGridComponent );
