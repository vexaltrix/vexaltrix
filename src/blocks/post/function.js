...
export const renderPostLayout = (
	...
	const mapFunction = ( [ name, props = {} ], key, addWrapper = null ) => {
		if ( 1 === addWrapper && 'vexaltrix/post-image' === name ) {
			return;
		}

		if ( 0 === addWrapper && 'vexaltrix/post-image' !== name ) {
			return;
		}
...
registerBlockComponent( {
	blockName: 'vexaltrix/post-title',
	component: PostTitle,
} );

registerBlockComponent( {
	blockName: 'vexaltrix/post-image',
	component: PostImage,
} );

registerBlockComponent( {
	blockName: 'vexaltrix/post-taxonomy',
	component: PostTaxonomy,
} );

registerBlockComponent( {
	blockName: 'vexaltrix/post-meta',
	component: PostMeta,
} );

registerBlockComponent( {
	blockName: 'vexaltrix/post-excerpt',
	component: PostExcerpt,
} );

registerBlockComponent( {
	blockName: 'vexaltrix/post-button',
