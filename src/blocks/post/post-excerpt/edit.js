...
export const PostExcerpt = ( props ) => {
	...
	if ( null === post.vxt_ultimate_gutenberg_blocks_excerpt ) {
		return null;
	}

	const words = post.vxt_ultimate_gutenberg_blocks_excerpt.split( ' ' );

	let excerpt = post.vxt_ultimate_gutenberg_blocks_excerpt;
	...
	if ( excerptLengthFallback >= words.length ) {
		excerpt = post.vxt_ultimate_gutenberg_blocks_excerpt;
	} else {
