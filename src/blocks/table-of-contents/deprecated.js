...
const deprecated = [
	{
		...
		save( props ) {
			...
			const { align, block_id, tColumns, heading, headerLinks, mappingHeaders, scrollToTop } = props.attributes;
			...
			return (
				...
				>
					<div className="uagb-toc__wrap">
						<RichText.Content value={ heading } tagName="div" className="uagb-toc__title" />
						<TableOfContents
							align={ align }
							numcolumns={ tColumns }
	...
	},
	{
		...
		save( props ) {
			...
			if ( makeCollapsible && icon ) {
				icon_html = <span className="uag-toc__collapsible-wrap">{ renderSVG( icon ) }</span>;
			}
			...
			return (
				...
				>
					<div className="uagb-toc__wrap">
						...
						<TOC mappingHeaders={ mappingHeaders } headers={ headerLinks && JSON.parse( headerLinks ) } />
					</div>
				</div>
			);
		},
