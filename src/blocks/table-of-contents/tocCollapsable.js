...
function TableOfContents( props ) {
	...
	const renderList = ( header ) => {
		...
		header.forEach( ( item, index ) => {
			...
			const collapsibleClass =
				hasChildren[ index ] && depthMapping[ index ] <= collapsibleListDepth
					? 'uagb-toc__list--expandable'
					: '';

			const collapsibleSpan =
				hasChildren[ index ] && depthMapping[ index ] <= collapsibleListDepth
	...
	return (
		<p className="vxt_ultimate_gutenberg_blocks_table-of-contents-placeholder">
			{ __( 'Add a header to begin generating the table of contents', 'vexaltrix' ) }
		</p>
	);
}
