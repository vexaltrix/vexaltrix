...
const VexaltrixTableOfContentsEdit = ( props ) => {
	const {
		isSelected,
		setAttributes,
		attributes,
	...
	useSelect( ( select ) => {
		...
		if ( headerArray ) {
			headerArray.forEach( function ( index, value ) {
				const header = index;
				let excludeHeading;
...
export default compose( addInitialAttr, AddStaticStyles )( VexaltrixTableOfContentsEdit );
