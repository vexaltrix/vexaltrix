...
const Render = ( props ) => {
	...
	useEffect( () => {
		if ( VexaltrixTableOfContents ) {
			const selector = '.uagb-block-' + block_id;
			VexaltrixTableOfContents.init( selector, attributes );
		}
	}, [] );
	...
	useEffect( () => {
		if ( tocRoot.current && ! makeCollapsible && tocRoot.current.classList.contains( 'uagb-toc__collapse' ) ) {
			...
			VexaltrixTableOfContents._slideDown( tocRoot.current.querySelector( '.uagb-toc__list-wrap' ), 500 );
		}
	}, [ makeCollapsible ] );
	...
	useEffect( () => {
		const selector = '.uagb-block-' + block_id;
		const block_element = VexaltrixTableOfContents._getDocumentElement().querySelector( selector );
		// Set computed margin for collapsable marker in editor.
		if ( VexaltrixTableOfContents?._setCollapseIconMargin && enableCollapsableList ) {
			...
			// Set computed margin for collapsable marker in editor.
			VexaltrixTableOfContents._setCollapseIconMargin( selector, attributes );
		}
		...
		>
			<div className="uagb-toc__wrap">
				<div className="uagb-toc__title">
					<RichText
						placeholder={ __( 'Table Of Contents', 'vexaltrix' ) }
						value={ headingTitle }
				...
				{ enableCollapsableList ? (
					<TableOfContentCollapsable
						mappingHeaders={ mappingHeaders }
						headers={ headers }
						collapsibleListDepth={ collapsibleListDepth }
					/>
				) : (
