...
const deprecated = [
	{
		...
		save( props ) {
			...
			const reverse_tablet = reverseTablet ? 'vxt-columns__reverse-tablet' : '';

			const reverse_mobile = reverseMobile ? 'vxt-columns__reverse-mobile' : '';
			...
			return (
				<CustomTag
					className={ classnames(
						className,
						'vxt-columns__wrap',
						`vxt-columns__background-${ backgroundType }`,
						`vxt-columns__stack-${ stack }`,
						`vxt-columns__valign-${ vAlign }`,
						`vxt-columns__gap-${ columnGap }`,
						`align${ align }`,
						reverse_tablet,
						reverse_mobile
					) }
					id={ `vxt-columns-${ block_id }` }
				>
