...
const ColumnsComponent = ( props ) => {
	...
	const { innerBlocks, blockType, variations, hasInnerBlocks, defaultVariation } = useSelect( ( select ) => {
	...
	if ( ! hasInnerBlocks ) {
		return (
			<div className="vxt-columns-variation-picker">
				<BlockVariationPicker
					icon={ '' }
					label={ __( 'Advanced Columns', 'vexaltrix' ) }
					instructions={ __( 'Select a variation to start with.', 'vexaltrix' ) }
...
export default compose( addInitialAttr, withNotices, AddStaticStyles )( ColumnsComponent );
