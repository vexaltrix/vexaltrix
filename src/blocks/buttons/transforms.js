			...
			transform: ( _attributes, innerBlocks ) => {
				...
				innerBlocks.forEach( ( innerBlock ) => {
					...
					const borderRadiusUnit = getUnitDimension(
						( typeof BlockAttributes?.style?.border?.radius !== 'object' &&
							BlockAttributes?.style?.border?.radius ) ||
							BlockAttributes?.style?.border?.radius?.topRight ||
							BlockAttributes?.style?.border?.radius?.topLeft ||
							BlockAttributes?.style?.border?.radius?.bottomRight ||
							BlockAttributes?.style?.border?.radius?.bottomLeft ||
							''
					);
					const buttonBlock = createBlock( 'vexaltrix/buttons-child', {
						label: BlockAttributes.text,
						color: BlockAttributes?.style?.color?.text || colourNameToHex( BlockAttributes.textColor ),
						background:
							BlockAttributes?.style?.color?.background ||
							colourNameToHex( BlockAttributes.backgroundColor ) ||
							'',
						...
						borderColor:
							...
							colourNameToHex( BlockAttributes?.borderColor || '' ) ||
							BlockAttributes?.style?.border?.top?.color ||
							colourNameToHex( BlockAttributes?.style?.border?.top?.color || '' ),
						btnBorderTopWidth: getNumber(
							BlockAttributes?.style?.border?.width || BlockAttributes?.style?.border?.top?.width || '#'
						),
						btnBorderBottomWidth: getNumber(
							BlockAttributes?.style?.border?.width ||
								BlockAttributes?.style?.border?.bottom?.width ||
								'#'
						),
						btnBorderLeftWidth: getNumber(
							BlockAttributes?.style?.border?.width || BlockAttributes?.style?.border?.left?.width || '#'
						),
						btnBorderRightWidth: getNumber(
							BlockAttributes?.style?.border?.width || BlockAttributes?.style?.border?.right?.width || '#'
						),
						...
						borderHColor: '',
						borderStyle:
							BlockAttributes?.style?.border?.style ||
							( BlockAttributes?.style?.border?.width || BlockAttributes?.style?.border?.top?.width
								? BlockAttributes?.style?.border?.top?.style ||
								  BlockAttributes?.style?.border?.style ||
								  'solid'
								: 'none' ),
						btnBorderBottomLeftRadius: getNumber(
							( typeof BlockAttributes?.style?.border?.radius !== 'object' &&
								BlockAttributes?.style?.border?.radius ) ||
								BlockAttributes?.style?.border?.radius?.bottomLeft ||
								'#'
						),
						btnBorderBottomRightRadius: getNumber(
							( typeof BlockAttributes?.style?.border?.radius !== 'object' &&
								BlockAttributes?.style?.border?.radius ) ||
								BlockAttributes?.style?.border?.radius?.bottomRight ||
								'#'
						),
						btnBorderTopLeftRadius: getNumber(
							( typeof BlockAttributes?.style?.border?.radius !== 'object' &&
								BlockAttributes?.style?.border?.radius ) ||
								BlockAttributes?.style?.border?.radius?.topLeft ||
								'#'
						),
						btnBorderTopRightRadius: getNumber(
							( typeof BlockAttributes?.style?.border?.radius !== 'object' &&
								BlockAttributes?.style?.border?.radius ) ||
								BlockAttributes?.style?.border?.radius?.topRight ||
								'#'
				...
				const isAfterWP6Point7 = compareVersions(
					vxt_ultimate_gutenberg_blocks_blocks_info.wp_version,
					'6.7',
					'>='
				);
				...
				const gapBeforeWP6Point7 = convertToPixel(
					isAfterWP6Point7 ? '10px' : _attributes?.style?.spacing?.blockGap || '10px'
				);
				const gapAfterWP6Point7 =
					'vertical' === layoutType
						? convertToPixel( _attributes.style?.spacing?.blockGap?.top || '10px' )
						: convertToPixel( _attributes.style?.spacing?.blockGap?.left || '10px' );
				...
				const overallAlign = hasWidthFull
					? 'full'
					: ( justifyContent === 'space-between' && 'center' ) || justifyContent;
				// Map `justifyContent` to a vertical alignment option based on `justifyMap`.
				const verticalAlign = justifyMap[ justifyContent ];
				// Create and return a 'vexaltrix/buttons' block with the configured properties.
				return createBlock(
					'vexaltrix/buttons',
					{
