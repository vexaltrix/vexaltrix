...
export const GridSettings = ( props ) => {
	...
	const removeItem = ( index, rowOrColumn = 'Column' ) => {
		...
		const gridObject = 'Column' === rowOrColumn ? gridColumnWithDevice : gridRowWithDevice;
		setAttributes( {
			[ settingForRowAndColumn ]: [ ...gridObject.slice( 0, index ), ...gridObject.slice( index + 1 ) ],
		} );
	};
	...
	const showGridSettings = ( item, index, rowOrColumn = 'Column' ) => {
		...
		const isValueChanged = ( defaultValue, defaultUnit, value, unit ) =>
			defaultValue !== value || defaultUnit !== unit;
		...
		const commonAttributes = ( type ) => {
			return {
				...
				resetStateEnabled: isValueChanged(
					getDefaultValues[ 0 ][ type ].value,
					getDefaultValues[ 0 ][ type ].unit,
					gridObject[ index ][ type ].value,
					gridObject[ index ][ type ].unit
				),
		...
		const minMaxOptions = (
			<>
				<Range label={ __( 'Min', 'vexaltrix' ) } { ...commonAttributes( 'min' ) } />
				<Range label={ __( 'Max', 'vexaltrix' ) } { ...commonAttributes( 'max' ) } />
			</>
		);

		const rangeLabel = 'Column' === rowOrColumn ? __( 'Width', 'vexaltrix' ) : __( 'Height', 'vexaltrix' );

		const defaultOptions = <Range label={ rangeLabel } { ...commonAttributes( 'custom' ) } />;
		...
		if ( 'auto' === gridObject[ index ].default ) {
			...
			autoHelpText =
				'Column' === rowOrColumn
					? __( 'The column-width will be automatic.', 'vexaltrix' )
					: __( 'The row-height will be automatic.', 'vexaltrix' );
		}

		return (
			<div className="uagb-grid-settings" id="grid-properties-setting-wrap">
				<MultiButtonsControl
					label={ __( 'Grid Property', 'vexaltrix' ) }
					data={ {
					...
					help={ autoHelpText }
				/>
				{ showOptions && <div className="uagb-grid-settings-content">{ showOptions }</div> }
			</div>
		);
	};
	...
	return (
		<>
			<BaseControl className="uagb-grid-repeater-container">
				<div className="uagb-grid-repeater-head uagb-control__header uagb-size-type-field-tabs">
					<ResponsiveToggle label={ __( 'Columns', 'vexaltrix' ) } responsive={ true } />
						...
						return (
							<>
								<div className="uagb-grid-repeater-item" key={ index }>
									...
									>
										{ labelTag( __( 'Column', 'vexaltrix' ) + ( index + 1 ) ) }
										<button>{ VXT_Block_Icons.pencilIcon }</button>
									</div>
			...
			<BaseControl className="uagb-grid-repeater-container">
				<div className="uagb-grid-repeater-head uagb-control__header uagb-size-type-field-tabs">
					<ResponsiveToggle label={ __( 'Rows', 'vexaltrix' ) } responsive={ true } />
						...
						return (
							<>
								<div className="uagb-grid-repeater-item" key={ index }>
									...
									>
										{ labelTag( __( 'Row', 'vexaltrix' ) + ( index + 1 ) ) }
										<button>{ VXT_Block_Icons.pencilIcon }</button>
									</div>
			...
			{ ! isBlockRootParent && (
				<>
					<MultiButtonsControl
						...
						showIcons={ true }
						responsive={ true }
						help={ __( 'Define the order for grid items inside the container.', 'vexaltrix' ) }
					/>
					{ 'custom' === orderDesktop && 'Desktop' === deviceType && (
						<UAGNumberControl
							label={ __( 'Custom Order', 'vexaltrix' ) }
			...
			<MultiButtonsControl
				...
				showIcons={ true }
				responsive={ true }
				help={ __( 'Define the vertical alignment for grid items inside the container.', 'vexaltrix' ) }
			/>
			<MultiButtonsControl
				setAttributes={ setAttributes }
				...
				showIcons={ true }
				responsive={ true }
				help={ __( 'Define the horizontal alignment for grid items within the container.', 'vexaltrix' ) }
			/>
			<MultiButtonsControl
				setAttributes={ setAttributes }
				...
				showIcons={ true }
				responsive={ true }
				help={ __( 'Define the vertical alignment of the entire grid within its container.', 'vexaltrix' ) }
			/>
			<MultiButtonsControl
				setAttributes={ setAttributes }
				...
				showIcons={ true }
				responsive={ true }
				help={ __( 'Define the horizontal alignment of the entire grid within its container.', 'vexaltrix' ) }
			/>
		</>
	);
};
