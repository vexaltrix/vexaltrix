...
function styling( attributes, clientId, name, deviceType, gbsSelector = false ) {
	...
	if ( backgroundVideoFallbackImage?.url ) {
		selectors[
			' .uagb-container__video-wrap video'
		].background = `url(${ backgroundVideoFallbackImage.url }) 50% 50%;`;
	...
	if ( 'alignfull' === contentWidth && 'alignwide' === innerContentWidth ) {
		...
		widthSelectorsTablet[
			`.block-editor-block-list__block.uagb-editor-preview-mode-tablet.wp-block-uagb-container${ gbsWidthSelector } > .uagb-container-inner-blocks-wrap`
		] = {
	...
	if ( overlayType && 'none' !== overlayType && Object.keys( getContainerBGStyle ).length ) {
		...
		selectors[ '.wp-block-uagb-container::before' ] = {
			...
			width: `calc(100% + ${ generateCSSUnit( desktopBorderWidth.left, 'px' ) } + ${ generateCSSUnit(
				desktopBorderWidth.right,
				'px'
			) })`,
			height: `calc(100% + ${ generateCSSUnit( desktopBorderWidth.top, 'px' ) } + ${ generateCSSUnit(
				desktopBorderWidth.bottom,
				'px'
			) })`,
			...borderCSS,
			...getContainerBGStyle,
	...
	const containerSelector2 =
		'.wp-block-uagb-container > .uagb-container-inner-blocks-wrap > .block-editor-inner-blocks > .block-editor-block-list__layout > .wp-block';
	...
	const base_selector = gbsSelector
		...
		: `.editor-styles-wrapper #block-${ clientId }`;
