...
const VexaltrixContainer = ( props ) => {
	const {
		isSelected,
		attributes,
		attributes: {
	...
	const { isParentOfSelectedBlock, variations, defaultVariation, getBlockParents, parentBlocks, parentAttributes } =
		useSelect( ( select ) => {
	...
	const checkSpecialBlockAvailability = useMemo( () => {
		...
		const specialBlocks = {
			sliderBlocks: [ 'vexaltrix/slider', 'vexaltrix/slider-child' ],
			popupBlocks: [ 'vexaltrix/modal', 'vexaltrix/popup-builder' ],
		};
	...
	useEffect( () => {
		...
		if (
			...
			! parentBlocks.some( ( parent ) => parent.name === 'vexaltrix/container' )
		) {
		...
		if (
			attributesToUpdate.isBlockRootParent !== attributes.isBlockRootParent ||
...
export default compose( containerWrapper, addInitialAttr, AddStaticStyles, AddGBSStyles )( VexaltrixContainer );
