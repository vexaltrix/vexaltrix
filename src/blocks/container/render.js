...
const Render = ( props ) => {
	...
	if ( hasSliderParent || hasPopupParent ) {
		...
		if ( hasSliderParent ) {
			ALLOWED_BLOCKS = ALLOWED_BLOCKS.filter(
				( blockName ) => [ 'vexaltrix/slider' ].indexOf( blockName ) === -1
			);
		}
		...
		if ( hasPopupParent ) {
			ALLOWED_BLOCKS = ALLOWED_BLOCKS.filter(
				( blockName ) => [ 'vexaltrix/modal' ].indexOf( blockName ) === -1
			);
		}
	...
	if ( parentBlockNames.includes( 'vexaltrix/loop-builder' ) ) {
		const allowedBlocks = [
