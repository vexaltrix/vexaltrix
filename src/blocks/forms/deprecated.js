...
const attributes = {
	...
	submitButtonText: {
		type: 'string',
		default: __( 'Submit', 'vexaltrix' ),
	},
	formLabel: {
		type: 'string',
		default: __( 'UAG Form', 'vexaltrix' ),
	},
	buttonAlign: {
		type: 'string',
		default: 'left',
	...
	confirmationMessage: {
		type: 'string',
		default: __( 'The form has been submitted successfully!', 'vexaltrix' ),
	},
	failedMessage: {
		type: 'string',
		default: __(
	...
	afterSubmitEmailSubject: {
		type: 'string',
		default: __( 'Form Submission', 'vexaltrix' ),
	},
	submitColor: {
		type: 'string',
		default: '#ffffff',
	...
	captchaMessage: {
		type: 'string',
		default: __( 'Please fill up the above captcha.', 'vexaltrix' ),
	},
	fieldGapType: {
		type: 'string',
		default: 'px',
...
const deprecated = [
	{
		...
		save( props ) {
			...
			const renderButtonHtml = () => {
				return (
					<button className="vxt-forms-main-submit-button">
						<RichText.Content
							tagName="div"
			...
			return (
				...
				>
					<form className="vxt-forms-main-form" method="post" name={ `vxt-form-${ block_id }` }>
						<InnerBlocks.Content />
						...
						{ reCaptchaEnable && 'v2' === reCaptchaType && reCaptchaSiteKeyV2 && reCaptchaSecretKeyV2 && (
							<>
								<div
									className="g-recaptcha vxt-forms-field-set"
									data-sitekey={ reCaptchaSiteKeyV2 }
								></div>
								<div className={ `vxt-form-reacaptcha-error-${ block_id }` }></div>
							</>
						) }
						<div className="vxt-forms-main-submit-button-wrap">{ renderButtonHtml() }</div>
					</form>
