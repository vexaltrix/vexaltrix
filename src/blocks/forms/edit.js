...
const VexaltrixFormsEdit = ( props ) => {
	const {
		isSelected,
		attributes,
		attributes: {
	...
	const { variations, hasInnerBlocks, defaultVariation } = useSelect( ( select ) => {
		const { getBlockVariations, getDefaultBlockVariation } = select( 'core/blocks' );

		return {
			hasInnerBlocks: select( 'core/block-editor' ).getBlocks( clientId ).length > 0,
			defaultVariation: typeof getDefaultBlockVariation === 'undefined' ? null : getDefaultBlockVariation( name ),
	...
	useEffect( () => {
		...
		if ( reCaptchaEnable ) {
			const keys = {};
			if (
				'' === vxt_ultimate_gutenberg_blocks_blocks_info.recaptcha_site_key_v2 &&
				'' === vxt_ultimate_gutenberg_blocks_blocks_info.recaptcha_secret_key_v2 &&
				reCaptchaSiteKeyV2 &&
				reCaptchaSecretKeyV2
			) {
			...
			if (
				'' === vxt_ultimate_gutenberg_blocks_blocks_info.recaptcha_site_key_v3 &&
				'' === vxt_ultimate_gutenberg_blocks_blocks_info.recaptcha_secret_key_v3 &&
				reCaptchaSiteKeyV3 &&
				reCaptchaSecretKeyV3
			) {
			...
			if ( Object.keys( keys ).length !== 0 ) {
				...
				const getApiFetchData = getApiData( {
					...
					action: 'vxt_ultimate_gutenberg_blocks_forms_recaptcha',
					data,
				} );
	...
	const renderReadyClasses = useCallback( ( id ) => {
		...
		if ( iframeEl ) {
			...
			formscope = mainDiv.getElementsByClassName( 'vxt-forms__outer-wrap' );
		} else {
...
const addAdvancedClasses = createHigherOrderComponent( ( BlockListBlock ) => {
	return ( props ) => {
		...
		const addClassNames = uagbClassNames( [ props?.className || '', props?.attributes?.className || '' ] );
		return <BlockListBlock { ...props } className={ addClassNames ? addClassNames : null } />;
	};
}, 'addAdvancedClasses' );

addFilter( 'editor.BlockListBlock', 'vexaltrix/forms', addAdvancedClasses );

export default compose( withNotices, addAdvancedClasses, addInitialAttr, AddStaticStyles )( VexaltrixFormsEdit );
