	...
	if ( setAttributes !== 'not_set' ) {
		return (
			<div className="uagb-ifb-cta">
				...
				{ attributes.ctaType === 'text' && ! attributes.inheritFromTheme && (
					<a target={ target } className={ classnames( 'uagb-infobox-cta-link' ) } rel={ rel }>
						{ ctaTextOutput }
					</a>
				) }
				{ attributes.ctaType === 'button' && (
					<div className={ classnames( 'uagb-ifb-button-wrapper', 'wp-block-button' ) }>
						<CustomTag className={ ctaBtnClass } target={ target } rel={ rel } style={ borderStyle }>
							{ ctaTextOutput }
						</CustomTag>
					</div>
				) }
	...
	return (
		<>
			<div
				className={ classnames(
					'uagb-ifb-button-wrapper',
					attributes.ctaType === 'text' && attributes.inheritFromTheme ? 'uagb-ifb-button-type-text' : '',
					'wp-block-button'
				) }
			>
				{ attributes.ctaType === 'text' && attributes.inheritFromTheme && (
