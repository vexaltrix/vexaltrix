	...
	const desc = enableMultilineParagraph ? (
		...
		<>
			...
			{ showDesc && '' !== headingDesc && (
				<RichText.Content tagName="p" value={ attributes.headingDesc } className="vxt-ifb-desc" />
			) }
			{ renderSeparator( 'after_desc' ) }
			{ ctaType !== 'none' && <CallToAction attributes={ attributes } /> }
		</>
		...
		<div className="vxt-ifb-title-wrap">
			{ showPrefix && '' !== prefixTitle && (
				<RichText.Content
					...
					className="vxt-ifb-title-prefix"
				/>
			) }
	...
	return (
		<CustomTag
			className={ uagbClassNames( [
				...
				hasMargin ? 'wp-block-vxt-info-box--has-margin' : '',
			] ) }
		>
			{ hasMargin ? (
				<div className="vxt-infobox-margin-wrapper">{ ifbInnerContent }</div>
			) : (
				<>{ ifbInnerContent }</>
			) }
		</CustomTag>
