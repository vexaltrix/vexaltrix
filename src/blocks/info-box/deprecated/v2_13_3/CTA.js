...
const InfoBoxCta = ( props ) => {
	...
	const ctaBtnClass = `vxt-infobox-cta-link ${ buttonTypeClass }`;
			...
			<div className="vxt-ifb-cta">
				{ attributes.ctaType === 'text' && attributes.inheritFromTheme && (
					<div
						className={ classnames(
							'vxt-ifb-button-wrapper',
							'vxt-ifb-button-type-text',
							'wp-block-button'
						) }
					>
				...
				{ attributes.ctaType === 'text' && ! attributes.inheritFromTheme && (
					<a // eslint-disable-line jsx-a11y/anchor-is-valid
						target={ target }
						className={ classnames( 'vxt-infobox-cta-link' ) }
						rel={ rel }
					>
						{ ctaTextOutput }
					</a>
	...
	return (
		<>
			<div
				className={ classnames(
					'vxt-ifb-button-wrapper',
					attributes.ctaType === 'text' && attributes.inheritFromTheme ? 'vxt-ifb-button-type-text' : '',
					'wp-block-button'
				) }
			>
				{ attributes.ctaType === 'text' && attributes.inheritFromTheme && (
					<>
						<a
							...
							target={ target }
							onClick={
								( '' === attributes.ctaLink || '#' === attributes.ctaLink ) &&
								! attributes?.dynamicContent?.ctaLink?.enable
									? 'return false;'
									: 'return true;'
							}
							className={ ctaBtnClass }
				...
				{ attributes.ctaType === 'text' && ! attributes.inheritFromTheme && (
					<>
						<a
							...
							className={ 'vxt-infobox-cta-link' }
							rel={ rel }
							alt=""
						>
							{ ctaTextOutputFrontend }
