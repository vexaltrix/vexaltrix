...
import newSaveV2_7_4 from './deprecated/v2_7_4/save';
...
const attributes = {
	...
	prefixTitle: {
		source: 'html',
		selector: 'span.vxt-ifb-title-prefix',
		default: 'Prefix',
	},
	classMigrate: {
		type: 'boolean',
...
const deprecated = [
		...
		save( props ) {
			...
			const desc = (
				<>
					...
					<div className="vxt-ifb-text-wrap">
			...
			const output = (
				<>
					...
					>
						<div className="vxt-ifb-left-right-wrap">
			...
			return (
				<>
					<div
						className={ classnames( className, 'vxt-infobox__outer-wrap' ) }
						id={ `vxt-infobox-${ block_id }` }
					>
						{ ctaType == 'all' && (
							<>
								<a
									href={ ctaLink }
									className="vxt-infobox-link-wrap"
									target={ target }
					...
					>
						{ ctaType == 'all' && (
							<>
								<a
									href={ ctaLink }
									className="vxt-infobox-link-wrap vxt-infbox__link-to-all"
									target={ target }
									rel="noopener noreferrer"
								></a>
								{ output }
	...
	},
	{
		...
		save( props ) {
			...
			return (
				<>
					<div className={ classnames( className, 'vxt-infobox__outer-wrap', `vxt-block-${ block_id }` ) }>
						{ ctaType == 'all' && (
							<>
								<a
									href={ ctaLink }
	...
	},
	{
		...
		save( props ) {
			...
			if ( source_type === 'icon' && icon !== '' ) {
				isImage = <IconNewVer attributes={ props.attributes } />;
			} else {
				isImage = <InfoBoxIconImageNewVer attributes={ props.attributes } />;
			}
			...
			return (
				<div
					className={ classnames(
						...
						'vxt-infobox__content-wrap',
						ctaType === 'all' ? ' vxt-infobox_cta-type-all' : '',
						...InfoBoxPositionClassesNewVer( props.attributes )
					) }
				>
					{ ctaType === 'all' && (
						<a
	...
	{
		attributes: newAttributesV2_7_4,
		save: newSaveV2_7_4,
	},
	{
