...
const Render = ( props ) => {
	...
	const addTab = () => {
		const tabItemBlock = createBlock( 'vexaltrix/tabs-child' );
			...
			>
				<ul className={ `uagb-tabs__panel uagb-tabs__align-${ tabAlign }` }>
						...
						>
							{ tabHeaders.length > 0 && (
								<div className="uagb-tabs-editor-controls">
									{ index !== 0 && (
										<Tooltip text={ __( 'Move item back', 'vexaltrix' ) }>
											<span
												role="button"
									...
									{ index + 1 !== tabHeaders.length && (
										<Tooltip text={ __( 'Move item forward', 'vexaltrix' ) }>
											<span
												role="button"
									...
									<Tooltip text={ __( 'Remove tab', 'vexaltrix' ) }>
										<span
											role="button"
											tabIndex={ index }
					...
					<li
						...
						onClick={ () => addTab() }
					>
						<Tooltip text={ __( 'Add tab', 'vexaltrix' ) }>
							<Dashicon icon="plus" />
						</Tooltip>
					</li>
				</ul>
				<div className="uagb-tabs__body-wrap">
					<InnerBlocks
						...
						templateLock={ false }
						allowedBlocks={ [ 'vexaltrix/tabs-child' ] }
