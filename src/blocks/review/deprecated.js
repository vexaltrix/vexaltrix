...
const deprecated = [
	{
		...
		save: function save( props ) {
			...
			if ( mainimage && mainimage.url ) {
				image_icon_html = <img className="vxt-howto__source-image" src={ url } title={ title } />;
			}
			...
			return (
				<div
					className={ classnames(
						className,
						'vxt-ratings__outer-wrap',
						`vxt-block-${ block_id.substr( 0, 8 ) }`
					) }
				>
					...
					<div className="vxt_ultimate_gutenberg_blocks_review_block">
						<a
							href={ ctaLink }
							className={ classnames( 'vxt-rating-link-wrapper' ) }
							target={ target }
							rel={ rel }
						>
						...
						{ enableImage === true && <div className="vxt-rating__source-wrap">{ image_icon_html }</div> }
						{ parts.map(
							( j, i ) =>
								showFeature === true && (
	...
	},
	{
		...
		save: function save( props ) {
			...
			if ( mainimage && mainimage.url ) {
				imageIconHtml = (
					<img className="vxt-howto__source-image" src={ url } title={ title } alt={ defaultedAlt } />
				);
			}
