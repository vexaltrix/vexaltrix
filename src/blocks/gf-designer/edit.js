...
const VexaltrixGF = ( props ) => {
	...
	useSelect( ( select ) => {
		...
		if ( formId && -1 !== formId && 0 !== formId && ! isHtml ) {
			...
			const getApiFetchData = getApiData( {
				...
				action: 'vxt_ultimate_gutenberg_blocks_gf_shortcode',
				data,
			} );
	...
	if ( formId === 0 ) {
		return (
			<Placeholder icon="admin-post" label={ __( 'Select a Gravity Form', 'vexaltrix' ) }>
				<SelectControl
					value={ formId }
...
export default compose( addInitialAttr, AddStaticStyles )( VexaltrixGF );
