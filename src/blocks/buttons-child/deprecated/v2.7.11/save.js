...
export default function save( props ) {
	...
	const { block_id, opensInNewTab, link, label, icon, iconPosition, removeText, noFollow, showIcon, dynamicContent } =
		attributes;

	const btnText = () => {
		if ( ! removeText ) {
			return <RichText.Content value={ label } tagName="div" className="uagb-button__link" />;
		}
		return '';
	};
		...
		>
			<div className="uagb-button__wrapper">
				<a // eslint-disable-line jsx-a11y/anchor-is-valid
					className={ classnames( 'uagb-buttons-repeater', 'wp-block-button__link' ) }
					href={ '' === link || dynamicContent?.link?.enable ? '#' : link }
					onClick={
						( '' === link || '#' === link ) && ! dynamicContent?.link?.enable
							? 'return false;'
							: 'return true;'
					}
