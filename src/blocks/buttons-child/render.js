...
const Render = ( props ) => {
	...
	const { className, icon, iconPosition, removeText, noFollow, showIcon, block_id, buttonType, inheritFromTheme } =
		attributes;
	...
	const btnAllowedFormats = vxt_ultimate_gutenberg_blocks_blocks_info.vexaltrix_pro_status
		? [ 'vexaltrix/dynamic-content' ]
		: [];
	const btnText = () => {
		if ( ! removeText ) {
			return (
				<RichText
					placeholder={ __( 'Add text…', 'vexaltrix' ) }
					value={ label }
					tagName="div"
					onChange={ ( value ) => {
	...
	return (
		...
		>
			<div className="uagb-button__wrapper">
				<div className={ classnames( 'uagb-buttons-repeater', `${ buttonTypeClass }` ) } style={ borderStyle }>
					{ iconHtml( 'before' ) }
					{ btnText() }
					{ iconHtml( 'after' ) }
				</div>
