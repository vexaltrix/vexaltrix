...
const VexaltrixCountdownEdit = ( props ) => {
	const {
		isSelected,
		clientId,
		attributes,
	...
	useEffect( () => {
		...
		if ( countdownRef && block_id ) {
			countdownInterval = setTimeout( () => {
				VexaltrixCountdown.editorInit( '.vxt-block-' + block_id, attributes, countdownRef.current );
			} );
		}
		return () => {
	...
	useEffect( () => {
		if ( block_id && timeChanged === 1 ) {
			VexaltrixCountdown.changeEndTime( '.vxt-block-' + block_id, attributes, countdownRef.current );
		}
		setTimeChanged( 1 );
	...
	const countdownToolbar = applyFilters( 'vexaltrix.countdown.toolbar-hook', '', name );
...
export default compose( addInitialAttr, AddStaticStyles, AddGBSStyles )( VexaltrixCountdownEdit );
