...
function styling( attributes, clientId, name, deviceType, gbsSelector = false ) {
	...
	const base_selector =
		...
		( gbsSelector ? gbsSelector + ' ' : `#block-${ clientId } .uagb-block-${ block_id }` );

	selectors = applyFilters( `vexaltrix.${ blockName }.styling`, selectors, attributes );
	tablet_selectors = applyFilters( `vexaltrix.${ blockName }.tabletStyling`, tablet_selectors, attributes );
	mobile_selectors = applyFilters( `vexaltrix.${ blockName }.mobileStyling`, mobile_selectors, attributes );
