	...
	from: [
		{
			...
			transform: ( attributes ) => {
				const maxGap = Math.max(
					getNumber( attributes?.style?.spacing?.blockGap?.left || '8' ),
					getNumber( attributes?.style?.spacing?.blockGap?.top || '8' )
				);
				return createBlock( 'vexaltrix/image-gallery', {
					columnsDesk: attributes?.columns || 3,
					readyToRender: true,
					mediaGallery: attributes.images
						? attributes.images.map( ( image ) =>
								pickRelevantMediaFiles( image, 'thumbnail', 'medium', 'large', 'full', null )
						  )
						: [],
					...
					imageClickEvent:
						attributes?.linkTo === 'attachment' || attributes?.linkTo === 'media' ? 'image' : 'none',
