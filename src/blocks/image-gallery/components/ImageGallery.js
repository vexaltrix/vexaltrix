...
const ImageGallery = ( { attributes, setAttributes, name } ) => {
					...
					if ( tiledImages.current[ image.id ] !== undefined && tiledImages.current[ image.id ] !== null ) {
						if (
							...
							! tiledImages.current[ image.id ].classList.contains(
								'vexaltrix-image-gallery__media-wrapper--focus'
							)
						) {
							tiledImages.current[ image.id ].classList.add(
								'vexaltrix-image-gallery__media-wrapper--focus'
							);
						}
					}
	...
	useEffect( () => {
		if ( 'tiled' === feedLayout ) {
			setTimeout( () => {
				...
				imagesLoaded( gallery ).on( 'always', () => {
					setTileResizeTrigger( true );
				} );
			}, 250 );
		}
	...
	useEffect( () => {
		...
		Object.keys( updateFocusListObject ).forEach( ( currentImageID ) => {
			...
			if (
				...
				tiledImages.current[ currentImageID ].classList.add( 'vexaltrix-image-gallery__media-wrapper--focus' );
			} else if ( ! updateFocusListObject[ currentImageID ] ) {
				tiledImages.current[ currentImageID ].classList.remove(
					'vexaltrix-image-gallery__media-wrapper--focus'
				);
	...
	const SlickNextArrow = () => (
		<button
			...
			className={ uagbClassNames( [
				'vexaltrix-image-gallery__control-arrows',
				`vexaltrix-image-gallery__control-arrows--${ feedLayout }`,
		...
		>
			{ VXT_Block_Icons.carousel_right }
		</button>
	);
	...
	const svgFocus = ( imageId ) => (
		<button
			className="vexaltrix-image-gallery__control-focus"
			onClick={ () => {
	...
	const createSpecialTile = ( instance, image ) => {
		if ( image && image.isLoaded ) {
			...
			if ( ! tileWrapper.classList.contains( 'vexaltrix-image-gallery__media-wrapper--focus' ) ) {
				...
				if ( imageElement.naturalWidth >= imageElement.naturalHeight * 2 - imageElement.naturalHeight / 2 ) {
					tileWrapper.classList.add( 'vexaltrix-image-gallery__media-wrapper--wide' );
					...
					tileWrapper.classList.add( 'vexaltrix-image-gallery__media-wrapper--tall' );
	...
	const renderGridPagintion = () => (
		<div className="vexaltrix-image-gallery__control-wrapper">
			<SlickPrevArrow />
			<ul className="vexaltrix-image-gallery__control-dots">
				{ [ ...Array( gridPages ) ].map( ( pageData, pageIndex ) => (
					<li
						key={ pageIndex }
						className={ uagbClassNames( [
							'vexaltrix-image-gallery__control-dot',
							pageIndex === gridPageNumber - 1 && 'vexaltrix-image-gallery__control-dot--active',
						] ) }
	...
	const renderMasonryPagintion = () => {
		...
		return (
			<>
				{ paginateUseLoader ? (
					<div className="vexaltrix-image-gallery__control-loader wp-block-button">
						<div className="wp-block-button__link vexaltrix-image-gallery__control-loader--1"></div>
						<div className="wp-block-button__link vexaltrix-image-gallery__control-loader--2"></div>
						<div className="wp-block-button__link vexaltrix-image-gallery__control-loader--3"></div>
					</div>
				) : (
					<div className="vexaltrix-image-gallery__control-wrapper wp-block-button">
						<RichText
							tagName="div"
							placeholder={ __( 'Load More Images', 'vexaltrix' ) }
							...
							className="vexaltrix-image-gallery__control-button wp-block-button__link"
							multiline={ false }
		...
		switch ( feedLayout ) {
					...
					<>
						<div
							className={ uagbClassNames( [
								...
								`vexaltrix-image-gallery__layout--${ gridLayout }`,
								`vexaltrix-image-gallery__layout--${ gridLayout }-col-${ columnsDeskFallback }`,
								`vexaltrix-image-gallery__layout--${ gridLayout }-col-tab-${ columnsTabFallback }`,
								`vexaltrix-image-gallery__layout--${ gridLayout }-col-mob-${ columnsMobFallback }`,
							] ) }
						>
							{ renderImageLooper() }
						</div>
				...
				return (
					<>
						<Masonry
							className={ uagbClassNames( [
								'vexaltrix-image-gallery',
								`vexaltrix-image-gallery__layout--${ feedLayout }`,
								`vexaltrix-image-gallery__layout--${ feedLayout }-col-${ columnsDeskFallback }`,
								`vexaltrix-image-gallery__layout--${ feedLayout }-col-tab-${ columnsTabFallback }`,
								`vexaltrix-image-gallery__layout--${ feedLayout }-col-mob-${ columnsMobFallback }`,
							] ) }
			...
			case 'tiled':
				return (
					...
					>
						{ renderImageLooper() }
						<div className="vexaltrix-image-gallery__media-sizer" ref={ tileSizer }></div>
					</div>
				);
			case 'carousel':
				return (
					...
					>
						<Slider className="vxt-slick-carousel" { ...carouselSettings }>
							{ renderImageLooper() }
						</Slider>
					</div>
				);
	...
	const renderMasonryHoverHandler = ( mediaObject ) => (
		<div className="vexaltrix-image-gallery__media-wrapper--isotope" key={ mediaObject.id }>
	...
	const renderImage = ( mediaObject ) => (
		<div
			className="vexaltrix-image-gallery__media-wrapper"
			key={ mediaObject.id }
		...
		<>
			{ imageDisplayCaption &&
				...
				'top' === getMatrixAlignment( imageCaptionAlignment, 1 ) && (
					<div
						className={ uagbClassNames( [
							'vexaltrix-image-gallery__media-thumbnail-caption-wrapper',
							`vexaltrix-image-gallery__media-thumbnail-caption-wrapper--${ captionDisplayType }`,
						] ) }
					>
			...
			<div
				className={ uagbClassNames( [
					'vexaltrix-image-gallery__media',
					`vexaltrix-image-gallery__media--${ feedLayout }`,
				] ) }
			>
				<img
					className={ uagbClassNames( [
						'vexaltrix-image-gallery__media-thumbnail',
						`vexaltrix-image-gallery__media-thumbnail--${ feedLayout }`,
					] ) }
					...
					tabIndex="0"
				/>
				<div className="vexaltrix-image-gallery__media-thumbnail-blurrer" />
				{ imageDisplayCaption ? (
					'bar-outside' !== captionDisplayType && (
						<div
							className={ uagbClassNames( [
		...
		mediaObject.caption ? (
			<RichText
				...
				className={ uagbClassNames( [
					...
					`vexaltrix-image-gallery__media-thumbnail-caption--${ captionDisplayType }`,
				] ) }
				multiline={ false }
			/>
		) : mediaObject.url ? (
			<RichText
				tagName="div"
				placeholder={ __( 'No Caption', 'vexaltrix' ) }
	...
	const renderEmpty = () => (
		<div className="vexaltrix-image-gallery-init-wrapper">
			...
			<h3>{ __( 'Image Gallery', 'vexaltrix' ) }</h3>
			<p>{ __( 'No images to display…', 'vexaltrix' ) }</p>
		</div>
	);
	...
	return mediaGallery ? (
		<>
			{ renderGallery() }
			{ applyFilters(
				'vexaltrix.image-gallery.render.customLinks',
				null,
				tiledImages,
				attributes,
				setAttributes
