...
function styling( attributes, clientId, name, deviceType ) {
	...
	let selectors = {
		...
		'.wp-block-vxt-image-gallery': {
			padding: generateSpacing(
				...
				'carousel' === feedLayout && mediaGallery?.length > 1
					? feedMarginBottomFallback + 5
					: feedMarginBottomFallback, // Editor Only Fix for the Carousel Dots.
		...
		' .vexaltrix-image-gallery__control-arrows:hover svg': {
			fill: paginateColorHover,
		},
		' .vexaltrix-image-gallery__control-arrows--carousel': {
			...arrowBorderCSS,
		},
		' .vexaltrix-image-gallery__control-arrows--carousel svg': {
		...
		' .vexaltrix-image-gallery__layout--carousel': {
			...
			'margin-bottom':
				mediaGallery?.length > 1
					? `${ generateCSSUnit( paginateDotDistanceFallback, 'px' ) } !important`
					: '0px !important',
		},
		' .vexaltrix-image-gallery__layout--carousel .vexaltrix-image-gallery__media-wrapper': {
			padding:
				mediaGallery?.length > 1 ? generateSpacing( gridImageGapUnit, 0, gridImageGapFallback / 2 ) : undefined,
		},
		' .vexaltrix-image-gallery__layout--carousel .slick-list': {
			margin:
				mediaGallery?.length > 1
					? generateSpacing( gridImageGapUnit, 0, -( gridImageGapFallback / 2 ) )
					: undefined,
		},
		...
		' .vexaltrix-image-gallery__media-thumbnail-caption-wrapper--overlay a': {
		...
		' .vexaltrix-image-gallery__media-thumbnail-caption--bar-inside': {
		...
		' .vexaltrix-image-gallery__media-thumbnail-caption--bar-outside': {
		...
		' .vexaltrix-image-gallery__control-lightbox--caption': {
		...
		' .vexaltrix-image-gallery__control-lightbox--thumbnails-wrapper': {
			'background-color': lightboxDisplayCaptions ? lightboxCaptionBackgroundColor : 'transparent',
		},
		' .vexaltrix-image-gallery__control-lightbox--count': {
		...
		' .vexaltrix-image-gallery__control-lightbox--close': {
			top: generateCSSUnit( lightboxEdgeDistance, 'px' ),
			right: generateCSSUnit( lightboxEdgeDistance, 'px' ),
		},
		' .vexaltrix-image-gallery__control-lightbox--close svg': {
			width: generateCSSUnit( lightboxIconSize, 'px' ),
			height: generateCSSUnit( lightboxIconSize, 'px' ),
			fill: lightboxIconColor,
		},
		' .vexaltrix-image-gallery__control-lightbox--main .swiper-button-prev': {
			left: generateCSSUnit( lightboxEdgeDistance, 'px' ),
			color: lightboxIconColor,
		},
		' .vexaltrix-image-gallery__control-lightbox--main .swiper-button-next': {
			right: generateCSSUnit( lightboxEdgeDistance, 'px' ),
			color: lightboxIconColor,
		},
	};
	...
	let tabletSelectors = {
		'.wp-block-vxt-image-gallery': {
			padding: generateSpacing(
				...
				'carousel' === feedLayout && mediaGallery?.length > 1
					? feedMarginBottomTabFallback + 5
					: feedMarginBottomTabFallback, // Editor Only Fix for the Carousel Dots.
		...
		' .vexaltrix-image-gallery__layout--carousel .vexaltrix-image-gallery__media-wrapper': {
			padding:
				mediaGallery?.length > 1
					? generateSpacing( gridImageGapUnitTab, 0, gridImageGapTabFallback / 2 )
					: undefined,
		},
		' .vexaltrix-image-gallery__layout--carousel .slick-list': {
			margin:
	...
	let mobileSelectors = {
		'.wp-block-vxt-image-gallery': {
			padding: generateSpacing(
				...
				'carousel' === feedLayout && mediaGallery?.length > 1
					? feedMarginBottomMobFallback + 5
					: feedMarginBottomMobFallback, // Editor Only Fix for the Carousel Dots.
		...
		' .vexaltrix-image-gallery__layout--carousel .vexaltrix-image-gallery__media-wrapper': {
			padding:
				mediaGallery?.length > 1
					? generateSpacing( gridImageGapUnitMob, 0, gridImageGapMobFallback / 2 )
					: undefined,
		},
		' .vexaltrix-image-gallery__layout--carousel .slick-list': {
			margin:
	...
	switch ( captionBackgroundEffect ) {
		case 'none':
			selectors[ ' .vexaltrix-image-gallery__media-thumbnail' ] = {
				...selectors[ ' .vexaltrix-image-gallery__media-thumbnail' ],
				'-webkit-filter': `none`,
				filter: `none`,
			};
	...
	switch ( captionBackgroundEffectHover ) {
		case 'none':
			selectors[ ' .vexaltrix-image-gallery__media-wrapper:hover .vexaltrix-image-gallery__media-thumbnail' ] = {
				'-webkit-filter': `none`,
				filter: `none`,
			};
			break;
	...
	if ( ! captionBackgroundEnableBlur ) {
		selectors[ ' .vexaltrix-image-gallery__media-thumbnail-blurrer' ] = {
			...selectors[ ' .vexaltrix-image-gallery__media-thumbnail-blurrer' ],
	...
	if ( imageDisplayCaption && 'bar-outside' === captionDisplayType ) {
		if ( 'top' === imageCaptionAlignment01 ) {
			selectors[ ' .vexaltrix-image-gallery__media-thumbnail-caption-wrapper' ] = {
				...selectors[ ' .vexaltrix-image-gallery__media-thumbnail-caption-wrapper' ],
	...
	if ( 'grid' === feedLayout && feedPagination ) {
		selectors[ ' .vexaltrix-image-gallery__control-wrapper' ] = {
			'margin-top': generateCSSUnit( paginateDotDistanceFallback, paginateDotDistanceUnit ),
		};
	}
	...
	if ( 'carousel' === feedLayout && carouselSquares ) {
		selectors[ ' .vexaltrix-image-gallery__media--carousel' ] = {
			'aspect-ratio': 1,
		};
		selectors[ ' .vexaltrix-image-gallery__media-thumbnail--carousel' ] = {
			height: '100%',
			width: '100%',
			'-o-object-fit': 'cover',
			'object-fit': 'cover',
	...
	switch ( imageZoomType ) {
		case 'zoom-in':
			if ( imageEnableZoom ) {
				...
				selectors[
					' .vexaltrix-image-gallery__media-wrapper:hover .vexaltrix-image-gallery__media-thumbnail'
				] = {
					...selectors[
						' .vexaltrix-image-gallery__media-wrapper:hover .vexaltrix-image-gallery__media-thumbnail'
					],
	...
	if ( 'outset' === imageBoxShadowPosition ) {
		selectors[ ' .vexaltrix-image-gallery__media' ] = {
			...selectors[ ' .vexaltrix-image-gallery__media' ],
	...
	if ( 'outset' === imageBoxShadowPositionHover ) {
		selectors[ ' .vexaltrix-image-gallery__media-wrapper:hover .vexaltrix-image-gallery__media' ] = {
	...
	selectors = applyFilters( `vexaltrix.image-gallery.styling`, selectors, attributes );
	tabletSelectors = applyFilters( `vexaltrix.image-gallery.tabletStyling`, tabletSelectors, attributes );
	mobileSelectors = applyFilters( `vexaltrix.image-gallery.mobileStyling`, mobileSelectors, attributes );
