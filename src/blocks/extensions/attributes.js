const { enableMasonryGallery } = vxt_ultimate_gutenberg_blocks_blocks_info;
...
function addAttributes( settings ) {
	const excludeBlock =
		vxt_ultimate_gutenberg_blocks_blocks_info.vxt_ultimate_gutenberg_blocks_exclude_blocks_from_extension;
	...
	// Grid setting attributes specific to the vexaltrix/container block.
	if ( 'vexaltrix/container' === settings.name && settings?.attributes ) {
...
addFilter( 'blocks.registerBlockType', 'vexaltrix/advanced-control-block', addAttributes );
...
if ( 'enabled' === enableMasonryGallery || true === enableMasonryGallery ) {
	...
	addFilter( 'blocks.registerBlockType', 'vexaltrix/masonry-gallery', addMasonryAttribute );
}

/**
 * Adds a metadata attribute to the block settings if it is a Vexaltrix block.
 * If the block is a Vexaltrix block, it adds a metadata attribute of type 'object' with a default value of { name: '' }.
...
export function blockMetadataAttribute( settings ) {
	...
	const isVexaltrixBlock = blockName.includes( 'vexaltrix/' );
	if ( isVexaltrixBlock ) {
		settings.attributes = {
			...settings.attributes,
			metadata: {
		...
		// Don't override if already set for any vexaltrix block.
...
addFilter( 'blocks.registerBlockType', 'vexaltrix/block-label', blockMetadataAttribute );
