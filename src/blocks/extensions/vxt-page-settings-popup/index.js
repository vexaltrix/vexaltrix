...
import PageCustomCSS, { applyScopedCSS } from '../custom-page-css/index.js';
...
const VexaltrixPageSettingsPopup = ( props ) => {
	const getSidebarStore =
		'site-editor' !== vxt_ultimate_gutenberg_blocks_blocks_info.is_site_editor
			? window?.wp?.editPost
			: window?.wp?.editSite;
	...
	return (
		<>
			{ /* Page Settings Icon. */ }
			<PluginSidebarMoreMenuItem target="vexaltrix-page-settings-panel" icon={ SettingsIcons.logo }>
				{ __( 'Vexaltrix Page Settings', 'vexaltrix' ) }
			...
			<PluginSidebar
				...
				name="vexaltrix-page-settings-panel"
				title={ __( 'Vexaltrix Block Settings', 'vexaltrix' ) }
				className={ 'vexaltrix-sidebar' }
			>
				{ pluginSidebarBefore }
				{ 'site-editor' !== vxt_ultimate_gutenberg_blocks_blocks_info.is_site_editor &&
					'yes' === vxt_ultimate_gutenberg_blocks_blocks_info.enable_on_page_css_button && (
						<>
							<PanelBody
								title={ __( 'Custom CSS', 'vexaltrix' ) }
								initialOpen={ true }
								className={ 'vexaltrix-custom-css-panel' }
							>
								<PageCustomCSS />
							</PanelBody>
						</>
...
const VexaltrixSupport = () => {
	const supportModalData = {
		title: __( 'VIP Priority Support', 'vexaltrix' ),
		Image,
		header: __( 'Enjoy Uninterrupted Productivity and Complete Peace of Mind', 'vexaltrix' ),
		description: __(
			'Get the red carpet treatment! Exclusive support service designed for faster assistance and benefits.',
			'vexaltrix'
		),
		features: [
			__( 'Unmatched Speed', 'vexaltrix' ),
			__( 'Priority Treatment', 'vexaltrix' ),
			__( 'Expert Support', 'vexaltrix' ),
			__( '24/7 Peace of Mind', 'vexaltrix' ),
		],
	};
	...
	return (
		...
		>
			...
			>
				...
				>
					{ __( 'Experience peace of mind with premium support.', 'vexaltrix' ) }
					<div
						style={ {
							display: 'inline',
							color: '#0369A1',
					...
					<div className="flex justify-between items-center mt-4 gap-3">
						...
						>
							...
							<span className="text-text-tertiary">
								{ __( '/month (billed annually)', 'vexaltrix' ) }
							</span>
						</div>
