/**
 * The Quick Access React App for Vexaltrix Enhanced Editor.
 */
...
const AddEventCommandForQAB = () => {
	...
	const createCustomEvent = () => {
		// Create a new CustomEvent object with the name 'VexaltrixQABEditor'.
		const loadQABEditor = new CustomEvent( 'VexaltrixQABEditor', {
...
domReady( () => {
	...
	if ( 'disabled' === vxt_ultimate_gutenberg_blocks_blocks_info.enableQuickActionSidebar ) {
		return;
	}
	if ( '/wp-admin/site-editor.php' === currentUrl.pathname ) {
...
const toggleSidebar = ( url ) => {
	...
	if ( '/wp-admin/site-editor.php' === currentUrl.pathname ) {
		if ( 'edit' === currentUrl.searchParams.get( 'canvas' ) ) {
			...
			const container = document.querySelector( '.vexaltrix-ee-quick-access' );
			if ( container ) {
...
const attachSidebar = () => {
	const interval = setInterval( () => {
		...
		const blockElement = document.querySelector( '.vexaltrix-ee-quick-access__sidebar--blocks--block' );
		if ( blockElement ) {
		...
		let container = rootElement.querySelector( '.vexaltrix-ee-quick-access-container' );
		...
		if ( ! container ) {
			...
			container.classList.add( 'vexaltrix-ee-quick-access-container' );
		}
...
const attachSidebarAfterLoading = () => {
	const skeletonInterval = setInterval( () => {
		const skeleton =
			document.querySelector( '.edit-site-editor__interface-skeleton' ) ||
			document.querySelector( '.editor-editor-interface.edit-site-editor__editor-interface' ); // For Old user with 6.5 and below || For New user with 6.6 and above.
		if ( skeleton ) {
...
document.addEventListener( 'VexaltrixQABEditor', function ( event ) {
