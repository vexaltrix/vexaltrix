...
const DraggableBlock = ( props ) => {
	...
	const removeElementFromArray = ( arrayFromWhichElementNeedToRemove, elementToRemove ) =>
		arrayFromWhichElementNeedToRemove.filter( ( element ) => element !== elementToRemove );
	...
	const handleRemoveBlock = ( elementToRemove ) => {
		...
		createNotice(
			'success',
			sprintf(
				/* translators: abbreviation for units */ __( '%s Removed from Quick Action Bar.', 'vexaltrix' ),
	...
	const hoverPopover = (
		<Popover placement="right" key={ id } className="vexaltrix-ee-quick-access__sidebar--blocks--block--icon--name">
			<div className="block-title">
				<div
					onClick={ () => {
						handleRemoveBlock( block );
					} }
					className="vexaltrix-ee-quick-access__sidebar--blocks--block--name"
				>
					<svg
		...
		<div id={ `draggable-box__${ slug }--${ blockName }` }>
			...
			>
				{ ( { onDraggableStart, onDraggableEnd } ) => (
					<div
						className="vexaltrix-ee-quick-access__sidebar--blocks--block"
						key={ id }
						onClick={ ( e ) => {
					...
					>
						<div
							className={ `vexaltrix-ee-quick-access__sidebar-icon__${ slug } vexaltrix-ee-quick-access__sidebar-icon__${ slug }--${ blockName } vexaltrix-ee-quick-access__sidebar--blocks--block--icon` }
						>
