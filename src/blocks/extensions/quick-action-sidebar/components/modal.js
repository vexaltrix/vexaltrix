...
const PopoverModal = ( {
	...
	const saveOptionToDatabase = ( allowedBlocks ) => {
		...
		// Create an object with the vxt_ultimate_gutenberg_blocks_ajax_nonce and confirmation properties.
		const data = {
			security: vxt_ultimate_gutenberg_blocks_blocks_info.vxt_ultimate_gutenberg_blocks_ajax_nonce,
	...
	const handleBlockClick = ( selectedBlock ) => {
		...
		const allowedBlocks = [ ...getDefaultAllowedQuickSidebarBlocks, selectedBlock.name ];
		...
		createNotice(
			'success',
			sprintf(
				/* translators: abbreviation for units */ __( '%s Added to Quick Action Bar.', 'vexaltrix' ),
				selectedBlock.title
			),
			{
				type: 'snackbar',
	...
	const noParentNoAncestor = ( item ) => {
		if ( ! item.parent && ! item.ancestor && 'vexaltrix/popup-builder' !== item.name ) {
			return item;
		}
	};
	const addToSidebar = () => {
		return unusedArray.map(
			( item, index ) =>
				! [ 'vexaltrix/sure-forms', 'vexaltrix/sure-cart-product', 'vexaltrix/sure-cart-checkout' ].includes(
					item.name
				) &&
				noParentNoAncestor( item ) && (
					<div
						key={ index }
						className="vexaltrix-block-wrap"
						onClick={ () => handleBlockClick( item ) }
						style={ { cursor: 'pointer' } }
					>
						<div className="vexaltrix-ee-quick-access__sidebar--blocks--block--icon">
	...
	const alreadyPresentInSidebar = () => {
		return usedArray.map(
			( item, index ) =>
				! item.parent && (
					<div key={ index } className="vexaltrix-block-wrap">
						<div className="vexaltrix-ee-quick-access__sidebar--blocks--block--icon">
							<Icon icon={ item.icon?.src ? item.icon.src : item.icon } />
						</div>
						<div className="block-title">{ item.title }</div>
	...
	return (
		<Popover onClose={ closePopup } placement="right-start" className="vexaltrix-quick-action-block-popover">
			<SearchControl
				value={ searchTerm }
				onChange={ handleSearchChange }
				label="Search Blocks"
				className="vexaltrix-quick-action-block-popover-search"
			/>
			<div className="vexaltrix-block-container">
				{ unusedArray.some(
					( item ) => ! item.parent && item.title.toLowerCase().includes( searchTerm.toLowerCase() )
				) && (
					<div className="block-editor-inserter__panel-header vexaltrix-quick-action-block-popover-header__add-to-quick-action-bar">
						<h2 className="block-editor-inserter__panel-title">
							{ __( 'Add to Quick Action Bar', 'vexaltrix' ) }
						</h2>
					</div>
				) }
				{ addToSidebar() }
				{ usedArray.some(
					...
					<div className="block-editor-inserter__panel-header vexaltrix-quick-action-block-popover-header__already-present-in-quick-action-bar">
						<h2 className="block-editor-inserter__panel-title">
							{ __( 'Already Present in Quick Action Bar', 'vexaltrix' ) }
						</h2>
					</div>
				) }
				...
				{ ! unusedArray.some(
					...
					! usedArray.some(
						...
						<div className="block-editor-inserter__no-results">
							<p>{ __( 'No results found.', 'vexaltrix' ) }</p>
						</div>
					) }
			</div>
		</Popover>
