...
export const SortableItem = ( { id, item } ) => {
	...
	return (
		...
		>
			<div
				className={ `vexaltrix-ee-quick-access__sidebar-icon__${
					item?.name?.split( '/' )[ 0 ]
				} vexaltrix-ee-quick-access__sidebar-icon__${ item?.name?.split( '/' )[ 0 ] }--${
					item?.name?.split( '/' )[ 1 ]
				} vexaltrix-ee-quick-access__sidebar--blocks--block--icon` }
			>
...
const DragAndDropComponent = ( { initialItems, updateDefaultAllowedQuickSidebarBlocks } ) => {
	...
	const onDragStart = () => {
		...
		if ( contentDiv ) {
			contentDiv.classList.add( 'vexaltrix-ee-quick-access__sidebar--prevent-editor-interactions' );
		}
	};
	...
	const onDragEnd = ( { active, over } ) => {
		const contentDiv = document.querySelector( '.interface-interface-skeleton__content' );
		if ( contentDiv ) {
			contentDiv.classList.remove( 'vexaltrix-ee-quick-access__sidebar--prevent-editor-interactions' );
		}
	...
	return (
		...
		>
			...
			>
				<div className="vexaltrix-ee-quick-access__sidebar--blocks-sortable">
