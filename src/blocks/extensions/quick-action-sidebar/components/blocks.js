...
const Blocks = ( props ) => {
	...
	const getDefaultAllowedQuickSidebarBlocks = useSelect( ( select ) =>
		select( storeName ).getDefaultAllowedQuickSidebarBlocks()
	);
	const blocks = getBlockTypes();
	const { blockInsertionPoint, getSelectedBlockClientId, getSelectedBlockAllowedBlocks, getBlockRootClientId } =
		useSelect( ( select ) => {
	...
	return (
		<>
			...
			{ enableRearrange && <DragAndDropComponent initialItems={ sortedY } /> }
		</>
	);
};
