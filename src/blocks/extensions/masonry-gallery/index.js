...
const MasonryGallery = createHigherOrderComponent( ( BlockEdit ) => {
	return ( props ) => {
		...
		const getDocumentElement = () => {
			...
			// Get the style element with the ID 'vexaltrix-editor-styles' from the iframe document.
			const _element = document_element.getElementById( 'vexaltrix-editor-styles' );
			const styleElement = document.getElementById( 'vexaltrix-editor-styles' );
		...
		return (
			<>
				...
				{ isSelected && blockType.includes( blockName ) && imagesID.length !== 0 && (
					<InspectorControls>
						<UAGAdvancedPanelBody title={ __( 'Masonry Gallery', 'vexaltrix' ) } initialOpen={ false }>
							<ToggleControl
								label={ __( 'Enable Masonry Layout', 'vexaltrix' ) }
...
function ApplyUniqueClass( extraProps, blockType, attributes ) {
	if ( 'core/gallery' === blockType.name && attributes.masonry ) {
		extraProps.className = extraProps.className + ' vxt-block-' + attributes.block_id;
	}
...
if ( 'enabled' === enableMasonryGallery || true === enableMasonryGallery ) {
	addFilter( 'editor.BlockEdit', 'vexaltrix/masonry-gallery', MasonryGallery, 999 );

	addFilter( 'blocks.getSaveContent.extraProps', 'vexaltrix/apply-extra-class', ApplyUniqueClass );
}
