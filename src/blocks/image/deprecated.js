...
import saveV2_12_4 from './deprecated/v2_12_4/save';
import newAttributesV2_12_4 from './deprecated/v2_12_4/attributes';
...
const deprecated = [
			...
			const imageCaption = (
				<>
					{ ! RichText.isEmpty( caption ) && (
						<RichText.Content tagName="figcaption" className="vxt-image-caption" value={ caption } />
					) }
				</>
			);

			const separator = 'none' !== seperatorStyle && <div className="vxt-image-separator"></div>;

			const imageOverlayLink = (
				<a
					className={ `wp-block-vxt-image--layout-overlay-link ${ linkClass }` }
					href={ href }
					target={ linkTarget }
					rel={ getRel() }
				></a>
			...
			return (
				<div
					className={ classnames(
						...
						`vxt-block-${ block_id }`,
						'wp-block-vxt-image',
						`wp-block-vxt-image--layout-${ layout }`,
						`wp-block-vxt-image--effect-${ imageHoverEffect }`,
						`wp-block-vxt-image--align-${ align ? align : 'none' }`
					) }
				>
					<figure className="wp-block-vxt-image__figure">
						{ figureImage }
						{ layout === 'overlay' ? (
							<>
								<div className="wp-block-vxt-image--layout-overlay__color-wrapper"></div>
								<div
									className={ `wp-block-vxt-image--layout-overlay__inner ${ overlayContentPosition.replace(
										' ',
										'-'
									) }` }
								>
	...
	{
		attributes: newAttributesV2_12_4,
		save: saveV2_12_4,
	},
];
