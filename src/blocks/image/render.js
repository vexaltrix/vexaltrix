...
const Render = ( props ) => {
	...
	*/
	...
	const mediaPreview = 'not-installed' === url && (
		<img
			alt={ __( 'Edit image', 'vexaltrix' ) }
			title={ __( 'Edit image', 'vexaltrix' ) }
	...
	function uploadExternal() {
		mediaUpload( {
			...
			onFileChange( [ img ] ) {
				...
				setExternalBlob();
				createNotice( 'success', __( 'Image uploaded.', 'vexaltrix' ), {
					type: 'snackbar',
				} );
			},
			allowedTypes: ALLOWED_MEDIA_TYPES,
	...
	const placeholder = ( content ) => {
		return (
			<Placeholder
				...
				icon={ <BlockIcon icon={ VXT_Block_Icons.image } /> }
	...
	const blockProps = {
		ref,
		className: `${ className } vxt-editor-preview-mode-${ deviceType.toLowerCase() } vxt-block-${ block_id } wp-block-vxt-image--layout-${ layout } wp-block-vxt-image--effect-${ imageHoverEffect } wp-block-vxt-image--align-${
			align ? align : 'none'
		}`,
	};
	...
	if ( ! ( temporaryURL || url ) ) {
		return (
			<div className="wp-block-vxt-image vxt-image-placeholder-wrapper">
				<MediaPlaceholder
					icon={ <BlockIcon icon={ VXT_Block_Icons.image } /> }
			...
			<BlockControls group="block">
				...
				{ externalBlob && (
					<ToolbarButton
						...
						icon={ upload }
						label={ __( 'Upload external image', 'vexaltrix' ) }
					/>
				) }
			</BlockControls>
			<div { ...blockProps }>
