...
export default function Image( {
		...
		( select ) => {
			...
			return {
				multiImageSelection:
					...
					multiSelectedClientIds.every( ( _clientId ) => getBlockName( _clientId ) === 'vexaltrix/image' ),
			};
		},
		[ id, isSelected ]
	);
	...
	if ( alt ) {
		...
		defaultedAlt = sprintf(
			...
			__( 'This image has an empty alt attribute; its file name is %s', 'vexaltrix' ),
			filename
		);
	} else {
		defaultedAlt = __( 'This image has an empty alt attribute', 'vexaltrix' );
	}
	...
	if ( title ) {
		...
		defaultedTitle = sprintf(
			/* translators: %s: file name */
			__( 'This image has an empty title attribute; its file name is %s', 'vexaltrix' ),
			filename
		);
	} else {
		defaultedTitle = __( 'This image has an empty title attribute', 'vexaltrix' );
	}
	...
	let img = (
		...
		<>
			<img
				srcSet={
					shouldSrcSet
						? `${ temporaryURL || url } ${ urlTablet ? ',' + urlTablet + ' 780w' : '' }${
