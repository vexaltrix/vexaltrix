...
const CounterIcon = ( props ) => {
	...
	if ( attributes.sourceType === 'icon' ) {
		return <span className="wp-block-vxt-counter__icon">{ renderSVG( attributes.icon, setAttributes ) }</span>;
	}
	...
	if ( attributes.sourceType === 'image' ) {
		...
		if ( urlCheck !== '' ) {
			...
			return (
				<span className="wp-block-vxt-counter__image-wrap">
					<img
						className="wp-block-vxt-counter__image"
						src={ url }
