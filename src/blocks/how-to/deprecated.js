...
const deprecated = [
	{
		...
		save( props ) {
			...
			if ( mainimage && mainimage.url ) {
				imageIconHtml = <img className="vxt-howto__source-image" src={ url } title={ title } />;
			}

			return (
				<div className={ classnames( className, `vxt-block-${ block_id }` ) }>
					<script type="application/ld+json">{ schema }</script>
					<div className="vxt-how-to-main-wrap">
						<div className="vxt-howto__wrap">
							<RichText.Content
								value={ headingTitle }
								tagName={ headingTag }
							...
							{ mainimage.url && <div className="vxt-howto__source-wrap">{ imageIconHtml }</div> }
							{ showTotaltime && (
								<span className="vxt-howto__time-wrap">
									...
									<RichText.Content
										value={ time }
										tagName="p"
										className="vxt-howto-timeNeeded-value"
									/>
									<RichText.Content
										tagName="p"
										value={ timeIn }
										className="vxt-howto-timeINmin-text"
									/>
								</span>
							) }
							{ showEstcost && (
						...
						{ showTools && (
							<div className="vxt-how-to-tools__wrap">
								<RichText.Content
									value={ toolsTitle }
									tagName="h4"
						...
						{ showTools && (
							<div className="vxt-tools__wrap">
								{ tools.map( ( tools, index ) => {
									return (
										<div
											className={ classnames(
												`vxt-how-to-tools-${ index }`,
												'vxt-how-to-tools-child__wrapper'
											) }
											key={ index }
										>
											<div className="vxt-tools">
												<RichText.Content
													tagName="div"
													value={ tools.add_required_tools }
													className="vxt-tools__label"
												/>
											</div>
										</div>
									);
						...
						{ showMaterials && (
							<div className="vxt-how-to-materials__wrap">
								<RichText.Content
									value={ materialTitle }
									tagName="h4"
						...
						{ showMaterials && (
							<div className="vxt-how-to-materials">
								{ materials.map( ( materials, index ) => {
									return (
										<div
											className={ classnames(
												`vxt-how-to-materials-${ index }`,
												'vxt-how-to-materials-child__wrapper'
											) }
										>
											<div className="vxt-materials">
												<RichText.Content
													tagName="div"
													value={ materials.add_required_materials }
													className="vxt-materials__label"
												/>
											</div>
										</div>
									);
							...
							{ showTotaltime && (
								<span className="vxt-howto__time-wrap">
									...
									<>
										{ timeInYears && (
											<>
												<p className="vxt-howto-timeNeeded-value"> { timeInYears }</p>
												<p className="vxt-howto-timeINmin-text"> { yearlabel }</p>
											</>
										) }
										{ timeInMonths && (
											<>
												<p className="vxt-howto-timeNeeded-value">{ timeInMonths }</p>
												<p className="vxt-howto-timeINmin-text">{ monthlabel }</p>
											</>
										) }
										{ timeInDays && (
											<>
												<p className="vxt-howto-timeNeeded-value">{ timeInDays }</p>
												<p className="vxt-howto-timeINmin-text">{ daylabel }</p>
											</>
										) }
										{ timeInHours && (
											<>
												<p className="vxt-howto-timeNeeded-value">{ timeInHours }</p>
	...
	},
	{
		...
		save( props ) {
			...
			if ( mainimage && mainimage.url ) {
				imageIconHtml = <img className="vxt-howto__source-image" src={ url } title={ title } alt="" />;
			}
			...
			return (
				<div className={ classnames( className, `vxt-block-${ block_id }`, 'vxt-how-to-main-wrap' ) }>
					...
					{ showTools && (
						<RichText.Content value={ toolsTitle } tagName="h4" className="vxt-howto-req-tools-text" />
					) }
					{ showTools && (
						<>
