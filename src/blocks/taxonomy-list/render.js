...
const Render = ( props ) => {
	...
	const { records: categories } = useEntityRecords( 'taxonomy', taxonomyType, query );
	...
	const renderCategoryName = ( name ) => ( ! name ? __( '(Untitled)', 'vexaltrix' ) : decodeEntities( name ).trim() );

	const renderCategoryList = () => {
		const parentId = showhierarchy ? 0 : null;
		const categoryList = getCategoriesList( parentId );
		return categoryList.map( ( category ) => renderCategoryListItem( category ) );
	};
	...
	const renderCategoryListItem = ( category ) => {
		...
		if ( ( 0 === category.parent && ! showhierarchy ) || 'post_tag' === taxonomyType ) {
			return (
				<li key={ id } className={ `vxt-tax-list` }>
					<Tag class="vxt-tax-link-wrap">
						<a className="vxt-tax-link" href={ link } target="_blank" rel="noreferrer noopener">
							{ renderCategoryName( name ) }
						</a>
						{ showCount && ` (${ count })` }
					</Tag>
					{ 'none' !== seperatorStyle && <div className="vxt-tax-separator"></div> }
				</li>
			);
			...
			return (
				<li key={ id } className={ `vxt-tax-list` }>
					...
					{ showhierarchy && !! childCategories.length && (
						<ul className="vxt-taxonomy-list-children">
	...
	return categoriesList.length ? (
		...
		>
			{ 'grid' === layout &&
				categoriesList.map( ( p, index ) => (
					<div className="vxt-taxomony-box" key={ index }>
						<a className="vxt-tax-link" href={ p.link }>
							<Tag
								className="vxt-tax-title"
			...
			{ 'list' === layout && 'list' === listDisplayStyle && (
				<ul className="vxt-list-wrap">{ renderCategoryList() }</ul>
			) }

			{ 'list' === layout && 'dropdown' === listDisplayStyle && (
				<select className="vxt-list-dropdown-wrap">
					{ categoriesList.map( ( p, index ) => (
		...
		>
			<div className="vxt-tax-not-available">{ noTaxDisplaytext }</div>
		</div>
	);
};
