...
import { vexaltrixProFeatures } from './VexaltrixProFeatures';
...
const UpgradeComponent = ( props ) => {
	...
	return (
		<>
			<div className="vxt-upgrade-pro-wrap">
				<VexaltrixSupport campaign={ campaign } />
			</div>
		</>
	);
};
...
const VexaltrixSupport = ( { campaign } ) => {
	const postCampaigns = {
		'post-grid': 'post-grid',
	...
	const modalData = vexaltrixProFeatures[ postCampaigns[ campaign ] || campaign ];
	...
	const allPlansData = {
		'Vexaltrix Pro': supportModalData,
		'Essential Toolkit': {
			...supportModalData,
			features: [
	...
	return (
		...
		>
			<div
				className="vexaltrix-support-container"
				style={ {
					padding: '8px',
			...
			>
				<div className="vexaltrix-support-icon" style={ { height: '100%' } }>
				...
				<div
					className="vexaltrix-support-text"
...
const ProModal = ( { modalData, setIsModalOpen } ) => {
	...
	const contryCode = vxt_ultimate_gutenberg_blocks_blocks_info.contry_code;
			...
			try {
				...
				const filteredData = Object.entries( data.data ).reduce( ( acc, [ key, value ] ) => {
					if (
						...
						( value.product.includes( 'Essential Toolkit for Vexaltrix - Annual Subscription' ) &&
							value.variant.includes( '1 Site' ) ) ||
	...
	const handleProductSelect = ( option ) => {
		if ( productsList && Object.keys( productsList ).length > 0 ) {
			for ( const key in productsList ) {
				if ( productsList[ key ]?.product.split( ' - ' )[ 0 ]?.replace( ' for Vexaltrix', '' ) === option ) {
					setProductKey( key );
					break;
				}
			}
	...
	const utmParams = '&utm_medium=vexaltrix-editor&utm_campaign=upsell-popup-buy-now';
		...
		>
			...
			{ loading ? (
				...
				<>
					...
					<div className={ `${ getGridClass() } w-full gap-1 mt-4` }>
						{ features?.map( ( ele, idx ) => (
							...
							>
								...
								<p className="text-field-label m-0" style={ { fontSize: '14px', lineHeight: '20px' } }>
									{ ele }
								</p>
							</div>
						) ) }
					...
					<div className="flex justify-between items-center mt-4">
								...
								options={
									productsList
										? Object.values( productsList )?.map( ( item ) =>
												item?.product.split( ' - ' )[ 0 ]?.replace( ' for Vexaltrix', '' )
										  )
										: []
								}
								onSelect={ ( option ) => {
						...
						<div className="flex items-center justify-between gap-2">
							...
							{  }
							<a
								href={ `${ VXT_LINKS.VXT_URL }/pricing/?utm_medium=vexaltrix-editor&utm_campaign=upsell-popup-buy-now` }
								target="_blank"
								rel="noreferrer"
								className="no-underline text-text-on-color"
							>
					...
					<div className="flex justify-end md:pr-[10px] pr-2 w-full">
						<a
							href={ vxt_ultimate_gutenberg_blocks_blocks_info?.upsellModalEditor }
							target="_blank"
							rel="noreferrer"
							className="text-xxs text-brand-primary-600 vxt-remove-ring"
						>
							{ __( 'View plans', 'vexaltrix' ) }
						</a>
					</div>
				</>
			) }
...
const CustomDropdown = ( { options, onSelect } ) => {
	...
	if ( loading ) {
		return <div className="w-full flex justify-center items-center text-lg">Loading...</div>;
	}
