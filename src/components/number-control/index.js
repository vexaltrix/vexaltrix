...
const UAGNumberControl = ( props ) => {
		...
		uSizes.map( ( key ) =>
			items.push(
				...
				>
					<Button
						key={ key.unitValue }
						className={ 'vxt-number-control__units--' + key.name }
						isSmall
		...
		return (
			<div className="vxt-control__header">
				<div className="vxt-number-control__actions vxt-control__actions">
					...
					/>
					{ props.displayUnit && (
						<ButtonGroup className="vxt-control__units" aria-label={ __( 'Select Units', 'vexaltrix' ) }>
	...
	return (
		<div
			ref={ panelRef }
			className={ `components-base-control uag-number-control vxt-size-type-field-tabs${
				isEnableDynamicContent() ? ' vxt-text-control--open-dynamic-content' : ''
			}` }
		>
			{ controlBeforeDomElement }
			{ props.showControlHeader && <ControlHeader /> }
			<div className={ classnames( 'vxt-number-control__mobile-controls', 'uag-number-control__' + variant ) }>
