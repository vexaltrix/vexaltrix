...
import { cloneElement, Children, useState, useRef, useEffect, useLayoutEffect } from '@wordpress/element';
...
const InspectorTabs = ( props ) => {
	...
	const renderUAGTabsSettingsInOrder = () => {
		...
		const tabsContainer = document.querySelector( '.vxt-inspector-tabs-container' );
		let dynamicContentContainer = document.querySelector( '.components-panel__body.vxt-dynamic-content-wrap' );
		let tabsGeneralContainer = document.querySelector( '.vxt-tab-content-general' );
		let tabsStyleContainer = document.querySelector( '.vxt-tab-content-style' );
		let tabsAdvanceContainer = document.querySelector( '.vxt-tab-content-advance' );
	...
	useEffect( () => {
		...
		if (
			...
			if ( sidebarPanel ) {
				sidebarPanel.setAttribute( 'data-vxt-tab', uagSettingState[ blockName ]?.selectedTab || 'general' );
			}
		} else if ( sidebarPanel ) {
			sidebarPanel.setAttribute( 'data-vxt-tab', 'general' );
		}
		// Above Section Ends.
		// component will unmount
		return () => {
			if ( sidebarPanel ) {
				const inspectorTabs = sidebarPanel.querySelector( '.vxt-inspector-tabs-container' );

				if ( ! inspectorTabs || null === inspectorTabs ) {
					sidebarPanel.removeAttribute( 'data-vxt-tab' );
				}
			}
		};
	}, [] );
	...
	return (
		<>
			<div className={ 'vxt-inspector-tabs-container' }>
				{ /*
				...
				<div
					...
					className={ classnames(
						'vxt-inspector-tabs',
						'vxt-inspector-tabs-count-' + tabs.length,
						currentTab
					) }
				>
					{ tabs.indexOf( LAYOUT ) > -1 && (
						<div
							className={ classnames( {
								'vxt-active': currentTab === LAYOUT,
							} ) }
							onClick={ () => _onTabChange( LAYOUT ) }
						>
							<svg xmlns="https://www.w3.org/2000/svg" width="16" height="15">
								<path
									fillRule="nonzero"
					...
					{ tabs.indexOf( STYLE ) > -1 && (
						<div
							className={ classnames( {
								'vxt-active': currentTab === STYLE,
							} ) }
							onClick={ () => _onTabChange( STYLE ) }
						>
							<svg xmlns="https://www.w3.org/2000/svg" width="18" height="21">
								<g fillRule="nonzero">
					...
					{ tabs.indexOf( ADVANCE ) > -1 && (
						<div
							className={ classnames( {
								'vxt-active': currentTab === ADVANCE,
							} ) }
							onClick={ () => _onTabChange( ADVANCE ) }
						>
							<svg xmlns="https://www.w3.org/2000/svg" width="17" height="16">
								<path
									fillRule="nonzero"
