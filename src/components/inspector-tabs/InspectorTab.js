...
const InspectorTab = ( props ) => {
	...
	const inspectorTabBefore = applyFilters( `vexaltrix.${ blockNameForHook }.tab_${ type }.before`, '', blockName );
	const inspectorTabAfter = applyFilters( `vexaltrix.${ blockNameForHook }.tab_${ type }`, '', blockName );
	...
	if ( 'vexaltrix/container' === blockName && 'general' === type ) {
	...
	return (
		<div
			...
			className={ `vxt-inspector-tab vxt-tab-content-${ type }` }
			ref={ tabRef }
		>
			{ inspectorTabBefore }
			{ GBSNotice }
