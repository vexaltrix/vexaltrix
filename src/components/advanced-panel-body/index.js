...
const UAGAdvancedPanelBody = ( props ) => {
	...
	const getInspectorTabName = () => {
		...
		if ( panelRef?.current?.parentElement?.classList.contains( 'vxt-tab-content-general' ) ) {
			inspectorTabName = 'general';
		}
		if ( panelRef?.current?.parentElement?.classList.contains( 'vxt-tab-content-advance' ) ) {
	...
	const onPanelToggle = () => {
		if ( 'enabled' === vxt_ultimate_gutenberg_blocks_blocks_info.collapse_panels ) {
	...
	const tabBodyBefore = applyFilters(
		`vexaltrix.${ blockNameForHook }.${ panelNameForHook }.${ panelTitle }.before`,
		'',
		blockName
	);
	const tabBodyAfter = applyFilters(
		`vexaltrix.${ blockNameForHook }.${ panelNameForHook }.${ panelTitle }`,
		'',
		blockName
	);
