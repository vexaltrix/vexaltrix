...
const TypographyControl = ( props ) => {
	...
	useLayoutEffect( () => {
		window.addEventListener( 'click', function ( e ) {
			const popupButton = document.querySelector(
				`.active.popup-${ props?.attributes?.block_id } .vexaltrix-control-popup__options--action-button`
			);
			const popupWrap = document.querySelector(
				`.active.popup-${ props?.attributes?.block_id } .vexaltrix-control-popup`
			);
	...
	if ( ! disableFontSize ) {
		...
		if ( props.fontSizeTypeMobile && props.fontSizeTypeTablet ) {
			attributeNames.push( props.fontSizeTypeMobile.label, props.fontSizeTypeTablet.label );
		}
	}
	...
	if ( true !== disableLineHeight ) {
		lineHeight = (
			<RangeTypographyControl
				...
				min={ 0 }
				sizeMobileText={ __( 'Line Height', 'vexaltrix' ) }
				sizeTabletText={ __( 'Line Height', 'vexaltrix' ) }
				sizeText={ __( 'Line Height', 'vexaltrix' ) }
	...
	if ( props.letterSpacing ) {
		letterSpacing = (
			<RangeTypographyControl
				...
				min={ -50 }
				sizeMobileText={ __( 'Letter Spacing', 'vexaltrix' ) }
				sizeTabletText={ __( 'Letter Spacing', 'vexaltrix' ) }
				sizeText={ __( 'Letter Spacing', 'vexaltrix' ) }
		...
		transform = (
			<UAGSelectControl
				...
				options={ [
					...
					{
						value: 'capitalize',
						label: __( 'Capitalize', 'vexaltrix' ),
					},
					{
						value: 'uppercase',
						label: __( 'Uppercase', 'vexaltrix' ),
					},
					{
						value: 'lowercase',
						label: __( 'Lowercase', 'vexaltrix' ),
					},
				] }
			/>
		);
			...
			<div className="uag-typography-decoration">
				<UAGSelectControl
					...
					options={ [
						...
						{
							value: 'overline',
							label: __( 'Overline', 'vexaltrix' ),
						},
						{
							value: 'line-through',
							label: __( 'Line Through', 'vexaltrix' ),
						},
					] }
				/>
			</div>
	...
	if ( true !== disableFontFamily && true !== disableFontSize ) {
		fontAdvancedControls = (
			<Button
				className="uag-typography-button vexaltrix-control-popup__options--action-button"
	...
	if ( showAdvancedControls === true ) {
		showAdvancedFontControls = (
			<div className="vxt-typography-advanced vexaltrix-control-popup">
				{ fontFamily }
				{ transform }
				{ decoration }
				{ lineHeight }
		...
		fontTypoAdvancedControls = (
			<div className="vexaltrix-control-popup__options--action-wrapper">
				<span className="uag-control-label">
					{ props.label }
					{ isTypographyUpdated && <div className="vexaltrix__change-indicator--dot-right" /> }
				</span>
				{ fontAdvancedControls }
			</div>
		);
	...
	return (
		<div ref={ panelRef } className="components-base-control">
			...
			<div
				className={ ` uag-typography-options vexaltrix-control-popup__options popup-${ props?.attributes?.block_id } ${ activeClass }` }
			>
