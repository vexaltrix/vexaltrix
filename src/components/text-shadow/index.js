...
const TextShadowControl = ( props ) => {
	...
	if ( showAdvancedControls ) {
		advancedControls = <div className="vxt-text-shadow-advanced vexaltrix-control-popup">{ overallControls }</div>;
	}
	...
	const textShadowAdvancedControls = (
		<div className="vexaltrix-control-popup__options--action-wrapper">
			<span className="uag-control-label">
				{ label }
				{ isTextShadowUpdated && <div className="vexaltrix__change-indicator--dot-right" /> }
			</span>
			<Button
				className="uag-text-shadow-button vexaltrix-control-popup__options--action-button"
	...
	return (
		<div ref={ panelRef } className={ popup ? 'components-base-control' : '' }>
			...
			{ popup ? (
				<div
					className={ ` uag-text-shadow-options vexaltrix-control-popup__options popup-${ blockId } ${ activeClass }` }
				>
