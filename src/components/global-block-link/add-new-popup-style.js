...
const AddNewPopupStyle = ( props ) => {
	...
	const onClickSaveButton = () => {
		...
		if ( ! tempStyleNameTrimmed || '' === tempStyleNameTrimmed ) {
			setLabelError( __( 'Please enter a style name', 'vexaltrix' ) );
			return;
		}
		...
		if ( tempStyleNameTrimmed.length < 3 || tempStyleNameTrimmed.length > 50 ) {
			setLabelError(
				__( 'Style name should be more then 3 characters and less then 50 characters', 'vexaltrix' )
			);
			return;
		}
		...
		if (
			globalBlockStyles.find( ( style ) => style.label === tempStyleNameTrimmed && style.blockName === blockName )
		) {
			setLabelError( __( 'Style name already exists', 'vexaltrix' ) );
			return;
		}
		...
		if ( hasSpecialCharacter.test( tempStyleNameTrimmed ) ) {
			setLabelError( __( 'Style name should not contain special characters', 'vexaltrix' ) );
			return;
		}
		...
		closeModal();
		setTempStyleName( '' );
		updateGlobalBlockStyles( vexaltrixGlobalStyles );
		setLabelNull();
	};
		...
		<>
			...
			{ 'open' === isOpen && (
				<Modal
					...
					className="vexaltrix-global-block-style-name-modal"
				>
					<div className="gbs-add-new-style-head">
						<div>
							<h3>{ __( 'Save as Global Block Style', 'vexaltrix' ) }</h3>
							<p> { __( 'Use the saved style anywhere on the website', 'vexaltrix' ) }</p>
						</div>
						<svg
					...
					<div className={ `input-wrap ${ labelError ? 'gbs-error' : '' }` }>
						<UAGTextControl
					...
					<div className="button-wrap">
						...
						<Button onClick={ onClickSaveButton } variant="primary">
							{ __( 'Save', 'vexaltrix' ) }
						</Button>
					</div>
				</Modal>
