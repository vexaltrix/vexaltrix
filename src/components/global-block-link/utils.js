...
export function getLabel( globalBlockStyleId ) {
	return ! globalBlockStyleId ? __( 'Link to Existing Style', 'vexaltrix' ) : __( 'Linked Style', 'vexaltrix' );
}
...
export function getNewAttributes( style, attributes, currentBlockDefaultAttributes ) {
	...
	for ( const attrKey in attributes ) {
		...
		if (
			currentBlockDefaultAttributes[ attrKey ]?.isGBSStyle &&
			attrValue &&
			'0.001020304' !== attrValue &&
			{} !== attrValue &&
			[] !== attrValue &&
			! isEmptyObject( attrValue )
		) {
...
export const currentGlobalBlockStyleIdAttributes = ( globalBlockStyles, globalBlockStyleId ) => {
	...
	// Block name removed vexaltrix/
	const blockName = getGbsValue?.blockName?.replace( 'vexaltrix/', '' );
