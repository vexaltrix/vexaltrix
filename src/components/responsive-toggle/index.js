...
const ResponsiveToggle = ( props ) => {
	...
	const devices = [
		{
			name: __( 'Desktop', 'vexaltrix' ),
			staticName: 'Desktop',
			title: devicesSvgs.desktop,
			itemClass: 'vxt-desktop-tab vxt-responsive-tabs',
		},
		{
			name: __( 'Tablet', 'vexaltrix' ),
			staticName: 'Tablet',
			title: devicesSvgs.tablet,
			itemClass: 'vxt-tablet-tab vxt-responsive-tabs',
		},
		{
			name: __( 'Mobile', 'vexaltrix' ),
			staticName: 'Mobile',
			key: 'mobile',
			title: devicesSvgs.mobile,
			itemClass: 'vxt-mobile-tab vxt-responsive-tabs',
		},
	];
	...
	const commonResponsiveHandler = ( e ) => {
		...
		const inspectorTab = eventTriggerElement.closest( '.vxt-inspector-tab' );
		...
		if ( inspectorTab.classList.contains( 'vxt-tab-content-general' ) ) {
			inspectorTabName = 'general';
		}
		if ( inspectorTab.classList.contains( 'vxt-tab-content-advance' ) ) {
	...
	return (
		<div ref={ panelRef } className="uag-responsive-label-wrap">
			...
			{ displayResponsive && responsive && (
				<ButtonGroup
					className="vxt-range-control-responsive components-tab-panel__tabs"
					aria-label={ __( 'Device', 'vexaltrix' ) }
				>
					{ devices.map( ( { name, staticName, key, title, itemClass } ) => (
						<Tooltip
							text={ sprintf(
