 ...
 *
 */
import cIcons from './vxt-color-icons';
import maybeGetColorForVariable from '@Controls/maybeGetColorForVariable';
import { __ } from '@wordpress/i18n';
import { Button, Popover, ColorIndicator, Tooltip, Dashicon, ColorPicker, ColorPalette } from '@wordpress/components';
...
const AdvancedPopColorControl = ( props ) => {
	...
	const [ value, setValue ] = useState( {
		...
		isPalette: colorValue && colorValue.startsWith( 'palette' ) ? true : false,
		refresh: false,
	} );
	...
	const pickIconColorBasedOnBgColorAdvanced = ( color ) => {
		...
		const parsedColor = result
			...
			: null;
		if ( parsedColor ) {
			const brightness = Math.round( ( parsedColor.r * 299 + parsedColor.g * 587 + parsedColor.b * 114 ) / 1000 );
	...
	const globalIndicator =
		colorVal && colorVal.includes( 'var' ) ? `uag-global-indicator uag-global-icon-${ globalIconColor }` : '';
	...
	return (
		<div ref={ panelRef } className="components-base-control">
			...
			<div className="vxt-color-popover-container new-vxt-advanced-colors">
				<div className="vxt-advanced-color-settings-container">
					{ label && <span className="vxt-beside-color-label uag-control-label">{ label }</span> }
					<UAGReset
						onReset={ resetValues }
						attributeNames={ [ data?.label ] }
						setAttributes={ setAttributes }
					/>
					<div className="vxt-beside-color-click">
						{ visible.isVisible && (
							<Popover
								position="top left"
								className="vxt-popover-color new-vxt-advanced-colors-pop"
								onClose={ toggleClose }
							>
								{ value.refresh && (
									<>
								...
								<button
									...
									className="vxt-clear-btn-inside-picker components-button components-circular-option-picker__clear is-secondary is-small"
								>
									{ __( 'Clear', 'vexaltrix' ) }
								</button>
							</Popover>
						...
						{ visible.isVisible && (
							<Tooltip text={ __( 'Select Color', 'vexaltrix' ) }>
								<Button className={ `vxt-color-icon-indicate vxt-has-alpha` } onClick={ toggleClose }>
									<ColorIndicator
										className={ `vxt-advanced-color-indicate ${ globalIndicator }` }
										colorValue={ colorVal }
									/>
						...
						{ ! visible.isVisible && (
							<Tooltip text={ __( 'Select Color', 'vexaltrix' ) }>
								<Button className={ `vxt-color-icon-indicate vxt-has-alpha` } onClick={ toggleVisible }>
									<ColorIndicator
										className={ `vxt-advanced-color-indicate ${ globalIndicator }` }
										colorValue={ colorVal }
									/>
