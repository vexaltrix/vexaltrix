...
const ModalContainer = ( props ) => {
	...
	const renderIconList = () => {
		if ( ! iconList.length ) {
			return (
				<div className="vxt-ip-icons icon-not-found">
					<div className="vxt-icon-not-available">
						<span>{ __( 'No Icons Found', 'vexaltrix' ) }</span>
					</div>
				</div>
			);
		}
	...
	return (
		<Modal
			className="vxt-ip-modal-wrapper"
			onRequestClose={ closeModal }
			overlayClassName="vxt-ip-modal-wrapper-overlay"
			shouldCloseOnClickOutside={ false }
			closeButtonLabel={ __( 'Close', 'vexaltrix' ) }
		>
			{ /* Header  */ }
			<HeaderContainer
			...
			/>
			{ /* middle  */ }
			<section className="vxt-ip-lr-container">
				<div className="vxt-ip-left">{ listOfCategory() }</div>
				<div className="vxt-ip-right">
					<div className="vxt-ip-modal-container" ref={ iconContainerRef }>
						{ renderIconList() }
					</div>
				</div>
			</section>
			{ /* Footer */ }
			<section className="vxt-ip-footer">
				<button
