...
const UAGIconPicker = ( props ) => {
	...
	const defaultIcons = [ ...wp.VexaltrixSvgIcons ];
	const iconCategoryList = [ ...wp.vxt_ultimate_gutenberg_blocks_icon_category_list ];
	// For modal.
		...
		<div className={ `uag-ip-placeholder-wrap` }>
			...
			<div className="uag-ip-actions">
				<span onClick={ openModal }>
					{ isIconAvailable ? __( 'Change Icon', 'vexaltrix' ) : __( 'Choose Icon', 'vexaltrix' ) }
				</span>
			</div>
		</div>
	);
	return (
		<div className="uag-custom-ip components-base-control">
			<span className="uag-control-label">{ label || __( 'Icon', 'vexaltrix' ) }</span>
			{ modalPlaceHolder }
			{ isOpen && (
				<ModalContainer
					{ ...{
