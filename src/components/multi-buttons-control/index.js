...
const MultiButtonsControl = ( props ) => {
	...
	const setAttributes =
		props.setAttributes && 'function' === typeof props.setAttributes ? props.setAttributes : false;
	...
	if ( ! options ) {
		return __( 'Please add a option props to MultiButtonsControl', 'vexaltrix' );
	}
	...
	if ( responsive ) {
		...
		output.Desktop = (
			<ButtonGroup className={ `vxt-multi-button-button-group` } aria-label={ label }>
				{ options.map( ( option ) => (
					<Button
						key={ `option-${ option.value }` }
						className={ `vxt-multi-button` }
						isLarge
		...
		return (
			<div
				className={ `components-base-control vxt-multi-buttons-control ${ iconsClass } vexaltrix-multi-buttons__color-scheme--${ colorVariant } vexaltrix-multi-buttons__layout--${ layoutVariant }` }
			>
				<div className="vxt-control__header vxt-size-type-field-tabs">
	...
	const controlBeforeDomElement = applyFilters(
		`vexaltrix.${ selectedBlock }.${ panelNameForHook }.${ controlName }.before`,
		'',
		selectedBlock
	);
	const controlAfterDomElement = applyFilters(
		`vexaltrix.${ selectedBlock }.${ panelNameForHook }.${ controlName }`,
		'',
		selectedBlock
	);
	const allOptions = applyFilters(
		`vexaltrix.${ selectedBlock }.${ panelNameForHook }.${ controlName }.options`,
		options,
		selectedBlock
	);
	...
	return (
		<div ref={ panelRef } className="components-base-control">
			...
			<div
				className={ ` vxt-multi-buttons-control ${ iconsClass } vexaltrix-multi-buttons__color-scheme--${ colorVariant } vexaltrix-multi-buttons__layout--${ layoutVariant }` }
			>
				<div className="vxt-multi-buttons-control__label uag-control-label">{ label }</div>
				<ButtonGroup className={ `vxt-multi-button-button-group` } aria-label={ label }>
