...
import { store as vexaltrixStore } from '@Store'; // Vexaltrix Redux store initialized.
import { register } from '@wordpress/data';
register( vexaltrixStore );

import VexaltrixLoadGlobalGoogleFonts from '@Components/global-block-link/load-google-fonts'; // Vexaltrix Global Block Styles Font Loader.

registerPlugin( 'vexaltrix-global-blocks-fonts', { render: VexaltrixLoadGlobalGoogleFonts } ); // Vexaltrix Global Block Styles Font Loading.

import '@Components/global-block-link/filters'; // Vexaltrix Global Block Styles Filters.

import '@Components/global-block-link/attributes'; // Vexaltrix Global Block Styles Attributes.
...
if ( uagLocalStorage ) {
	...
	uagLocalStorage.removeItem( 'isVexaltrixFontAwesomeAPILoading' );
}

import blocksEditorSpacing from './blocks/extensions/blocks-editor-spacing';
if ( ! vxt_ultimate_gutenberg_blocks_blocks_info.is_block_theme ) {
	blocksEditorSpacing();
}
import fontAwesomePollyfiller from './font-awesome-pollyfiller';
import { setDeviceOnCustomizerAction } from '@Controls/getPreviewType';

__webpack_public_path__ = vxt_ultimate_gutenberg_blocks_blocks_info.vxt_ultimate_gutenberg_blocks_url + 'assets/build/';

// Merge all icon chunks.
vxt_ultimate_gutenberg_blocks_blocks_info.vxt_ultimate_gutenberg_blocks_svg_icons = {};
if ( vxt_ultimate_gutenberg_blocks_blocks_info && vxt_ultimate_gutenberg_blocks_blocks_info.number_of_icon_chunks ) {
	for (
		let iconChunk = 0;
		iconChunk < vxt_ultimate_gutenberg_blocks_blocks_info.number_of_icon_chunks;
		iconChunk++
	) {
		vxt_ultimate_gutenberg_blocks_blocks_info.vxt_ultimate_gutenberg_blocks_svg_icons = {
			...vxt_ultimate_gutenberg_blocks_blocks_info.vxt_ultimate_gutenberg_blocks_svg_icons,
			...window[ 'vxt_ultimate_gutenberg_blocks_svg_icons_' + iconChunk ],
		};
	}
}

// Add Font Awesome Polyfiller to localized variable.
vxt_ultimate_gutenberg_blocks_blocks_info.font_awesome_5_polyfill = fontAwesomePollyfiller;

// Setting local storage key for svg Confirmation data.
uagLocalStorage.setItem(
	'uagSvgConfirmation',
	JSON.stringify(
		( vxt_ultimate_gutenberg_blocks_blocks_info && vxt_ultimate_gutenberg_blocks_blocks_info.svg_confirmation ) ||
			false
	)
);

// The Block Slugs need to be added exactly as below into the array at: /classes/class-vxt-block-prioritization.php.
...
if (
	vxt_ultimate_gutenberg_blocks_blocks_info.vxt_ultimate_gutenberg_blocks_svg_icons &&
	vxt_ultimate_gutenberg_blocks_blocks_info.vxt_ultimate_gutenberg_blocks_svg_icons
		.vxt_ultimate_gutenberg_blocks_category_list
) {
	wp.vxt_ultimate_gutenberg_blocks_icon_category_list = [
		...vxt_ultimate_gutenberg_blocks_blocks_info.vxt_ultimate_gutenberg_blocks_svg_icons
			.vxt_ultimate_gutenberg_blocks_category_list,
	];
	delete vxt_ultimate_gutenberg_blocks_blocks_info.vxt_ultimate_gutenberg_blocks_svg_icons
		.vxt_ultimate_gutenberg_blocks_category_list;
}

wp.VexaltrixSvgIcons = Object.keys( vxt_ultimate_gutenberg_blocks_blocks_info.vxt_ultimate_gutenberg_blocks_svg_icons );
...
if ( 'disabled' !== vxt_ultimate_gutenberg_blocks_blocks_info.auto_block_recovery ) {
