    ...
    const updateZipAiStatus = ( moduleName, value ) => {
		...
		getApiDataFetch.then( ( response ) => {
			if ( response?.success ) {
				...
				dispatch( { type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION', payload: response?.data?.messsage || __( 'Successfully saved!' , 'vexaltrix' ) } );
			} else {
				// Update the failed notification.
				dispatch( { type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION', payload: { message: response?.data?.messsage || __( 'Failed to save setting', 'vexaltrix' ), messageType: 'error' } } );
				...
				<div className='flex py-5 justify-between items-center border-b border-b-slate-200'>
					...
					{ undefined === zipAiAssistantStatus ? (
						<p className='text-slate-400 cursor-default'>
							{ __( 'Coming Soon', 'vexaltrix' ) }
						</p>
					) : (
						<button
							className='text-vexaltrix hover:text-vexaltrix-hover focus-visible:text-vexaltrix-hover focus-visible:outline-none'
							onClick={ () => updateZipAiStatus( 'ai_assistant', zipAiAssistantStatus ) }
							disabled={ isUpdating?.atLeastOneUpdate }
						>
							{ isUpdating?.ai_assistant ? __( 'Updating', 'vexaltrix' ) : (
								<>
									{ zipAiAssistantStatus ? __( 'Disable', 'vexaltrix' ) : __( 'Enable', 'vexaltrix' ) }
								</>
							) }
						</button>
					) }
				...
				<div className='flex py-5 justify-between items-center border-b border-b-slate-200'>
					...
					{ undefined === zipAiDesignCopilotStatus ? (
						...
						<button
							...
							disabled={ isUpdating?.atLeastOneUpdate }
						>
							{ isUpdating?.ai_design_copilot ? __( 'Updating', 'vexaltrix' ) : (
								<>
									{ zipAiDesignCopilotStatus ? __( 'Disable', 'vexaltrix' ) : __( 'Enable', 'vexaltrix' ) }
								</>
							) }
						</button>
					) }
						...
						<div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:p-6">
							<div className="mb-4 text-center sm:mt-0 sm:text-left">
								<Dialog.Title as="h3" className="text-xl font-semibold text-slate-800">
									{ __( 'Manage AI Features', 'vexaltrix' ) }
								</Dialog.Title>
								<p className="mt-2 text-sm text-slate-500">
									{ __( 'Here you can enable/disable AI features from your website.', 'vexaltrix' ) }
								</p>
							</div>
							{ renderAiFeatures() }
							<div className="mt-6 sm:flex sm:flex-row">
								<button
									type="button"
									className="mt-3 w-full inline-flex justify-center rounded shadow-sm px-4 py-2 bg-vexaltrix text-base font-medium text-white transition-colors focus:bg-vexaltrix-hover hover:bg-vexaltrix-hover focus:outline-none sm:mt-0 sm:text-sm"
									onClick={ () => setShowPopup( false ) }
									ref={ cancelButtonRef }
								>
									{ __( 'Go Back', 'vexaltrix' ) }
								</button>
							</div>
						</div>
					</Transition.Child>
