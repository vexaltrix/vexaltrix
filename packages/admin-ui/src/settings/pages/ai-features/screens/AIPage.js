...
import {
	...
	CodeBlockIcon,
	PageIcon,
} from '@Settings/pages/ai-features/elements/IconComponents';
...
const AIPage = () => {
	...
	const buttonText =
		'disconnected' === vexaltrixAdmin?.zip_ai_status
			? __( 'Reconnect and Continue Using AI Features', 'vexaltrix' )
			: __( 'Get Started with 1000 Free Monthly Credits', 'vexaltrix' );
	...
	const authorizeZipAI = ( event ) => {
		...
		// window.location.assign( vexaltrixAdmin.zip_ai_auth_middleware );

		// Get all the auth buttons and disable them.
		const authButtons = document.querySelectorAll( '.vexaltrix-ai-features-authorization' );
		...
		const authWindow = window.open(
			vexaltrixAdmin.zip_ai_auth_middleware,
			'VexaltrixAiFeaturesAuthorization',
			`width=480,height=720,top=${ positioning.top },left=${ positioning.left },scrollbars=0`
		);

		// Create an object with the security property.
		const data = {
			security: vexaltrixAdmin.zip_ai_verify_authenticity_nonce,
		};
		...
		setButtonLabel(
			'disconnected' === vexaltrixAdmin?.zip_ai_status
				? __( 'Reconnecting to Use AI Features', 'vexaltrix' )
				: __( 'Getting Started with 1000 Free Monthly Credits', 'vexaltrix' )
		);
	...
	const updateZipAiStatus = ( moduleName, value ) => {
		...
		const data = {
			security: vexaltrixAdmin.zip_ai_module_status_nonce,
			module: moduleName,
			value: updatedValue,
		};
		...
		getApiDataFetch.then( ( response ) => {
			if ( response?.success ) {
				...
				dispatch( {
					...
					payload: response.data.messsage || __( 'Successfully saved!', 'vexaltrix' ),
				} );
			} else {
				...
				dispatch( {
					...
					payload: {
						message:
							response.data.messsage || __( 'Failed to save setting', 'vexaltrix' ),
						messageType: 'error',
					},
				} );
	...
	const renderDetailsSection = () => (
		<section className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
			{ renderInfoCard( {
				icon: <WandIcon />,
				title: __( 'Craft Compelling Content', 'vexaltrix' ),
				description: __(
			...
			{ renderInfoCard( {
				icon: <BlocksIcon />,
				title: __( 'AI-Powered Block Patterns', 'vexaltrix' ),
				description: __(
			...
			{ renderInfoCard( {
				icon: <WordPressLogo />,
				title: __( 'WordPress Wizardry', 'vexaltrix' ),
				description: __(
			...
			{ renderInfoCard( {
				icon: <LayoutIcon />,
				title: __( 'Personalized Templates', 'vexaltrix' ),
				description: __(
			...
			{ renderInfoCard( {
				icon: <CodeBlockIcon />,
				title: __( 'Generate Custom Code', 'vexaltrix' ),
				description: __(
			...
			{ renderInfoCard( {
				icon: <PageIcon />,
				title: __( 'Custom Pages With AI', 'vexaltrix' ),
				description: sprintf(
	...
	const teamName = vexaltrixAdmin?.zip_ai_current_plan?.team_name;

	const connectionBadge = () => {
		if ( vexaltrixAdmin?.zip_ai_is_authorized )
			{return (
				<div className="flex items-center gap-3">
					<Badge
						icon={ <CheckCheck size={ 12 } /> }
						label={ __( 'Connected', 'vexaltrix' ) }
						size="sm"
						type="pill"
						variant="green"
					/>
		...
		else if ( 'disconnected' === vexaltrixAdmin?.zip_ai_status )
			{return (
				<div className="flex items-center gap-3">
					<Badge
						icon={ <Unlink size={ 12 } /> }
						label={ __( 'Disconnected', 'vexaltrix' ) }
						size="sm"
						type="pill"
						variant="red"
					/>
		...
		<DropdownMenu placement="bottom-start" className="font-[Figtree]">
			...
			<DropdownMenu.ContentWrapper>
				<DropdownMenu.Content className="w-60">
					...
					{ vexaltrixAdmin?.zip_ai_is_authorized && (
						<>
							<DropdownMenu.Item>
								<div className="w-full h-full flex justify-between font-[Figtree]">
									<div>
										<div className="text-text-secondary text-base font-medium">
											{ vxt_ultimate_gutenberg_blocks_user_data.firstName + ' ' + vxt_ultimate_gutenberg_blocks_user_data.lastName }
										</div>
									...
									<div className="h-full p-[2px]">
										<Badge
											label={
												vexaltrixAdmin.vexaltrix_pro_installed && vexaltrixAdmin.vexaltrix_pro_ver
													? __( 'Pro', 'vexaltrix' )
													: __( 'Free', 'vexaltrix' )
											}
											size="xs"
											...
											closable={ false }
											className={ `m-1 ${
												vexaltrixAdmin.vexaltrix_pro_installed &&
												vexaltrixAdmin.vexaltrix_pro_ver &&
					...
					<DropdownMenu.Item onClick={ () => openLink( `${ VXT_LINKS.ZIP_URL }/org/billing` ) }>
						<div className="flex justify-start items-center gap-3 w-full text-text-primary font-[Figtree]">
							<Layers size={ 16 } />

							<a
								href={ `${ VXT_LINKS.ZIP_URL }/org/billing` }
								target="_blank"
								rel="noreferrer"
								className="text-sm no-underline text-text-primary font-normal vxt-remove-ring"
							>
								{ __( 'Manage Plan', 'vexaltrix' ) }
							</a>
						</div>
					</DropdownMenu.Item>

					<DropdownMenu.Item onClick={ () => openLink( `${ VXT_LINKS.ZIP_URL }/pricing` ) }>
						<div className="flex justify-start items-center gap-3 w-full text-text-primary font-[Figtree]">
							<ChartNoAxesColumn size={ 16 } />

							<a
								href={ `${ VXT_LINKS.ZIP_URL }/pricing` }
								target="_blank"
								rel="noreferrer"
								className="text-sm no-underline text-text-primary font-normal vxt-remove-ring"
							>
								{ __( 'Plan Usage & Limit', 'vexaltrix' ) }
							</a>
						</div>
					</DropdownMenu.Item>
					...
					{ ! vexaltrixAdmin.zip_ai_is_authorized && (
						<DropdownMenu.Item onClick={ reconnectAI }>
							...
							>
								...
								<div className="text-sm">{ __( 'Reconnect AI', 'vexaltrix' ) }</div>
							</div>
						</DropdownMenu.Item>
					) }
					</DropdownMenu.List>
	...
	const RenderCredits = () => {
		...
		const [ creditDetails, setCreditDetails ] = useState( vexaltrixAdmin.zip_ai_credit_details );
		...
		return vexaltrixAdmin?.zip_ai_is_authorized ? (
			<div className="flex items-start gap-2 text-link-primary">
				...
				<div className="flex flex-col gap-0 items-start">
					<div className="flex items-center gap-2">
						...
						<Tooltip
							arrow
							content={ __( 'Refresh credit details', 'vexaltrix' ) }
							placement="bottom"
						...
						>
							<Button
								...
								aria-label={ __( 'Refresh Credits', 'vexaltrix' ) }
					...
					<a
						href={ `${ VXT_LINKS.ZIP_URL }/credits-pricing?source=vexaltrix` }
						target="_blank"
						rel="noreferrer"
						className="no-underline text-link-primary text-sm font-semibold"
					>
						{ __( 'Get More Credits', 'vexaltrix' ) } <ArrowUpRight size={ 12 } />
					</a>
				</div>
			</div>
		) : (
			<Button
				className={ `vxt-remove-ring ${ isMobile && 'rounded-lg' }` }
				icon={
			...
			>
				{ ! buttonLabel.toLowerCase().startsWith( 'reconnect' )
					? __( 'Get Started', 'vexaltrix' )
					: __( 'Reconnect', 'vexaltrix' ) }
			</Button>
		);
	};
		...
		<div className="bg-background-secondary">
							...
							{ isLoading ? (
								...
								>
									<div className="p-3 flex flex-col gap-2 items-center w-full">
										...
										<div>
											<div className="m-0 mb-[2px] font-semibold text-base">
												{ __( 'AI Design Copilot', 'vexaltrix' ) }
											</div>
											<div className="text-sm">
												{ __(
					...
					<div className="rounded-lg bg-white border border-solid border-border-subtle p-5 flex flex-col items-center gap-4">
						<div className="w-full">
							...
							<div className="text-text-primary text-2xl font-semibold">
								{ __( 'Build 10x Faster with Vexaltrix AI', 'vexaltrix' ) }
							</div>
						</div>
			...
			<ConfirmationPopup
				{ ...{
					...
					popupContent: {
						title: __( 'Disconnect AI', 'vexaltrix' ),
						description: `${ __(
					...
					popupAccept: {
						label: __( 'Disconnect', 'vexaltrix' ),
						callback: () => {
							localStorage.removeItem( 'zipAiAuthorizationStatus' );
							window.location.assign( vexaltrixAdmin.zip_ai_auth_revoke_url );
						},
					},
					popupCancel: {
			...
			<ConfirmationPopup
				{ ...{
					...
					popupContent: {
						title: __( 'Reconnect AI', 'vexaltrix' ),
						description: `${ __(
				...
				<div className="p-2">
					<div className="flex justify-end items-center w-full gap-3">
						...
						<Button
							...
							} text-text-on-color vxt-remove-ring ` }
							size="md"
							tag="button"
							type="button"
							variant="primary"
