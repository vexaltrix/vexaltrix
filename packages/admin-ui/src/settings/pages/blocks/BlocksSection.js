...
const BlocksSection = () => {
	...
	function convertToWidgetsArray( data ) {
		...
		for ( const key in data ) {
			if ( data.hasOwnProperty( key ) ) {
				...
				const IconElement = VXT_Block_Icons[ block.slug ];
	...
	useEffect( () => {
		const newBlocksArray = convertToWidgetsArray( vexaltrixAdmin.blocks_info );
		setFormattedBlocks( newBlocksArray );
	}, [ vexaltrixAdmin.blocks_info ] );
	...
	useEffect( () => {
		let filtered = convertToWidgetsArray( vexaltrixAdmin.blocks_info );

		let extesions = [
			{
				icon: React.cloneElement( VXT_Block_Icons[ 'animations-extension' ], { className: 'w-5 h-5' } ),
				id: '39',
				is_active: true,
				is_core: false,
				is_extension: true,
				...
				nonce: vexaltrixAdmin.enable_animations_extension_nonce,
			...
			{
				icon: React.cloneElement( VXT_Block_Icons[ 'display-conditions' ], { className: 'w-5 h-5' } ),
				id: '40',
				is_active: true,
				is_core: false,
				is_extension: true,
				...
				nonce: vexaltrixAdmin.enable_block_condition_nonce,
				action: 'vxt_enable_block_condition',
		...
		if ( filterState.type !== 'free' || filterState.type !== null ) {
			extesions.push( {
				icon: React.cloneElement( VXT_Block_Icons[ 'dynamic-content' ], { className: 'w-5 h-5' } ),
				id: '41',
				is_active: true,
				is_core: false,
				is_extension: true,
				...
				nonce: vexaltrixAdmin.enable_dynamic_content_nonce,
				action: 'vxt_enable_dynamic_content',
		...
		if ( filterState.type !== 'free' || filterState.type !== null ) {
			extesions.push( {
				icon: React.cloneElement( VXT_Block_Icons[ 'global-block-styles' ], { className: 'w-5 h-5' } ),
				id: '42',
				is_active: true,
				is_core: false,
				is_extension: true,
				...
				nonce: vexaltrixAdmin.enable_gbs_extension_nonce,
				action: 'vxt_enable_gbs_extension',
		...
		extesions = [
			...extesions,
			{
				icon: React.cloneElement( VXT_Block_Icons[ 'masonry-gallery' ], { className: 'w-5 h-5' } ),
				id: '43',
				is_active: true,
				is_core: false,
				is_extension: true,
				...
				title: __( 'Masonry Gallery', 'vexaltrix' ),
				extensionKey: 'enableMasonryExtension',
				nonce: vexaltrixAdmin.enable_masonry_gallery_nonce,
				action: 'vxt_enable_masonry_gallery',
			...
			{
				icon: React.cloneElement( VXT_Block_Icons[ 'responsive-conditions' ], { className: 'w-5 h-5' } ),
				id: '44',
				is_active: true,
				is_core: false,
				is_extension: true,
				...
				nonce: vexaltrixAdmin.enable_block_responsive_nonce,
				action: 'vxt_enable_block_responsive',
		...
		if ( vexaltrixAdmin.pro_plugin_status !== 'Activated' ) {
		...
		const isBlockActive = ( block ) => {
			if ( ! block.is_extension ) {
				if ( vexaltrixAdmin.pro_plugin_status !== 'Activated' ) {
					return blocksStatuses[ block.slug ] === block.slug && ! block.is_pro;
				}
				return blocksStatuses[ block.slug ] === block.slug;
			}
			return activeExtensions[ block.slug ] === true;
		};

		const getTier = ( block ) => {
			if ( block.is_pro ) {
				return 1; // Pro blocks and extensions first
			}
			if ( block.is_core ) {
				return 2; // Core blocks second
			}
			if ( ! block.is_extension && isBlockActive( block ) ) {
				return 3; // Active free blocks third
			}
			if ( ! block.is_extension && ! isBlockActive( block ) ) {
				return 4; // Inactive free blocks fourth
			}
			return 5; // Free extensions last
		};

		// Sort blocks: Pro first, Core blocks second, Active blocks third, Inactive blocks fourth, Extensions last. Within each tier, sort alphabetically.
		filtered.sort( ( a, b ) => {
	...
	const toggleAllBlocks = ( activate ) => {
		...
		const data = {
			security: vexaltrixAdmin.blocks_activation_and_deactivation_nonce,
			...
			<div className="rounded-lg bg-white w-full border border-solid border-border-subtle">
				<div className="flex flex-col sm:flex-row sm:items-center items-start justify-between gap-4 p-4">
					...
					<div className="flex items-center gap-3">
						<Input
							className="search-input vxt-input bg-[#F9FAFB]"
						...
						<DropdownMenu placement="bottom-end">
							<DropdownMenu.Trigger>
								<Button
									className="flex justify-center items-center relative vxt-input-border"
									size="sm"
									tag="button"
									type="button"
									variant="outline"
								...
								<span className="sr-only">{ __( 'Open Menu', 'vexaltrix' ) }</span>
							</DropdownMenu.Trigger>
