...
import VXT_Block_Icons from '@Common/block-icons';
...
const InstagramUsers = () => {
	...
	useEffect( () => {
		const data = {
			security: vexaltrixAdmin.insta_refresh_all_tokens_nonce,
			value: true,
		};
	...
	const handleInstaLinkUserLable = ( type = null ) => {
		switch ( type ) {
			case 'saving':
				setInstaLinkUserLabel( __( 'Linking', 'vexaltrix' ) );
				break;
			case 'invalid':
				setInstaLinkUserLabel( __( 'Inavalid Token', 'vexaltrix' ) );
				break;
			case 'exists':
				setInstaLinkUserLabel( __( 'Account Exists!', 'vexaltrix' ) );
				break;
			case 'success':
				setInstaLinkUserLabel( __( 'Account Linked!', 'vexaltrix' ) );
				break;
			case 'failed':
				setInstaLinkUserLabel( __( 'Failed to Add Account', 'vexaltrix' ) );
				break;
			default:
				setInstaLinkUserLabel( __( 'Link Account', 'vexaltrix' ) );
		}
	};

	// Highlight the User if they are already linked.
	const highlightLinkedUser = ( theID ) => {
		const heWhoExists = document.getElementById( `Vexaltrix-IG-User-${ theID }` );
		if ( heWhoExists ) {
			heWhoExists.classList.toggle( 'border-vexaltrix' );
			setTimeout( () => {
				heWhoExists.classList.toggle( 'border-vexaltrix' );
			}, 1000 );
		}
	...
	const handleInstaLinkAccount = ( clickedButton ) => {
		...
		const checkUser = `${ VXT_LINKS.INSTAGRAM_URL }/me?fields=id,username,account_type,profile_picture_url&access_token=${ escapeHTML(
			tempToken
		) }`;
		setLinkingUser( true );
		...
		window
			...
			.catch( () => {
				dispatch( {
					...
					payload: { message: __( 'Invalid Token', 'vexaltrix' ), messageType: 'error' },
				} );
	...
	const handleNewUserCreation = ( userID, userName, theButton, userType = 'personal', userProfile = null ) => {
		...
		const refreshLink = `${ VXT_LINKS.INSTAGRAM_URL }/refresh_access_token
		...
		if ( 0 !== instaLinkedAccounts.length ) {
			...
			if ( isFound ) {
				dispatch( {
					type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION',
					payload: {
						message: __( 'Account Exists!', 'vexaltrix' ),
						messageType: 'warning',
					},
				} );
		...
		window
			...
			.then( ( data ) => {
				...
				const formData = {
					security: vexaltrixAdmin.insta_linked_accounts_nonce,
				...
				getApiDataFetch.then( ( responseData ) => {
					if ( responseData.success ) {
						...
						dispatch( {
							type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION',
							payload: __( 'Account Linked!', 'vexaltrix' ),
						} );
						...
						dispatch( {
							type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION',
							payload: {
								message: __( 'Failed to Add Account', 'vexaltrix' ),
								messageType: 'error',
							},
						} );
						setTimeout( () => {
	...
	const generateDP = ( user ) => {
		const profileUserName =
			user?.userName && 'string' === typeof user.userName
				...
				: VXT_Block_Icons[ 'instagram-feed' ];
		switch ( user.userType.toLowerCase() ) {
			case 'personal':
				// Will be the profile picture.
				return (
					<div className="h-12 w-12 aspect-square rounded-full bg-violet-100 text-vexaltrix flex justify-center items-center text-xl">
						{ profileUserName }
					</div>
				);
			case 'business':
			default:
				// Will be the profile picture.
				return (
					<div className="h-12 w-12 aspect-square rounded-full bg-vexaltrix text-violet-100 overflow-hidden">
	...
	const getAccountType = ( userType ) => {
		switch ( userType ) {
			case 'personal':
				return __( 'Personal Account', 'vexaltrix' );
			case 'business':
				return __( 'Business Account', 'vexaltrix' );
			default:
				return __( 'Instagram Account', 'vexaltrix' ); // All the different Creator account types will show as Instagram account.
		}
	};
			...
			return (
				<>
					<p className="text-sm text-slate-400">
						{ __( 'No linked accounts.', 'vexaltrix' ) }
					</p>
					<p className="text-sm text-slate-400">
						{ __(
							'Please link an Instagram Account to start using it with Vexaltrix!',
							'vexaltrix'
						) }
					</p>
				</>
		...
		const userMatrix = instaLinkedAccounts.map( ( user ) => {
			...
			const RenderUserDetails = () => (
				<div
					...
					id={ `Vexaltrix-IG-User-${ user.userID }` }
				...
				>
					<button
						...
						aria-label={ __( 'Unlink', 'vexaltrix' ) }
		...
		return atLeastOneDeactivated ? (
			<>
				...
				<p className="text-sm text-red-600 pl-2">
					{ __( '1. The Instagram Account password was changed.', 'vexaltrix' ) }
				</p>
				<p className="text-sm text-red-600 pl-2">
					{ __( '2. The Instagram Account was made private.', 'vexaltrix' ) }
				</p>
				<p className="text-sm text-red-600 pl-2">
					{ __(
						'3. The Instagram Account was unused on Vexaltrix for 60 days.',
						'vexaltrix'
					) }
				</p>
				<br />
				<p className="text-sm text-red-600">
					{ __( 'Please unlink and relink the account(s) marked in red.', 'vexaltrix' ) }
				</p>
			</>
		) : null;
	};
	...
	const renderDevSettings = () => (
		<>
			<div className="mt-4 grid grid-cols-[1fr_auto] w-full">
				<input
					className="mr-5 h-10 text-sm placeholder-slate-400 transition vexaltrix-admin__input-field"
					type="text"
					aria-label="Token"
					placeholder={ __( 'Enter your Instagram access token', 'vexaltrix' ) }
					value={ tempToken }
				...
				<button
					...
					className={ uagbClassNames( [
						! tempToken
							...
							: 'bg-vexaltrix text-white hover:bg-vexaltrix-hover focus:bg-vexaltrix-hover cursor-pointer custom-border',
			...
			<Container align="center" className="mb-0.5 w-full flex justify-between">
				<Container.Item className="space-y-1 max-w-[480px]">
					<Label className="font-semibold" htmlFor="default-width" size="md">
						{ __( 'Connect Instagram', 'vexaltrix' ) }
					</Label>
						...
						<div className="mb-1">
							{ ReactHtmlParser(
								sprintf(
									...
									`<a class="text-vexaltrix focus:text-vexaltrix-hover active:text-vexaltrix-hover hover:text-vexaltrix-hover" href="${ VXT_LINKS.VXT_URL }/docs/meta-app-for-instagram/" target="_blank" rel="noreferrer">`,
									'</a>'
								)
							) }
						</div>
					...
					<div className="w-full border-t border-t-slate-200">{ renderAllLinkedAccounts() }</div>
