...
const BlocksEditorSpacing = () => {
	...
	const updateBlocksEditorSpacing = ( value ) => {
        ...
        const data = {
            security: vexaltrixAdmin.blocks_editor_spacing_nonce,
            value,
        };
        // Call the getApiData function with the specified parameters
        const getApiFetchData = getApiData( {
            url: vexaltrixAdmin.ajax_url,
            action: 'vxt_blocks_editor_spacing',
            data,
        } );
    ...
    return (
        <>
			<SettingsItem
				title={ __( 'Blocks Editor Spacing', 'vexaltrix' ) }
				settingText={ __( 'This setting will apply spacing in between all blocks inside block editor.', 'vexaltrix' ) }
			>
				<Input
					defaultValue={ 1140 }
					id="default-width"
