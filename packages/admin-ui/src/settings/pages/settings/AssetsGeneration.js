...
const AssetsGeneration = () => {
	...
	const url = vexaltrixAdmin.vxt_links?.docsUrl?.replace( /\/docs\/?/, '/docs/file-generation/' )?.toString();

	const renderText = () => (
		<p className='m-0'>
			{ __( 'Please read', 'vexaltrix' ) }
	...
	return (
		<>
			<SettingsItem
				title={ __( 'File Generation', 'vexaltrix' ) }
				settingText={ __(
					'Vexaltrix loads the CSS and JS inline on the page by default. If you want to generate separate CSS and JS files for Vexaltrix blocks, enable this option.',
					'vexaltrix'
				) }
