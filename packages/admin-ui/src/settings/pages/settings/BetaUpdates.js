...
const BetaUpdates = () => {
	...
	const checkForBetaUpdate = useCallback( () => {
		...
		if ( ! vexaltrixAdmin?.check_beta_update_available_nonce || ! vexaltrixAdmin?.ajax_url ) {
			setHasError( true );
		...
		const data = {
			security: vexaltrixAdmin.check_beta_update_available_nonce,
			value: 'yes',
		};
	...
	const handleConfirmUpdate = () => {
		...
		// Validate required data exists
		if ( ! vexaltrixAdmin?.update_beta_plugin_nonce || ! vexaltrixAdmin?.ajax_url ) {
			return;
		}
		...
		const data = {
			security: vexaltrixAdmin.update_beta_plugin_nonce,
			value: 'yes',
		};
		...
		getApiFetchData
			.then( ( response ) => {
				...
				if ( response?.success && response?.data ) {
					...
					dispatch( {
						...
						payload: response.data.messsage || __( 'Plugin updated successfully!', 'vexaltrix' ),
					} );
					...
					const errorMessage = response?.data?.messsage || __( 'Unable to update. Please try again.', 'vexaltrix' );
					dispatch( {
						type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION',
			...
			.catch( () => {
				if ( isMountedRef.current ) {
					dispatch( {
						type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION',
						payload: __( 'Unable to update. Please try again.', 'vexaltrix' ),
					} );
					setIsUpdating( false );
					setShowConfirmModal( false );
				}
	...
	const updateEnableBetaStatus = () => {
		// Validate required data exists
		if ( ! vexaltrixAdmin?.enable_beta_updates_nonce || ! vexaltrixAdmin?.ajax_url ) {
			return;
		}
		...
		const data = {
			security: vexaltrixAdmin.enable_beta_updates_nonce,
			value: assetStatus,
		};
		...
		getApiFetchData
			...
			.catch( () => {
				...
				dispatch( {
					type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION',
					payload: __( 'Unable to save settings. Please try again.', 'vexaltrix' ),
				} );
			} );
	};
	...
	return (
		<>
			<SettingsItem
				title={ __( 'Enable Beta', 'vexaltrix' ) }
				settingText={ __(
					'Enable this option to turn on beta updates and be notified when a new beta version of Vexaltrix is available. The beta version will not install automatically, you will have to install it when you get a notification. It is recommended to try beta on a test environment only.',
					'vexaltrix'
				) }
			>
				<Switch
				...
				<div className="mt-4 p-4 rounded-md border border-border-subtle bg-background-secondary">
					{ isCheckingUpdate && (
						<p className="text-sm text-text-secondary m-0">
							{ __( 'Checking for beta updates…', 'vexaltrix' ) }
						</p>
					) }
					...
					{ ! isCheckingUpdate && hasError && (
						<div className="flex items-center justify-between gap-4">
							<p className="text-sm text-text-secondary m-0">
								{ __( 'Unable to check for updates. Please try again.', 'vexaltrix' ) }
							</p>
							<Button
								variant="secondary"
								size="sm"
						...
						<div className="flex items-center justify-between gap-4">
							<div className="flex-1">
								<p className="text-sm font-medium text-text-primary m-0 mb-1">
									{ __( 'Beta Update Available!', 'vexaltrix' ) }
								</p>
								<p className="text-sm text-text-secondary m-0">
									{ betaUpdateInfo.messsage || __( 'A new beta version is available.', 'vexaltrix' ) }
								</p>
							</div>
							<Button
								variant="primary"
							...
							>
								...
								{ isUpdating ? __( 'Updating…', 'vexaltrix' ) : __( 'Update Now', 'vexaltrix' ) }
							</Button>
						</div>
					) }
					...
					{ ! isCheckingUpdate && ! hasError && betaUpdateInfo && ! betaUpdateInfo.has_update && (
						<div className="flex items-center justify-between gap-4">
							<p className="text-sm text-text-secondary m-0">
								{ __( 'You are running the latest version.', 'vexaltrix' ) }
							</p>
							<Button
								variant="secondary"
								size="sm"
							...
							<div className="inline-block p-3 rounded-lg bg-background-primary w-120 font-[Figtree] relative z-999999">
								<div className="mb-2 p-2">
									<div className="flex w-full justify-between items-center mb-2">
										<div className="text-base font-semibold text-text-primary">
											{ __( 'Update to Beta Version', 'vexaltrix' ) }
										</div>
									...
									<div className="text-sm text-text-secondary font-normal w-full text-left">
										{ __( 'This is a beta version and not recommended for live sites. It is recommended to test on a staging or test environment only.', 'vexaltrix' ) }
									</div>
								</div>
								...
								<div className="p-2">
									<div className="flex justify-end items-center w-full gap-3">
										...
										<Button
											className="bg-button-primary text-text-on-color vxt-remove-ring hover:bg-button-primary-hover"
											size="md"
											tag="button"
											type="button"
											variant="primary"
										...
										>
											{ isUpdating && (
												...
												{ isUpdating ? __( 'Updating…', 'vexaltrix' ) : __( 'Update', 'vexaltrix' ) }
										</Button>
									</div>
								</div>
							</div>
