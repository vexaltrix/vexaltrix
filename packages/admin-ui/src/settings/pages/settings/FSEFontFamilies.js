...
import FSEFontFamiliesSkeleton from '@Settings/pages/settings/FSEFontFamiliesSkeleton';
...
const FSEFontFamilies = () => {
	...
	const vexaltrixFSEFonts = useSelector( ( state ) => state.vexaltrixFSEFonts );
	...
	const customFonts = vexaltrixAdmin.vexaltrix_custom_fonts;
	const savedFonts = [];
	themeFonts.map( ( font ) => {
	...
	const updateEnableSelectedFontFamilies = () => {
		...
		const data = {
			security: vexaltrixAdmin.load_fse_font_globally_nonce,
			value: assetStatus,
		};
	...
	const saveGoogleFontsHandler = () => {
		...
		const data = {
			security: vexaltrixAdmin.fse_font_globally_nonce,
	...
	return (
		<>
			<SettingsItem
				title={ __( 'Font Families', 'vexaltrix' ) }
				settingText={ __(
					'Vexaltrix offers 1500+ Google font options. If this is overwhelming for your clients, you can use this option to show only limited number of fonts in the block settings.',
					'vexaltrix'
				) }
			>
				<Switch
			...
			<div className="w-full flex flex-col items-center">
				<Select
					placeholder={ __( 'Select the font families you want', 'vexaltrix' ) }
				...
				{ fseFontFamilies?.value && fseFontFamilies?.weights && (
					<Select
						isMulti
						placeholder={ __( 'Select one or more font weights', 'vexaltrix' ) }
				...
				{ fseFontFamilies?.value && fseFontFamilies?.styles && (
					<Select
						isMulti
						placeholder={ __( 'Select one or more font styles', 'vexaltrix' ) }
				...
				{ fseFontFamilies?.value && fseFontFamilies?.weight && fseFontFamilies?.style && (
					<button
						type="button"
						className="flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-vexaltrix transition focus:bg-vexaltrix-hover hover:bg-vexaltrix-hover focus:outline-none h-9 mt-3 cursor-pointer"
			...
			{ enableFSEFontFamiliesStatus && (
				<div className="relative overflow-x-auto py-8 lg:px-12 max-h-[23rem]">
					<table className="w-full text-sm text-left text-gray-500">
						<thead className="text-xs text-gray-700 uppercase bg-gray-50">
							<tr>
								<th scope="col" className="lg:px-6 py-3">
									{ __( 'Font Family', 'vexaltrix' ) }
								</th>
								<th scope="col" className="lg:px-6 py-3">
									{ __( 'Variant', 'vexaltrix' ) }
								</th>
								<th scope="col" className="lg:px-6 py-3">
									{ __( 'Style', 'vexaltrix' ) }
								</th>
						...
						<tbody>
							{ 0 === vexaltrixFSEFonts?.length && (
								<div className="lg:px-6 py-3">
									{ __( 'Currently, no Font Families added.', 'vexaltrix' ) }
								</div>
							) }
							{ vexaltrixFSEFonts &&
								0 !== vexaltrixFSEFonts?.length &&
								vexaltrixFSEFonts.map( ( font ) => {
									return (
