...
const Visibility = () => {
	...
	const updateVisibilityMode = ( mode ) => {
		...
		const data = {
			security: vexaltrixAdmin.visibility_mode_nonce,
			value: assetStatus,
		};
	...
	const updateSelectedPage = ( page ) => {
		...
		const data = {
			security: vexaltrixAdmin.visibility_page_nonce,
			value: page.value,
		};
	...
	const fetchPageHandler = ( keyword = '' ) => {
		// Create an object with the security and value properties
		const data = {
			security: vexaltrixAdmin.fetch_pages_nonce,
			keyword,
		};
		setFetchPages( true );
		// Call the getApiData function with the specified parameters
	...
	const renderSelectComponent = () => (
		<Select
			isMulti={ false }
			placeholder={ __( 'Select the page you want', 'vexaltrix' ) }
	...
	return (
		<>
			<SettingsItem
				title={ __( 'Enable Coming Soon Mode', 'vexaltrix' ) }
				settingText={ __(
			...
			<SettingsItem
				title={ __( 'Enable Maintenance Mode', 'vexaltrix' ) }
				settingText={ __(
