...
const BetaUpdateNotice = () => {
	...
	const handleConfirmUpdate = () => {
		...
		if ( ! vexaltrixAdmin?.enable_beta_updates_nonce || ! vexaltrixAdmin?.update_beta_plugin_nonce || ! vexaltrixAdmin?.ajax_url ) {
			return;
		}
		...
		getApiData( {
			...
			.then( ( enableResponse ) => {
				...
				if ( enableResponse?.success ) {
						...
						.then( ( updateResponse ) => {
							...
							if ( updateResponse?.success && updateResponse?.data ) {
								...
								dispatch( {
									...
									payload: updateResponse.data.messsage || __( 'Plugin updated successfully!', 'vexaltrix' ),
								} );
							...
							setIsUpdating( false );
								...
								const errorMessage = updateResponse?.data?.messsage || __( 'Unable to update. Please try again.', 'vexaltrix' );
								dispatch( {
									type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION',
					...
					dispatch( {
						type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION',
						payload: __( 'Unable to enable beta updates. Please try again.', 'vexaltrix' ),
					} );
							setShowConfirmModal( false );
					setIsUpdating( false );
				}
	...
	// Don't render when no beta update is available.
	if ( ! betaUpdateInfo?.has_update ) {
		return null;
	}
	...
	const versionTitle = majorVersion
		? `Vexaltrix v${ majorVersion } Beta is Here!`
		: __( 'Vexaltrix Beta is Here!', 'vexaltrix' );

	const currentVersion = betaUpdateInfo.current_version || vexaltrixAdmin?.plugin_ver || '';
			...
			<Container.Item className="flex flex-1 flex-col gap-2">
				...
				<div>
					...
					<p className="text-sm text-text-secondary m-0">
						{ __( 'Be among the first to explore what\'s next and share your thoughts to help us make it even better!', 'vexaltrix' ) }
					</p>
				</div>
				...
				<div className="flex items-start gap-2 bg-alert-background-warning border border-alert-border-warning rounded-md px-3 py-2 mt-2">
					...
					<p className="text-sm text-text-primary m-0">
						<span className="font-semibold">{ __( 'Important:', 'vexaltrix' ) }</span>{ ' ' }
						{ __( 'Vexaltrix v3 is still in Beta and should not be used on live sites yet.', 'vexaltrix' ) }
					</p>
				</div>
				...
				<div className="flex gap-3 mt-2">
					<Button
						variant="primary"
						className="vxt-remove-ring"
						onClick={ handleTryNowClick }
						disabled={ isUpdating }
					>
						{ isUpdating
							? __( 'Updating…', 'vexaltrix' )
							: __( 'Try It Now', 'vexaltrix' ) }
					</Button>
				</div>
			</Container.Item>
