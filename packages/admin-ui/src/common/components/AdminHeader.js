...
const WhatsNewFeed = ( { rssFeedURL } ) => {
	useWhatsNewRSS( {
		rssFeedURL,
		selector: '#vxt-whats-new',
		triggerButton: {
			beforeBtn:
	...
	return <div id="vxt-whats-new" size={ 16 }></div>;
};
...
const AdminHeader = ( props ) => {
	...
	const creditDetails = vexaltrixAdmin.zip_ai_credit_details;
	const whatsNewFeedUrl = vexaltrixAdmin.vxt_links?.whatsNewFeedUrl;
	...
	return (
		<Disclosure as="nav" className="bg-white shadow border border-solid border-border-subtle">
			{ 'Install' === vexaltrixAdmin.pro_plugin_status && showHeader && (
				<Container align="center" justify="center" className="py-2 relative bg-[#E9E4FF]">
					<div className="space-x-1 text-text-primary text-xs">
						<span className="font-semibold">
							{ __( "Unlock Vexaltrix' Full Potential!", 'vexaltrix' ) }
						</span>
						...
						<a
							...
							href={vexaltrixAdmin.vxt_links?.banner}
							target="_self"
							rel="noreferrer"
						>
			...
			<Topbar gap={ 0 } className="p-0 shadow-sm w-full">
				<Topbar.Left className="py-4 lg:px-4 lg:pl-8 md:px-2 sm:px-1 px-0">
					<Topbar.Item>
						<Link
							to={ {
								pathname: 'admin.php',
								search: `?page=${ vexaltrixAdmin.home_slug }`,
							} }
				...
				<Topbar.Right gap="md" className="lg:px-4 lg:pr-8 md:px-2 px-1">
					...
					{ null !== whatsNewFeedUrl && undefined !== whatsNewFeedUrl && (
						<WhatsNewFeed rssFeedURL={ whatsNewFeedUrl } />
					) }
					...
					<Topbar.Item className="relative after:content-[''] after:inline-block after:size-1.5 after:bg-background-important after:rounded-full after:absolute after:-top-0.5 after:left-5">
						<DropdownMenu placement="bottom-end">
							<DropdownMenu.Trigger>
								...
								{ vexaltrixAdmin.vexaltrix_pro_installed &&
								vexaltrixAdmin.vexaltrix_pro_ver &&
								typeof ( vxt_ultimate_gutenberg_blocks_user_data.firstName[ 0 ] + vxt_ultimate_gutenberg_blocks_user_data.lastName[ 0 ] ) === 'string' ? (
									<div className="bg-[#f3f0ff] text-black p-1 rounded-full font-[Figtree]">
										{ vxt_ultimate_gutenberg_blocks_user_data.firstName[ 0 ] + vxt_ultimate_gutenberg_blocks_user_data.lastName[ 0 ] }
									</div>
								) : (
							...
							<DropdownMenu.ContentWrapper>
								<DropdownMenu.Content className="w-60">
									<DropdownMenu.List>
											...
											<div className="flex w-full justify-between items-center font-[Figtree]">
												...
												<Badge
													label={ sprintf(
														...
														__( '%s', 'vexaltrix' ),
														vexaltrixAdmin.license_status ? 'Active' : 'Inactive'
													) }
													size="xxs"
													variant={ vexaltrixAdmin.license_status ? 'green' : 'yellow' }
												/>
											</div>
										</DropdownMenu.Item>
										...
										<DropdownMenu.Item>
											<div className="flex w-full justify-between items-center font-[Figtree]">
												<div className="flex gap-1">
													...
													<div>{ __( 'AI Credits', 'vexaltrix' ) }</div>
												</div>
										...
										<DropdownMenu.Item
											onClick={ () => openLink( `${ VXT_LINKS.ZIP_URL }/org/billing` ) }
										>
											<div className="flex w-full justify-between items-center font-[Figtree]">
												<div className="flex gap-1">
													{ plan() }
