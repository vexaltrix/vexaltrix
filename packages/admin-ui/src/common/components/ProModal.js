...
const ProModal = ( { modalData, setIsModalOpen, pricingUrl = '', usePricingApi = false } ) => {
	...
	useEffect( () => {
		const checkDropdown = () => {
			const modalContainers = document.querySelectorAll( '.vxt-upsell-modal, [data-floating-ui-focusable]' );
	...
	return (
		<div
			...
			className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-999999 vxt-upsell-modal"
		>
			<div
				...
				{ loading ? (
					...
					<>
						...
						<div className="w-full flex justify-end md:pr-[10px] pr-2">
							<a
								href={vexaltrixAdmin.vxt_links?.upsellModalAdmin}
								target="_blank"
								rel="noreferrer"
