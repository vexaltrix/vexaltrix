...
const BlockItem = ( { block, is_extension } ) => {
	...
	useEffect( () => {
		if ( block.is_extension && ! ( block.is_pro && vexaltrixAdmin.pro_plugin_status !== 'Activated' ) ) {
			block.onChange( ( prev ) => ( {
				...prev,
			...
			<Container align="center" containerType="flex" direction="column" justify="between" gap="">
				<div className="flex items-center justify-between w-full mb-1 p-1">
					...
					<div className="flex items-center gap-x-2">
						{ block.is_pro && vexaltrixAdmin.pro_plugin_status !== 'Activated' ? (
							...
							<>
								{ isCoreBlock ? (
									<Tooltip
										...
										content={
											<span>
												{ __( 'Core blocks are essential for Vexaltrix\' operation and cannot be turned off.', 'vexaltrix' ) }
											</span>
										}
										placement="bottom"
										title={ __( 'Core Block', 'vexaltrix' ) }
									...
									>
										<Switch
											...
											disabled={ true }
											className="switch-disabled vxt-remove-ring border-none"
										/>
									</Tooltip>
								) : (
									<Switch
				...
				<div className="flex flex-col w-full p-1">
					...
					<div className="flex items-center justify-between w-full">
						<a
							href={ `${ VXT_LINKS.VXT_URL }/blocks-and-extensions/${ block.link }` }
							target="_blank"
							rel="noreferrer"
