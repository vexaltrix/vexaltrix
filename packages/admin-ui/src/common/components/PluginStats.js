...
const PluginStats = () => {
	...
	const activatePro = () => {
		const isThisNull = vexaltrixAdmin.vexaltrix_pro_status;
			...
			vexaltrixAdmin.vexaltrix_pro_status ? (
				vexaltrixAdmin.license_status ? (
                    <Link
						...
						to={ {
							pathname: 'admin.php',
							search: '?page=vexaltrix&path=settings&settings=license',
						} }
						onClick={ () => {
							dispatch( { type:'UPDATE_SETTINGS_ACTIVE_NAVIGATION_TAB', payload: 'license' } )
						} }
                    >
						{ __( 'Licensed', 'vexaltrix' ) }
					</Link>
				) : (
                    <Link
				...
				>
					{ __( 'Activate Vexaltrix Pro', 'vexaltrix' ) }
				</div>
			)
		) : (
			<a
				className='mr-2 ml-2 text-sm flex items-center text-vexaltrix active:text-vexaltrix hover:text-vexaltrix focus:text-vexaltrix cursor-pointer font-medium'
	...
	return (
		<>
			{'not-installed' === vexaltrixAdmin.pro_plugin_status && (
				<>
					<a
						className='mr-2 ml-2 text-sm flex items-center text-vexaltrix active:text-vexaltrix hover:text-vexaltrix focus:text-vexaltrix cursor-pointer font-medium'
						href={vexaltrixAdmin.vxt_links?.topBar}
