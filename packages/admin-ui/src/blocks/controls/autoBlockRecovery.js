...
const createRecoveryCSS = () => {
	...
	recoveryCSS.setAttribute( 'id', 'vxt-recovery-styles' );
	recoveryCSS.innerHTML = '.has-warning[data-type^="vexaltrix/"] { opacity: 0 !important; }';
...
const destroyRecoveryCSS = () => {
	const recoveryCSS = document.querySelector( '#vxt-recovery-styles' );
	if ( recoveryCSS ) {
		document.body.removeChild( recoveryCSS );
	}
};

// Start Block Recovery for all Vexaltrix.
		...
		const unsubscribe = subscribe( () => {
			if ( select( 'core' ).getEntityRecords( 'postType', 'wp_block' ) !== null ) {
				...
				if ( recoveryDone ) {
					...
					console.log(
						'%cVexaltrix Auto Recovery Enabled: All Vexaltrix on this page have been recovered!',
