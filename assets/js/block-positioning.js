const VexaltrixBlockPositioning = {
	// Initialize the required positioning functionality.
	init( attr, selector ) {
		const element = document.querySelector( selector );
		if ( element?.classList.contains( 'vxt-position__sticky' ) ) {
			VexaltrixBlockPositioning.handleSticky( element, attr );
		}
	},
	...
	handleSticky( element, attr ) {
		...
		const createStickyFiller = ( elementNode, elementDimensions, elementParent ) => {
			...
			if ( ! elementParent ) {
				...
				fillerElement.style.maxWidth =
					elementStyles.getPropertyValue( 'max-width' ) || `${ elementDimensions.width }px`;
		...
		if ( attr?.UAGStickyRestricted ) {
			...
			parentHaltAt.bottom =
				...
				parentInnerPositions.bottom -
				stickyDimensions.height -
				getAdminbarHeight() -
				( attr?.UAGStickyOffset || 0 );
		}
		...
		if ( 'bottom' === attr?.UAGStickyLocation ) {
			...
			if ( scrollPosition <= haltAt && ! element.classList.contains( 'vxt-position__sticky--stuck' ) ) {
				element.parentNode.insertBefore( fillerElement, element );
				element.classList.add( 'vxt-position__sticky--stuck' );
			...
			window.addEventListener( 'scroll', () => {
				...
				if ( scrollPosition <= haltAt ) {
					if ( ! element.classList.contains( 'vxt-position__sticky--stuck' ) ) {
						element.parentNode.insertBefore( fillerElement, element );
						element.classList.add( 'vxt-position__sticky--stuck' );
						element.style.bottom = haltAtPosition;
						element.style.left = `${ stickyDimensions.left }px`;
				...
				} else if ( scrollPosition > haltAt && element.classList.contains( 'vxt-position__sticky--stuck' ) ) {
					element.parentNode.removeChild( fillerElement );
					element.classList.remove( 'vxt-position__sticky--stuck' );
			...
			if ( scrollPosition >= haltAt && ! element.classList.contains( 'vxt-position__sticky--stuck' ) ) {
				...
				if ( attr?.UAGStickyRestricted && scrollPosition >= parentHaltAt.bottom ) {
					element.classList.remove( 'vxt-position__sticky--stuck' );
					element.classList.add( 'vxt-position__sticky--restricted' );
					element.style.top = '';
			...
			window.addEventListener( 'scroll', () => {
				...
				if ( scrollPosition >= haltAt ) {
					...
					if (
						! element.classList.contains( 'vxt-position__sticky--stuck' ) &&
						! element.classList.contains( 'vxt-position__sticky--restricted' )
					) {
						element.parentNode.insertBefore( fillerElement, element );
						element.classList.add( 'vxt-position__sticky--stuck' );
						element.style.top = haltAtPosition;
						...
						attr?.UAGStickyRestricted &&
						! element.classList.contains( 'vxt-position__sticky--restricted' ) &&
						scrollPosition >= parentHaltAt.bottom
					) {
						element.classList.remove( 'vxt-position__sticky--stuck' );
						element.classList.add( 'vxt-position__sticky--restricted' );
						element.style.top = '';
						...
						element.classList.remove( 'vxt-position__sticky--restricted' );
						element.classList.add( 'vxt-position__sticky--stuck' );
						element.style.top = haltAtPosition;
						element.style.bottom = '';
						element.style.left = `${ stickyDimensions.left }px`;
