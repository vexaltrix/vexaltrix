VexaltrixForms = {
	getElement: ( id ) => {
		...
		const getJsELement = document.querySelector( `${ id }:not(.vxt-activated-script)` );
		if ( ! getJsELement ) {
			return null;
		}

		// Ensures that the script only runs once on the given element (required for homepage sidebar usage case).
		getJsELement.classList.add( 'vxt-activated-script' );
		return getJsELement;
	},

	init( attr, id, post_id ) {
		const scope = VexaltrixForms.getElement( id );
		if ( ! scope ) {
			return;
		}

		const form = scope.querySelector( '.vxt-forms-main-form' );

		const phoneinput = form.querySelectorAll( '.vxt-forms-phone-input' );
		...
		const toggleinput = form.querySelectorAll( '.vxt-forms-toggle-input' );
		...
		const requiredCheckboxes = scope.querySelectorAll( '.vxt-forms-checkbox-wrap' );
		...
		if ( attr.reCaptchaEnable === true && attr.reCaptchaType === 'v2' ) {
			reCaptchaSiteKeyV2 = vxt_ultimate_gutenberg_blocks_forms_data.recaptcha_site_key_v2;

			if ( reCaptchaSiteKeyV2 ) {
				if ( null === document.querySelector( '.vxt-forms-field-set' ).getAttribute( 'data-sitekey' ) ) {
			...
			reCaptchaSiteKeyV3 = vxt_ultimate_gutenberg_blocks_forms_data.recaptcha_site_key_v3;
		...
		//Ready Classes.
		const formscope = document.getElementsByClassName( 'vxt-block-' + attr.block_id );
		if ( formscope?.[ 0 ] ) {
		...
		form.addEventListener( 'submit', function ( e ) {
			...
			if ( attr.reCaptchaEnable === true && attr.reCaptchaType === 'v3' && reCaptchaSiteKeyV3 ) {
				if ( document.getElementsByClassName( 'grecaptcha-logo' ).length === 0 ) {
					document.querySelector( '.vxt-form-reacaptcha-error-' + attr.block_id ).innerHTML =
				...
				grecaptcha.ready( function () {
					grecaptcha.execute( reCaptchaSiteKeyV3, { action: 'submit' } ).then( function ( token ) {
						if ( token ) {
							if ( document.getElementsByClassName( 'vxt-forms-recaptcha' ).length !== 0 ) {
								document.getElementById( 'g-recaptcha-response' ).value = token;

								window.VexaltrixForms._formSubmit(
									e,
									form,
									attr,
									reCaptchaSiteKeyV2,
				...
				window.VexaltrixForms._formSubmit( e, this, attr, reCaptchaSiteKeyV2, reCaptchaSiteKeyV3, post_id );
			}
		} );
	},
	...
	_formSubmit( e, form, attr, reCaptchaSiteKeyV2, reCaptchaSiteKeyV3, post_id ) {
		...
		if ( '' === attr.afterSubmitToEmail || null === attr.afterSubmitToEmail ) {
			const hideForm = document.querySelector( '[name="vxt-form-' + attr.block_id + '"]' );
			hideForm.style.display = 'none';

			const errorMsg = document.querySelector( '.vxt-forms-failed-message-' + attr.block_id );
			errorMsg?.classList?.remove( 'vxt-forms-submit-message-hide' );
			errorMsg?.classList?.add( 'vxt-forms-failed-message' );
			return false;
		}
		...
		const originalSerialized = window.VexaltrixForms._serializeIt( form );
		...
		fetch( vxt_ultimate_gutenberg_blocks_forms_data.ajax_url, {
			method: 'POST',
			headers: new Headers( { 'Content-Type': 'application/x-www-form-urlencoded' } ),
			body: new URLSearchParams( {
				action: 'vxt_ultimate_gutenberg_blocks_process_forms',
				nonce: vxt_ultimate_gutenberg_blocks_forms_data.vxt_ultimate_gutenberg_blocks_forms_ajax_nonce,
			...
			.then( function ( data ) {
				...
				if ( 200 === data.data ) {
					if ( 'message' === attr.confirmationType ) {
						const errorMsg = document.querySelector( '.vxt-forms-success-message-' + attr.block_id );
						errorMsg.classList.remove( 'vxt-forms-submit-message-hide' );
						errorMsg.classList.add( 'vxt-forms-success-message' );
					}
					...
					if ( 'message' === attr.confirmationType ) {
						const successMsg = document.querySelector( '.vxt-forms-failed-message-' + attr.block_id );
						successMsg.classList.remove( 'vxt-forms-submit-message-hide' );
						successMsg.classList.add( 'vxt-forms-failed-message' );
					}
				}
			} )
