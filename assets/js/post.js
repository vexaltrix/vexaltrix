		...
		if ( scope.length > 0 ) {
			...
			Object.keys( postActive ).forEach( ( key ) => {
				const thisHeight = postActive[ key ].offsetHeight,
					blogPost = postActive[ key ].querySelector( '.vxt-post__inner-wrap' ),
			...
			Object.keys( postActive ).forEach( ( key ) => {
				const selector = postActive[ key ].querySelector( '.vxt-post__inner-wrap' );
				if ( selector ) {
			...
			Object.keys( postWrapper ).forEach( ( key ) => {
				...
				selector = $this.querySelector( '.vxt-post__inner-wrap' );
	...
	_unSetHeight( scope ) {
		if ( scope.length > 0 ) {
			...
			Object.keys( postActive ).forEach( ( key ) => {
				...
				const selector = $this.querySelector( '.vxt-post__inner-wrap' );
				selector.style.height = 'auto';
			} );
		}
	},
...
window.VexaltrixPostMasonry = {
	_init( $attr, $selector ) {
				...
				if ( window.pageYOffset + windowHeight50 >= offsetTop ) {
					...
					if ( true === loadStatus ) {
						if ( count <= total ) {
							...
							window.VexaltrixPostMasonry._callAjax( $scope, $args, $attr, loader, false, count );
							count++;
							loadStatus = false;
						}
					}
		...
		if ( 'button' === $attr.paginationEventType ) {
			if ( $scope?.querySelector( '.vxt-post-pagination-button' ) ) {
				$scope.style.marginBottom = '40px';

				$scope.querySelector( '.vxt-post-pagination-button' ).onclick = function () {
					$scope = this.closest( '.vxt-post-grid' );
					const total = $scope.getAttribute( 'data-total' );
					const $args = {
						total,
						page_number: count,
					};
					$scope.querySelector( '.vxt-post__load-more-wrap' ).style.display = 'none';
					if ( true === loadStatus ) {
						if ( count <= total ) {
							if ( loader.length > 0 ) {
								loader[ 0 ].style.display = 'none';
							}
							$scope.querySelector( '.vxt-post__load-more-wrap' ).style.display = 'block';
							window.VexaltrixPostMasonry._callAjax( $scope, $args, $attr, loader, true, count );
							count++;
							loadStatus = false;
						}
					}
	...
	_callAjax( $scope, $obj, $attr, loader, append = false, count ) {
		...
		PostData.append( 'action', 'vxt_ultimate_gutenberg_blocks_get_posts' );
		PostData.append( 'nonce', vxt_ultimate_gutenberg_blocks_data.vxt_ultimate_gutenberg_blocks_masonry_ajax_nonce );
		PostData.append( 'page_number', $obj.page_number );
		PostData.append( 'attr', JSON.stringify( $attr ) );

		fetch( vxt_ultimate_gutenberg_blocks_data.ajax_url, {
			method: 'POST',
			...
			.then( function ( data ) {
				...
				setTimeout( function () {
					...
					isotope.insert( window.VexaltrixPostMasonry.createElementFromHTML( data.data ) );
					loadStatus = true;
					...
					const articles = document.querySelectorAll(
						'.vxt-post__image-position-background .vxt-post__inner-wrap'
					);
					...
					for ( const article of articles ) {
						...
						const image = article.getElementsByClassName( 'vxt-post__image' );
						if ( image[ 0 ] ) {
...
window.VexaltrixPostGrid = {
	_callAjax( $attr, $page_number, block_id ) {
		// Create new FormData object with necessary data to send in AJAX call.
		const PostData = new FormData();
		PostData.append( 'action', 'vxt_ultimate_gutenberg_blocks_post_pagination_grid' );
		PostData.append( 'nonce', vxt_ultimate_gutenberg_blocks_data.vxt_ultimate_gutenberg_blocks_grid_ajax_nonce );
		...
		fetch( vxt_ultimate_gutenberg_blocks_data.ajax_url, {
			...
			.then( function ( data ) {
				...
				const grid_element = document.querySelector( '.vxt-block-' + block_id );
				if ( ! grid_element ) {
					return;
				}
				...
				const new_blockId = html.match( /vxt-block-([\w-]+)/ )?.[ 1 ] || '';
		...
		function addClickListeners( new_blockId ) {
			...
			const elements = document.querySelectorAll(
				`.vxt-post-grid.vxt-block-${ new_blockId } .vxt-post-pagination-wrap a`
			);
			elements.forEach( ( element ) => {
				element.addEventListener( 'click', ( event ) => {
					...
					window.VexaltrixPostGrid._callAjax( $attr, parseInt( pageNumber ), new_blockId );
				} );
			} );
		}
	},
...
function vxt_ultimate_gutenberg_blocks_carousel_height( id ) {
	const wrap = document.querySelector( '#wpwrap .is-carousel.vxt-block-' + id );
	if ( wrap ) {
		window.VexaltrixPostCarousel._setHeight( wrap );
	}
}
...
function vxt_ultimate_gutenberg_blocks_carousel_unset_height( id ) {
	const wrap = document.querySelector( '#wpwrap .is-carousel.vxt-block-' + id );
	if ( wrap ) {
		window.VexaltrixPostCarousel._unSetHeight( wrap );
	}
}
