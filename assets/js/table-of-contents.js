...
VexaltrixTableOfContents = {
	...
	_setCollapseIconMargin( id, attr ) {
		const document_collapsable = VexaltrixTableOfContents._getDocumentElement();
		const block_element = document_collapsable.querySelector( id );
		const uagbLoader = block_element.querySelector( '.vxt-toc__loader' );

		// Get the first list item to compute the ::marker styles.
		const firstListItem = block_element.querySelector( 'li.vxt-toc__list:not(.vxt-toc__list--expandable)' );
		if ( firstListItem ) {
			const listFontSize = window.getComputedStyle( firstListItem ).fontSize;
			const listWrap = block_element.querySelector( '.vxt-toc__list-wrap' );
			...
			const isSafari =
				/^((?!chrome|android|crios|fxios).)*safari/i.test( userAgent ) && ! userAgent.includes( 'edge' );
			...
			setTimeout( () => {
				...
				uagbLoader?.remove();
				listWrap?.classList.remove( 'vxt-toc__list-hidden' );
	...
	_initCollapsableList( id, attr ) {
		...
		if (
			attr?.isFrontend &&
			attr?.enableCollapsableList &&
			! block_element.classList.contains( 'init-collapsed-script' )
		) {
			block_element.classList.add( 'init-collapsed-script' ); // Mark script as executed.

			const ulElements = block_element.querySelectorAll( 'ul.vxt-toc__list' );

			// Set margins for collapsible icon in editor and frontend
			if ( 'function' === typeof VexaltrixTableOfContents._setCollapseIconMargin ) {
				VexaltrixTableOfContents._setCollapseIconMargin( id, attr );
			}
			...
			spanList.forEach( ( ele ) => {
				const handleToggle = () => {
					...
					if ( ! isExpanded ) {
						ulElement.classList.remove( 'vxt-toc__list--hidden-child' );
					}
					...
					setTimeout( () => {
						...
						ulElement.classList.toggle( 'vxt-toc__list--child-of-closed-list' );
					}, 0 );

					// If the list was expanded, add the display-none class after animating.
					if ( isExpanded ) {
						setTimeout( () => {
							ulElement.classList.add( 'vxt-toc__list--hidden-child' );
						}, 300 );
					}
				};
	...
	init( id, attr ) {
		if ( ( attr?.makeCollapsible && ! attr?.initialCollapse ) || ! attr?.makeCollapsible ) {
			VexaltrixTableOfContents._initCollapsableList( id, attr );
		}
		const document_element = VexaltrixTableOfContents._getDocumentElement();
		if ( document.querySelector( '.vxt-toc__list' ) !== null ) {
			document.querySelector( '.vxt-toc__list' ).addEventListener( 'click', VexaltrixTableOfContents._scroll );
		}
		if ( document.querySelector( '.vxt-toc__scroll-top' ) !== null ) {
			document
				.querySelector( '.vxt-toc__scroll-top' )
				.addEventListener( 'click', VexaltrixTableOfContents._scrollTop );
		}
		...
		if ( attr?.makeCollapsible ) {
			...
			function collapseListener( event ) {
				...
				const condition1 =
					element?.tagName === 'path' || element?.tagName === 'svg' || element?.tagName === 'DIV'; // Check if the clicked element type is either path or SVG or Title DIV.
				const condition2 =
					element?.className === 'vxt-toc__title' ||
					element?.parentNode?.className === 'vxt-toc__title' ||
					element?.parentNode?.tagName === 'svg'; // Check if the clicked element's parent has the required class.

				if ( condition1 && condition2 ) {
					const $root = element?.closest( `.wp-block-vxt-table-of-contents${ id }` );
					const tocListWrapEl = elementToOpen?.querySelector(
						'.wp-block-vxt-table-of-contents .vxt-toc__list-wrap'
					);
					// If not have the tocListWrapEl then return false!
					if ( ! tocListWrapEl ) {
						return;
					}
					if ( $root?.classList?.contains( 'vxt-toc__collapse' ) ) {
						$root?.classList?.remove( 'vxt-toc__collapse' );
						VexaltrixTableOfContents._slideDown( tocListWrapEl, 500, id, attr );
					} else {
						$root?.classList?.add( 'vxt-toc__collapse' );
						VexaltrixTableOfContents._slideUp( tocListWrapEl, 500 );
					}
				}
			}
		}

		document.addEventListener( 'scroll', VexaltrixTableOfContents._showHideScroll );
	},
	...
	hyperLinks() {
		...
		const node = document.querySelector( '.wp-block-vxt-table-of-contents' );
	...
	_showHideScroll() {
		scrollElement = document.querySelector( '.vxt-toc__scroll-top' );
		if ( null !== scrollElement ) {
			if ( window.scrollY > 300 ) {
				if ( scrolltoTop ) {
					scrollElement.classList.add( 'vxt-toc__show-scroll' );
				} else {
					scrollElement.classList.remove( 'vxt-toc__show-scroll' );
				}
			} else {
				scrollElement.classList.remove( 'vxt-toc__show-scroll' );
			}
	...
	mapTocAnchorsForHref( anchors ) {
		for ( const anchor of anchors ) {
			...
			const parsedHref = VexaltrixTableOfContents.parseTocSlug( href );
	...
	_run( attr, id ) {
		// Add setTime
		setTimeout( function () {
			VexaltrixTableOfContents._runWithTimeOut( attr, id );
		}, 500 );
	},
	_runWithTimeOut( attr, id ) {
		const $thisScope = VexaltrixTableOfContents.selectDomElement( id );
		...
		if ( $thisScope.querySelector( '.uag-toc__collapsible-wrap' ) !== null ) {
			if ( $thisScope.querySelector( '.uag-toc__collapsible-wrap' ).length > 0 ) {
				$thisScope.querySelector( '.vxt-toc__title-wrap' ).classList.add( 'vxt-toc__is-collapsible' );
			}
		}
		...
		if ( 0 !== allHeader.length ) {
			const tocListWrap = $thisScope.querySelector( '.vxt-toc__list-wrap' );
			if ( ! tocListWrap ) {
				return;
			}
			...
			VexaltrixTableOfContents.mapTocAnchorsForHref( aTags );
			...
			for ( let i = 0; i < divsArr.length; i++ ) {
				let headerText = VexaltrixTableOfContents.parseTocSlug( divsArr[ i ].innerText );
		...
		if ( scrollElement === null ) {
			...
			scrollToTopDiv.classList.add( 'vxt-toc__scroll-top' );
		...
		VexaltrixTableOfContents._showHideScroll();
		VexaltrixTableOfContents.hyperLinks();
		VexaltrixTableOfContents.init( id, attr );
	},
};
