VexaltrixCounter = {
	...
	init( mainSelector, data = {} ) {
		...
		const elements = document.querySelectorAll( `.wp-block-vxt-counter${ mainSelector }` );
	...
	_numberCount( data ) {
		const that = this;
		const el = this.elements.counterWrapper.querySelector( '.vxt-counter-block-number' );
	...
	_triggerBar( el, data ) {
		const that = this;
		const parentWrapClass = 'wp-block-vxt-counter--bars';
		const numberWrap = el.querySelector( '.wp-block-vxt-counter__number' );
	...
	_triggerCircle( el, data ) {
		...
		const circleWrap = el.querySelector(
			'.wp-block-vxt-counter-circle-container svg .vxt-counter-circle__progress'
		);
