document.addEventListener( 'UAGModalEditor', function ( e ) {
	VexaltrixModal.init( '.vxt-block-' + e.detail.block_id, true );
} );
document.addEventListener( 'AstraQuickViewForModal', function ( e ) {
	VexaltrixModal.init( e.detail.class_name, false );
} );
window.VexaltrixModal = {
	init( mainSelector, isAdmin ) {
		const document_element = VexaltrixModal._getDocumentElement();
		...
		if ( modalWrapper?.length ) {
			for ( const modalWrapperEl of modalWrapper ) {
				const modalTrigger = modalWrapperEl.querySelector( '.vxt-modal-trigger' );
				const closeOverlayClick = modalWrapperEl.dataset.overlayclick;
				if ( modalTrigger ) {
					modalTrigger.style.pointerEvents = 'auto';

					const innerModal = modalWrapperEl?.querySelector( '.vxt-modal-popup' );
					if ( ! innerModal ) {
						continue;
					}
					...
					if ( '.vxt-modal-wrapper' === mainSelector ) {
						// When we get mainSelector as a vxt-modal-wrapper from AstraQuickViewForModal event we get null for closeModal. So avoid this we need to use vxt-modal-popup as mainSelector.
						mainSelector = '.vxt-modal-popup';
					}
					const closeModal = innerModal.querySelector( `${ mainSelector } .vxt-modal-popup-close` );
					if ( closeModal ) {
						closeModal.addEventListener( 'click', function () {
							...
							if ( bodyWrap.classList.contains( 'hide-scroll' ) ) {
								VexaltrixModal.closeModalScrollCheck( bodyWrap, document_element );
							}
						} );
					}
					...
					if ( 'disable' !== closeOverlayClick ) {
						innerModal.addEventListener( 'click', function ( e ) {
							...
							if (
								...
								! innerModal.querySelector( '.vxt-modal-popup-wrap' ).contains( e.target )
							) {
								innerModal.classList.remove( 'active' );
							}
							if ( bodyWrap.classList.contains( 'hide-scroll' ) ) {
	...
	closeModalScrollCheck( bodyWrapper, document_element ) {
		const allActiveModals = document_element.querySelectorAll( '.vxt-modal-popup.active' );
