...
VexaltrixInlineNotice = {
	init( attr, id ) {
		...
		const currentCookie = Cookies.get( 'vxt-notice-' + uniqueId );
		...
		for ( const mainWrap of main ) {
			...
			const noticeDismissClass =
				mainWrap.querySelector( '.vxt-notice-dismiss' ) || mainWrap.querySelector( 'svg' );
			const closeBtn = noticeDismissClass
				? noticeDismissClass
				: mainWrap.querySelector( 'button[type="button"] svg' );
...
function dismissClick( isCookie, currentCookie, uniqueId, cookiesDays, main, focusedVisibleElement ) {
	if ( true === isCookie && 'undefined' === typeof currentCookie ) {
		Cookies.set( 'vxt-notice-' + uniqueId, true, { expires: cookiesDays } );
	}
	main[ 0 ]?.classList?.add( 'vxt-notice__active' );
