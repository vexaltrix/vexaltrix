// Global vxt_ultimate_gutenberg_blocks_countdown_data.
VexaltrixCountdown = {
	elements: {},
	countdownInterval: {},
	cookie_slug: vxt_ultimate_gutenberg_blocks_countdown_data.site_name_slug,
	cache: {},
	...
	init( mainSelector, data = {} ) {
		...
		if ( ! window?.VexaltrixCountdownTimeSignal ) {
			window.VexaltrixCountdownTimeSignal = {};
		}

		// Set flag variable to false, till it's overtime.
		window.VexaltrixCountdownTimeSignal[ mainSelector ] = false;
	...
	updateCountdown( mainSelector, data, ref = null ) {
		...
		let hoursWrap;
		let minutesWrap;
		const secondsWrap = ref.querySelector( '.wp-block-vxt-countdown__time-seconds' );

		if ( data?.showDays ) {
			daysWrap = ref.querySelector( '.wp-block-vxt-countdown__time-days' );
		}

		if ( data?.showHours ) {
			hoursWrap = ref.querySelector( '.wp-block-vxt-countdown__time-hours' );
		}

		if ( data?.showMinutes ) {
			minutesWrap = ref.querySelector( '.wp-block-vxt-countdown__time-minutes' );
		}
		...
		if ( isOvertime ) {
			...
			if ( ( 'redirect' === data?.timerEndAction || 'hide' === data?.timerEndAction ) && data?.isFrontend ) {
				window.VexaltrixCountdownTimeSignal[ mainSelector ] = true;
			}
		}

		// Check if the page is reloaded by checking the presence of a specific cookie
		const reloadCookieName = 'vexaltrixPageReloaded_' + mainSelector;
		...
		if ( data?.isFrontend ) {
			...
			if ( isOvertime && 'content' === data?.timerEndAction && ! data?.reloadOnExpire && ! data?.autoReload ) {
				...
				const countdownSelectors = timeUnits.map( ( unit ) =>
					ref.querySelector( `.wp-block-vxt-countdown__box-${ unit }` )
				);
				const innerBlocksWrapper = ref.querySelector( `.vxt-block-countdown-innerblocks-${ data.block_id }` );
