...
/**
 * Class \Vexaltrix\Assets\PostAssets.
 */
class PostAssets {
	...
	public function __construct( $postId ) {
		...
		if ( $this->preview || $this->isPostRevision ) {
			...
			$this->fileGeneration              = \Vexaltrix\Support\Helper::$fileGeneration;
			$this->isAllowedAssetsGeneration = $this->allowAssetsGeneration();
		}
		// Set other options.
		$this->loadGfontsLocally = \Vexaltrix\Admin\AdminSettings::get( 'uag_loadGfontsLocally', 'disabled' );
		$this->preloadLocalFonts = \Vexaltrix\Admin\AdminSettings::get( 'uag_preloadLocalFonts', 'disabled' );
			...
			if ( is_array( $style ) ) {
				foreach ( $style as $family ) {
					if ( ! in_array( $family, $families, true ) ) {
						\Vexaltrix\Support\Helper::blocksGoogleFont( true, $family, '' );
						$families[] = $family;
					}
				}
			}
	...
	public function allowAssetsGeneration() {
		...
		if ( 'enabled' === $this->fileGeneration ) {
			...
			if ( ! empty( $cssFileName ) ) {
				$cssAssetInfo = \Vexaltrix\Support\ScriptsUtils::getAssetInfo( 'css', $this->postId );
				$cssFilePath  = $cssAssetInfo['css'];
			}

			if ( ! empty( $jsFileName ) ) {
				$jsAssetInfo = \Vexaltrix\Support\ScriptsUtils::getAssetInfo( 'js', $this->postId );
	...
	public function enqueueScripts() {
		...
		$blocks = [];
		if ( \Vexaltrix\Admin\AdminSettings::isBlockTheme() ) {
		...
		if ( $this->uagFlag ) {
			...
			if ( 'disabled' === $this->fileGeneration || $this->fallbackCss ) {
				\Vexaltrix\Support\ScriptsUtils::enqueueBlocksStyles(); // Enqueue block styles.
	...
	public function printConditionalCss() {
		...
		$conditionalBlockCss = \Vexaltrix\Core\Blocks\BlockHelper::getConditionBlockCss();

		if ( in_array( 'vexaltrix/masonry-gallery', $this->currentBlockList, true ) ) {
			$conditionalBlockCss .= \Vexaltrix\Core\Blocks\BlockHelper::getMasonryGalleryCss();
		}
	...
	public function getBlockCssAndJs( $block ) {
		...
		if ( 'core/gallery' === $name && isset( $block['attrs']['masonry'] ) && true === $block['attrs']['masonry'] ) {
			...
			$this->uagFlag             = true;
			$css                       += \Vexaltrix\Core\Blocks\BlockHelper::getGalleryCss( $blockattr, $blockId );
		}
		...
		if ( strpos( $name, 'vexaltrix/' ) !== false ) {
			...
			$blockCss = \Vexaltrix\Core\Blocks\BlockModule::getFrontendCss( $blockSlug, $blockattr, $blockId );
			$blockJs  = \Vexaltrix\Core\Blocks\BlockModule::getFrontendJs( $blockSlug, $blockattr, $blockId, 'js' );
	...
	public function commonFunctionForAssetsPreparation( $postContent ) {
		...
		if ( 'yes' === $enableOnPageCssButton ) {
			...
			if ( ! empty( $customCss ) && ! self::$customCssAppended ) {
				$this->stylesheet         .= \Vexaltrix\Admin\AdminSettings::sanitizeInlineCss( $customCss );
		...
		// Update fonts.
		$this->gfonts = array_merge( $this->gfonts, \Vexaltrix\Support\Helper::$gfonts );
	}
	...
	public function createFile( $fileData, $type, $fileState = 'new', $oldFileName = '' ) {
		...
		$folderName    = \Vexaltrix\Support\ScriptsUtils::getAssetFolderName( $this->postId );
	...
	public function fileWrite( $fileData, $type = 'css', $postId = 0 ) {
		...
		$assetsInfo = \Vexaltrix\Support\ScriptsUtils::getAssetInfo( $type, $this->postId );
		...
		if ( '' === $fileName ) {
			...
			if ( $didCreate ) {
				$newAssetsInfo           = \Vexaltrix\Support\ScriptsUtils::getAssetInfo( $type, $this->postId );
