...
/**
 * Class \Vexaltrix\Assets\FrontAssets.
 */
class FrontAssets {
	...
	public function enqueueAssetFiles() {
		...
		if ( is_archive() || is_home() || is_search() || is_404() ) {
			...
			if ( 0 !== $currentObjectId && null !== $currentObjectId ) {
				$currentPostAssets = new \Vexaltrix\Assets\PostAssets( $currentObjectId );
				...
				foreach ( $cachedWpQuery as $post ) {
					$currentPostAssets = new \Vexaltrix\Assets\PostAssets( $post->ID );
					$currentPostAssets->enqueueScripts();
				}
			} else {
				/*
		...
		if ( class_exists( 'WooCommerce' ) ) {
			...
			if ( ! empty( $id ) ) {
				$currentPostAssets = new \Vexaltrix\Assets\PostAssets( intval( $id ) );
				$currentPostAssets->enqueueScripts();
			}
		}
...
/**
 *  Prepare if class 'Vexaltrix\\Assets\\FrontAssets' exist.
...
function vxtUltimateGutenbergBlocksGetFrontPostAssets() {
	return \Vexaltrix\Assets\FrontAssets::getInstance()->getPostAssetsObj();
}
