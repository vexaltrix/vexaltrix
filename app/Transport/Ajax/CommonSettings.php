...
class CommonSettings extends \Vexaltrix\Core\Base\AjaxController {
	...
	private function saveAdminSettings( $option, $value ) {
		\Vexaltrix\Presentation\Admin\AdminSettings::updateAdminSettingsOption( $option, $value );
	...
	public function fseFontGloballyDelete() {
		...
		\Vexaltrix\Integration\Compatibility\FseFontsCompatibility::deleteThemeFontFamily( $value );
	}
	...
	public function fseFontGlobally() {
		...
		$vexaltrixGlobalFseFonts = \Vexaltrix\Presentation\Admin\AdminSettings::get( 'vexaltrix_global_fse_fonts', [] );
	...
	public function blocksActivationAndDeactivation() {
		...
		if ( 'disabled' === \Vexaltrix\Core\Support\Helper::$fileGeneration ) {
			\Vexaltrix\Presentation\Admin\AdminSettings::createSpecificStylesheet(); // Get Specific Stylesheet.
		}
		if ( '' !== $status ) {
			...
			foreach ( $changeExtension as $key => $val ) {
				if ( '_vxt_ultimate_gutenberg_blocks_blocks' === $key ) {
					\Vexaltrix\Presentation\Admin\AdminSettings::updateAdminSettingsOption( '_vxt_ultimate_gutenberg_blocks_blocks', $val );
					continue;
				}
				// Update all extensions.
				\Vexaltrix\Presentation\Admin\AdminSettings::updateAdminSettingsOption( $key, $val );
			}
		} else {
	...
	public function deleteAllAssets() {
		...
		if ( ! empty( $value ) ) {
			...
			$fileGeneration = \Vexaltrix\Core\Support\Helper::allowFileGeneration();

			if ( 'enabled' === $fileGeneration ) {

				\Vexaltrix\Core\Support\Helper::deleteUagAssetDir();
			}
	...
	public function saveGbsDefaultInUploadFolder( $value ) {
		...
		foreach ( $createBlockArray as $blockName => $index ) {
			...
			if ( 'enabled' === $value ) {
				\Vexaltrix\Core\Support\Helper::removeFile( $pathAndFileName );
				continue;
			}
			...
			$blockCss = \Vexaltrix\Presentation\Blocks\BlockModule::getFrontendCss( $blockSlug, $dummyAttr, '', true );
