...
if ( ! class_exists( 'Vexaltrix\Domain\Analytics\\AnalyticsEventTracker' ) ) {

	/**
	 * Class \Vexaltrix\Domain\Analytics\AnalyticsEventTracker
	 *
	 * @since 2.19.22
	 */
	class AnalyticsEventTracker implements ServiceInterface {

		/**
		 * Instance.
		 *
		 * @var \Vexaltrix\Domain\Analytics\AnalyticsEventTracker|null
		 */
		 ...
		 *
		 * @since 2.19.22
		 * @return \Vexaltrix\Domain\Analytics\AnalyticsEventTracker
		 */
		...
		private function __construct() {
			class_exists( 'Vexaltrix\Domain\Analytics\\AnalyticsEvents' );
		...
		public function trackPluginActivated() {
			...
			\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'plugin_activated', VXT_VER, $properties );
		}
		...
		public function trackAnalyticsOptin( $oldValue, $newValue ) {
			if ( 'yes' === $newValue ) {
				\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'analytics_optin', 'yes' );
			}
		}
		...
		public function trackFirstVexaltrixBlockUsed( $postId, $post ) {
			...
			if ( ! \Vexaltrix\Domain\Analytics\AnalyticsEvents::isTracked( 'first_vexaltrix_block_used' ) ) {
				\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'first_vexaltrix_block_used', $blockSlug, $properties );
			}
			...
			if ( in_array( $editorContext, [ 'fse', 'fse_part' ], true )
				&& ! \Vexaltrix\Domain\Analytics\AnalyticsEvents::isTracked( 'first_fse_block_used' ) ) {
				\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'first_fse_block_used', $blockSlug, $properties );
			}
		}
		...
		public function trackPluginUpdated() {
			...
			\Vexaltrix\Domain\Analytics\AnalyticsEvents::retrackEvent( 'plugin_updated', VXT_VER, $properties );
		...
		public function trackPluginDeactivated( $plugin ) {
			...
			\Vexaltrix\Domain\Analytics\AnalyticsEvents::retrackEvent( 'plugin_deactivated', VXT_VER, $properties );
		}
		...
		public function trackSettingChanged( $oldValue, $newValue, $option ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
			...
			\Vexaltrix\Domain\Analytics\AnalyticsEvents::retrackEvent(
				'settings_changed',
				sanitize_key( $option ),
				[
		...
		private function detectVexaltrixProActivated() {
			if ( \Vexaltrix\Domain\Analytics\AnalyticsEvents::isTracked( 'vexaltrix_pro_activated' ) ) {
				return;
			}
			...
			if ( is_plugin_active( 'vexaltrix-pro/vexaltrix-pro.php' ) ) {
				...
				\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'vexaltrix_pro_activated', $proVersion );
			}
		}
		...
		private function detectAiAssistantFirstUse() {
			if ( \Vexaltrix\Domain\Analytics\AnalyticsEvents::isTracked( 'ai_assistant_first_use' ) ) {
				return;
			}
			...
			if ( \ZipAI\Classes\Helper::is_authorized() ) {
				\Vexaltrix\Domain\Analytics\AnalyticsEvents::track(
		...
		private function detectGbsFirstCreated() {
			if ( \Vexaltrix\Domain\Analytics\AnalyticsEvents::isTracked( 'gbs_first_created' ) ) {
				return;
			}
			...
			if ( ! empty( $gbsStored ) && is_array( $gbsStored ) ) {
				\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'gbs_first_created' );
				return;
			}
		...
		private function detectOnboardingCompleted() {
			if ( \Vexaltrix\Domain\Analytics\AnalyticsEvents::isTracked( 'onboarding_completed' ) ) {
				return;
			}
			...
			\Vexaltrix\Domain\Analytics\AnalyticsEvents::clearEvent( 'onboarding_skipped' );

			\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'onboarding_completed', '', $properties );
		}
		...
		public function trackFirstTemplateImported() {
			\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'first_template_imported' );
		}
		 ...
		 * by `\Vexaltrix\Domain\Analytics\AnalyticsEvents::track()`, so repeat fires are no-ops.
		 *
		 * @since 2.19.25
		 * @return void
		 */
		public function trackFirstPatternImported() {
			\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'first_pattern_imported' );
		}
		...
		public function trackDesignLibraryOpened() {
			...
			\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'first_design_library_opened' );
		...
		public function enqueueDesignLibraryOpenListener() {
			...
			if ( \Vexaltrix\Domain\Analytics\AnalyticsEvents::isTracked( 'first_design_library_opened' ) ) {
				return;
			}
		...
		private function detectFirstFormCreated() {
			if ( \Vexaltrix\Domain\Analytics\AnalyticsEvents::isTracked( 'first_form_created' ) ) {
				return;
			}

			$blockStats = \Vexaltrix\Domain\Analytics\BlockStatsProcessor::getBlockStats();

			if ( ! empty( $blockStats['vexaltrix/forms'] ) && $blockStats['vexaltrix/forms'] > 0 ) {
				\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'first_form_created' );
			}
		}
		...
		private function detectFirstPopupCreated() {
			if ( \Vexaltrix\Domain\Analytics\AnalyticsEvents::isTracked( 'first_popup_created' ) ) {
				return;
			}
			...
			if ( is_object( $popupCount ) && ( $popupCount->publish > 0 || $popupCount->draft > 0 ) ) {
				\Vexaltrix\Domain\Analytics\AnalyticsEvents::track( 'first_popup_created' );
			}
		}
		...
		public function trackOnboardingCompleted( $completionData, $request = null ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
			...
			\Vexaltrix\Domain\Analytics\AnalyticsEvents::retrackEvent( 'onboarding_completed', VXT_VER, $properties );
		}
		...
		public function trackOnboardingSkipped( $stateData, $request = null ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
			...
			\Vexaltrix\Domain\Analytics\AnalyticsEvents::retrackEvent( 'onboarding_skipped', VXT_VER, $properties );
		...
		public function trackLearnChapterProgress( $savedProgress ) {
			if ( empty( $savedProgress ) || ! class_exists( 'Vexaltrix\Presentation\Admin\\AdminLearn' ) ) {
				return;
			}

			$chapters = \Vexaltrix\Presentation\Admin\AdminLearn::getChaptersStructure();
			if ( empty( $chapters ) ) {
				return;
			}
			...
			\Vexaltrix\Domain\Analytics\AnalyticsEvents::retrackEvent( 'learn_chapter_progress', $eventValue, $properties );
		}
