...
if ( ! class_exists( 'Vexaltrix\\Core\\Blocks\\BlockModule' ) ) {
				...
				if ( is_string( $assetsFile ) && file_exists( $assetsFile ) ) {
					...
					if ( is_string( $attrFile ) && file_exists( $attrFile ) ) {
						...
						if ( ! empty( $attr['globalBlockStyleId'] ) && $isGbs ) {
							$gbsClass = \Vexaltrix\Support\Helper::getGbsSelector( $attr['globalBlockStyleId'] );
						}
					}
...
/**
 *  Prepare if class 'Vexaltrix\\Core\\Blocks\\BlockModule' exist.
