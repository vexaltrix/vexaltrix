...
if ( ! class_exists( 'Vexaltrix\\Core\\Analytics\\BlockAnalytics' ) ) {

	/**
	 * Class \Vexaltrix\Core\Analytics\BlockAnalytics
	 *
	...
	class BlockAnalytics {
		 ...
		 * Member Variable
		 *
		 * @var \Vexaltrix\Core\Analytics\BlockAnalytics|null
		 * @since 2.19.13
		 */
		 ...
		 *
		 * @var \Vexaltrix\Core\Analytics\BlockStatsProcessor
		 * @since 2.19.13
		 */
		 ...
		 *
		 * @var \Vexaltrix\Core\Analytics\IncrementalBlockTracker
		 * @since 2.19.13
		 */
		 ...
		 *
		 * @since 2.19.13
		 * @return \Vexaltrix\Core\Analytics\BlockAnalytics
		 */
		...
		public function __construct() {
			...
			class_exists( 'Vexaltrix\\Core\\Analytics\\BlockStatsProcessor' );
			class_exists( 'Vexaltrix\\Core\\Analytics\\IncrementalBlockTracker' );
			class_exists( 'Vexaltrix\\Core\\Analytics\\DailyKpiCounters' );

			$this->statsProcessor     = new \Vexaltrix\Core\Analytics\BlockStatsProcessor();
			$this->incrementalTracker = \Vexaltrix\Core\Analytics\IncrementalBlockTracker::getInstance();
...
// Hook into analytics option changes.
		...
		public function getBlockStatsForAnalytics( $existingStats = [] ) {
			...
			$stats               = \Vexaltrix\Core\Analytics\BlockStatsProcessor::getBlockStats();
			$collectionComplete = \Vexaltrix\Core\Analytics\BlockStatsProcessor::isCollectionComplete();
			$lastCollection     = \Vexaltrix\Core\Analytics\BlockStatsProcessor::getLastCollectionTime();
		 ...
		 * incrementally by {@see \Vexaltrix\Core\Analytics\IncrementalBlockTracker}. This is O(1)
		...
		public function getSiteActivityLevel() {
			...
			$activePagesCount = \Vexaltrix\Core\Analytics\DailyKpiCounters::getPagesWithVexaltrix();
		...
		private function seedPagesWithVexaltrixCounter() {
			...
			if ( empty( $postIds ) ) {
				update_option( \Vexaltrix\Core\Analytics\DailyKpiCounters::OPT_PAGES_WITH_vexaltrix, 0, false );
				return;
			}
			...
			update_option( \Vexaltrix\Core\Analytics\DailyKpiCounters::OPT_PAGES_WITH_vexaltrix, $pagesWithVexaltrix, false );
		}
	}
}
