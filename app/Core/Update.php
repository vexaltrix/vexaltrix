...
if ( ! class_exists( 'Vexaltrix\\Core\\Update' ) ) :
	...
	class Update {
		...
		public function init() {
			...
			if ( version_compare( $savedVersion, '2.0.5', '<' ) ) {
				\Vexaltrix\Admin\AdminSettings::updateAdminSettingsOption( 'uag_enable_legacy_blocks', 'yes' );
			}

			// If user is older than equal to 2.12.1 then set the option.
			if ( version_compare( $savedVersion, '2.12.1', '<=' ) ) {
				\Vexaltrix\Admin\AdminSettings::updateAdminSettingsOption( 'uag_enable_quick_action_sidebar', 'disabled' );
			}
			...
			if ( \Vexaltrix\Admin\AdminSettings::get( 'uag_enable_header_titlebar' ) ) {
				\Vexaltrix\Admin\AdminSettings::deleteAdminSettingsOption( 'uag_enable_header_titlebar' );
			}

			// Create a Core Block Array for all versions in which a Core Vexaltrix Block was added.
			$coreBlocks   = [];
			$blocksStatus = \Vexaltrix\Admin\AdminSettings::get( '_vxt_ultimate_gutenberg_blocks_blocks' );
			...
			$inheritFromTheme = \Vexaltrix\Admin\AdminSettings::get( 'uag_btn_inherit_from_theme' );
			...
			if ( version_compare( $savedVersion, '2.13.4', '<' ) && 'enabled' === $inheritFromTheme ) {
				\Vexaltrix\Admin\AdminSettings::updateAdminSettingsOption( 'uag_btn_inherit_from_theme_fallback', 'disabled' );
			}
			...
			if ( ! empty( $coreBlocks ) ) {
				...
				\Vexaltrix\Admin\AdminSettings::updateAdminSettingsOption( '_vxt_ultimate_gutenberg_blocks_blocks', $blocksStatus );
			}
		...
		public static function migrateVisibilityMode() {
			...
			$oldOption      = \Vexaltrix\Admin\AdminSettings::get( 'uag_enable_coming_soon_mode' );
			$oldOptionPage = \Vexaltrix\Admin\AdminSettings::get( 'uag_coming_soon_page' );
			...
			// Update the option.
			\Vexaltrix\Admin\AdminSettings::updateAdminSettingsOption( 'uag_visibility_mode', $oldOption ? $oldOption : 'disabled' );
			\Vexaltrix\Admin\AdminSettings::updateAdminSettingsOption( 'uag_visibility_page', $oldOptionPage ? $oldOptionPage : '' );

			// Delete the old option.
			\Vexaltrix\Admin\AdminSettings::deleteAdminSettingsOption( 'uag_enable_coming_soon_mode' );
			\Vexaltrix\Admin\AdminSettings::deleteAdminSettingsOption( 'uag_coming_soon_page' );
		}
		...
		public function freshInstallUpdateAssetGenerationOption() {
			...
			if ( \Vexaltrix\Support\Helper::isUagDirHasWritePermissions() ) {
