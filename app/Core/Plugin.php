	...
	final class Plugin {
		...
		public function vexaltrixGetSpecificStats( $defaultStats ) {
			$defaultStats['plugin_data']['vexaltrix'] = [
				...
				'migration_status'     => get_option( 'vxt_migration_status' ),
			];

			// Load global options values.
			$globalData = [
				'beta'                      => get_option( 'vxt_ultimate_gutenberg_blocks_beta' ),
				'enableLegacyBlocks'      => get_option( 'vxt_enable_legacy_blocks' ),
				'file_generation'           => get_option( '_vxt_ultimate_gutenberg_blocks_allow_file_generation' ),
				'templates_button'          => get_option( 'vxt_enable_templates_button' ),
				'on_page_css_button'        => get_option( 'vxt_enable_on_page_css_button' ),
				'block_condition'           => get_option( 'vxt_enable_block_condition' ),
				'quick_action_sidebar'      => get_option( 'vxt_enable_quick_action_sidebar' ),
				'gbs_extension'             => get_option( 'vxt_enable_gbs_extension' ),
				'block_responsive'          => get_option( 'vxt_enable_block_responsive' ),
				'loadSelectFontGlobally'     => get_option( 'vxt_load_select_font_globally' ),
				'loadGfontsLocally'           => get_option( 'vxt_load_gfonts_locally' ),
				'collapsePanels'               => get_option( 'vxt_collapse_panels' ),
				'copyPaste'                    => get_option( 'vxt_copy_paste' ),
				'preloadLocalFonts'           => get_option( 'vxt_preload_local_fonts' ),
				'visibilityMode'               => get_option( 'vxt_visibility_mode' ),
				'containerGlobalPadding'      => get_option( 'vxt_container_global_padding' ),
				'containerGlobalElementsGap' => get_option( 'vxt_container_global_elements_gap' ),
				'btnInheritFromTheme'        => get_option( 'vxt_btn_inherit_from_theme' ),
				'blocksEditorSpacing'         => get_option( 'vxt_blocks_editor_spacing' ),
				'loadFontAwesome5'           => get_option( 'vxt_load_font_awesome_5' ),
				'autoBlockRecovery'           => get_option( 'vxt_auto_block_recovery' ),
				'loadFseFontGlobally'        => get_option( 'vxt_load_fse_font_globally' ),
			];
