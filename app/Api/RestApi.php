...
if ( ! class_exists( 'Vexaltrix\\Api\\RestApi' ) ) {

	/**
	 * Class \Vexaltrix\Api\RestApi.
	 */
	final class RestApi {
		...
		public function deletePageAssetsOnTrash( $postId ) {
				...
				$cssAssetInfo = \Vexaltrix\Support\ScriptsUtils::getAssetInfo( 'css', $postId );
				$jsAssetInfo  = \Vexaltrix\Support\ScriptsUtils::getAssetInfo( 'js', $postId );
		...
		public function contentPreRender( $block ) {
			$tabStylingCss  = '';
			$mobStylingCss  = '';
			$postAssets = new \Vexaltrix\Assets\PostAssets( get_the_ID() );
		...
		public function blocksRegisterRestFields() {
			$postType = \Vexaltrix\Support\Helper::getPostTypes();
		...
		public function getQuickActionBarInitialStates() {
			...
			$dbValue                            = \Vexaltrix\Admin\AdminSettings::get( 'uag_enable_quick_action_sidebar' );
			$showEnable                         = ( empty( $dbValue ) ) ? 'enabled' : $dbValue;
			$vexaltrixEnableQuickActionSidebar = \Vexaltrix\Admin\AdminSettings::get( 'uag_enable_quick_action_sidebar', $showEnable );

			$vexaltrixDefaultAllowedQuickSidebarBlocks = \Vexaltrix\Admin\AdminSettings::get(
		...
		public function getImageSrc( $object, $fieldName, $request ) {
			$imageSizes = \Vexaltrix\Support\Helper::getImageSizes();
	...
	/**
	 *  Prepare if class 'Vexaltrix\\Api\\RestApi' exist.
