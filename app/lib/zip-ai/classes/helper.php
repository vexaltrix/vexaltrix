...
class Helper {
	...
	public static function get_credit_server_response( $endpoint, $body = [], $extra_args = [] ) {
		...
		if ( ! is_string( $endpoint ) ) {
			return array(
				'error' => __( 'The Zip AI Endpoint was not declared', 'vexaltrix' ),
			);
		}
		...
		if ( empty( $auth_token ) || ! is_string( $auth_token ) ) {
			return array(
				'error' => __( 'The Zip AI Auth Token is not set.', 'vexaltrix' ),
			);
		}
		...
		if ( is_wp_error( $response ) || empty( $response ) ) {
			$error_message = __( 'Empty response from API.', 'vexaltrix' );
			$error_code    = __( 'empty_response', 'vexaltrix' );
		...
		if ( 401 === $status_code || 403 === $status_code ) {
			$error_message = isset( $response_body['error'] ) ? $response_body['error'] : __( 'You do not have permission to perform this action.', 'vexaltrix' );
		...
		if ( 200 !== $status_code || empty( $response_body ) ) {
			$error_message = __( 'Encountered an error while processing your request. Please try again.', 'vexaltrix' );
	...
	public static function get_zipwp_api_response( $endpoint ) {
		...
		if ( ! is_string( $endpoint ) ) {
			return array(
				'error' => __( 'The ZipWP Endpoint was not declared', 'vexaltrix' ),
			);
		}
		...
		if ( empty( $zipwp_token ) || ! is_string( $zipwp_token ) ) {
			return array(
				'error' => __( 'The ZipWP Token is not set.', 'vexaltrix' ),
			);
		}
		...
		if ( is_wp_error( $response ) || empty( $response['response'] ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			return array(
				'error' => __( 'The ZipWP API server is not responding.', 'vexaltrix' ),
			);
		}
		...
		if ( empty( $response_body ) || ! json_decode( $response_body ) ) {
			return array(
				'error' => __( 'The ZipWP API server encountered an error.', 'vexaltrix' ),
			);
		}
