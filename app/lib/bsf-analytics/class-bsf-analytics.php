...
if ( ! class_exists( 'BSF_Analytics' ) ) {
	...
	class BSF_Analytics {
		 ...
		 *
		 * @param string $product_key Product key (e.g., 'vexaltrix', 'surerank').
		...
		public function option_notice() {
			if ( ! class_exists( 'VexaltrixAdminNotices' ) ) {
				return;
			}
			...
			foreach ( $this->entities as $key => $data ) {
				...
				$notice_string = sprintf(
					__(
						'<strong>Help shape the future of %1$s.</strong><br><br>Share how you use the plugin so we can build features that matter, fix issues faster, and make smarter decisions.', 'vexaltrix' ),
					esc_html( $data['product_name'] )
				);
				
				if ( is_multisite() ) {
					$notice_string .= __( 'This will be applicable for all sites from the network.', 'vexaltrix' );
				}
				...
				VexaltrixAdminNotices::add_notice(
					array(
						...
						'message'                    => sprintf(
							...
							sprintf( $notice_string . '<span dir="%1s"> <a href="%2s" target="_blank" rel="noreferrer noopener">%3s</a><span><br><br>', $language_dir, esc_url( $usage_doc_link ), __( 'Learn more.', 'vexaltrix' ) ),
							esc_url(
								add_query_arg(
									array(
		...
		public function register_usage_tracking_setting() {
			...
			foreach ( $this->entities as $key => $data ) {
				...
				add_settings_field(
					...
					__( 'Usage Tracking', 'vexaltrix' ),       // Field title.
		...
		public function render_settings_field_html( $args ) {
			...
			<label for="<?php echo esc_attr( $args['label_for'] ); ?>">
				...
				echo esc_html( sprintf( __( 'Help improve %s by sharing non-sensitive usage data — like PHP version and features used.', 'vexaltrix' ), $args['title'] ) );// phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText

				if ( is_multisite() ) {
					esc_html_e( ' This will be applicable for all sites from the network.', 'vexaltrix' );
				}
				?>
			</label>
			<?php
			echo wp_kses_post( sprintf( '<a href="%1s" target="_blank" rel="noreferrer noopener">%2s</a>', esc_url( $args['usage_doc_link'] ), __( 'Learn More.', 'vexaltrix' ) ) );
			?>
			</fieldset>
			<?php
		}
