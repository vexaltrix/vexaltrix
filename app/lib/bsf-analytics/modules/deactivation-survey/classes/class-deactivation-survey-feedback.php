	...
	class Deactivation_Survey_Feedback {
		...
		public static function show_feedback_form( array $args = array() ) {
			...
			$defaults = array(
				...
				'plugin_version'    => '',
				'popup_title'       => __( 'Quick Feedback', 'vexaltrix' ),
				'support_url'       => 'https://brainstormforce.com/contact/',
				'popup_reasons'     => self::get_default_reasons(),
				'popup_description' => __( 'If you have a moment, please share why you are deactivating the plugin.', 'vexaltrix' ),
				...
				<div class="uds-feedback-form--container">
					<div class="uds-form-header--wrapper">
						<div class="uds-form-title--icon-wrapper">
							<?php if ( ! empty( $args['popup_logo'] ) ) { ?>
								<img class="uds-icon" src="<?php echo esc_url( $args['popup_logo'] ); ?>" title="<?php echo esc_attr( $args['plugin_slug'] ); ?> <?php echo esc_attr( __( 'Icon', 'vexaltrix' ) ); ?>" />
							<?php } ?>
					...
					<div class="uds-form-body--content">
						...
						<?php foreach ( $args['popup_reasons'] as $key => $value ) {
							...
							<fieldset>
								<textarea class="uds-options-feedback hide" id="uds-options-feedback" rows="3" name="uds_options_feedback" placeholder="<?php echo esc_attr( __( 'Please tell us more details.', 'vexaltrix' ) ); ?>"></textarea>
								<?php
										...
										<p class="uds-option-feedback-cta hide">
											...
											echo wp_kses_post(
												...
												/* translators: %1$s: link html start, %2$s: link html end*/
													__( 'Need help from our experts? %1$sClick here to contact us.%2$s', 'vexaltrix' ),
							...
							<div class="uds-feedback-form-sumbit--actions">
								<button class="button button-primary uds-feedback-submit" data-action="submit"><?php esc_html_e( 'Submit & Deactivate', 'vexaltrix' ); ?></button>
								<button class="button button-secondary uds-feedback-skip" data-action="skip"><?php esc_html_e( 'Skip & Deactivate', 'vexaltrix' ); ?></button>
		...
		public function send_plugin_deactivate_feedback() {
			...
			$response_data = array( 'message' => __( 'Sorry, you are not allowed to do this operation.', 'vexaltrix' ) );
			...
			if ( ! check_ajax_referer( 'uds_plugin_deactivate_feedback', 'security', false ) ) {
				$response_data = array( 'message' => __( 'Nonce validation failed', 'vexaltrix' ) );
				wp_send_json_error( $response_data );
			}
			...
			return apply_filters(
				...
				array(
					'temporary_deactivation' => array(
						'label'           => esc_html__( 'This is a temporary deactivation for testing.', 'vexaltrix' ),
						'placeholder'     => esc_html__( 'How can we assist you?', 'vexaltrix' ),
						'show_cta'        => 'false',
						'accept_feedback' => 'false',
					),
					'plugin_not_working'     => array(
						'label'           => esc_html__( 'The plugin isn\'t working properly.', 'vexaltrix' ),
						'placeholder'     => esc_html__( 'Please tell us more about what went wrong?', 'vexaltrix' ),
						'show_cta'        => 'true',
						'accept_feedback' => 'true',
					),
					'found_better_plugin'    => array(
						'label'           => esc_html__( 'I found a better alternative plugin.', 'vexaltrix' ),
						'placeholder'     => esc_html__( 'Could you please specify which plugin?', 'vexaltrix' ),
						'show_cta'        => 'false',
						'accept_feedback' => 'true',
					),
					'missing_a_feature'      => array(
						'label'           => esc_html__( 'It\'s missing a specific feature.', 'vexaltrix' ),
						'placeholder'     => esc_html__( 'Please tell us more about the feature.', 'vexaltrix' ),
						'show_cta'        => 'false',
						'accept_feedback' => 'true',
					),
					'other'                  => array(
						'label'           => esc_html__( 'Other', 'vexaltrix' ),
						'placeholder'     => esc_html__( 'Please tell us more details.', 'vexaltrix' ),
						'show_cta'        => 'false',
						'accept_feedback' => 'true',
					),
				)
