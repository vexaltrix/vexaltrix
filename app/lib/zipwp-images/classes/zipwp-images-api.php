...
class Zipwp_Images_Api {
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			return new \WP_Error(
				...
				__( 'Sorry, you are not allowed to do that.', 'vexaltrix' ),
	...
	public function get_images( $request ) {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( (string) $nonce ), 'wp_rest' ) ) {
			wp_send_json_error(
				array(
					'data'   => __( 'Nonce verification failed.', 'vexaltrix' ),
					'status' => false,
	...
	public function zipwp_insert_image(): void {
		...
		if ( 0 === $photo_id ) {
			wp_send_json_error( __( 'Need to send photo ID', 'vexaltrix' ) );
		}

		if ( empty( $url ) ) {
			wp_send_json_error( __( 'Need to send URL of the image to be downloaded', 'vexaltrix' ) );
		}
		...
		if ( empty( $image ) ) {
			wp_send_json_error( __( 'Could not download the image.', 'vexaltrix' ) );
		}
