...
if ( ! class_exists( 'VexaltrixAdminNotices' ) ) :
	...
	class VexaltrixAdminNotices {
		...
		public function dismiss_notice() {
			...
			if ( ! $has_cap || ! $cap_check ) {
				wp_send_json_error( esc_html__( 'Permission denied.', 'vexaltrix' ) );
			}
			...
			if ( 0 !== strpos( $notice_id, 'astra-notices-id-' ) && ( ! in_array( $notice_id, $allowed_notices, true ) ) ) {
				wp_send_json_error( esc_html__( 'Invalid notice ID.', 'vexaltrix' ) );
			}
		...
		public function show_notices() {
			$defaults = array(
				...
				'display-with-other-notices' => true,    // Should the notice be displayed if other notices  are being displayed from VexaltrixAdminNotices.
				'is_dismissible'             => true,
		...
		public static function markup( $notice = array() ) {
			...
			<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="<?php echo esc_attr( 'astra-notice-wrapper ' . $notice['classes'] ); ?>" data-repeat-notice-after="<?php echo esc_attr( $notice['repeat-notice-after'] ); ?>">
				<div class="vxt-notice-container">
	 ...
	 */
	VexaltrixAdminNotices::get_instance();
...
if ( ! class_exists( 'Astra_Notices' ) ) {
	class_alias( 'VexaltrixAdminNotices', 'Astra_Notices' ); // phpcs:ignore PHPCompatibility.FunctionUse.NewFunctions.class_aliasFound
}
