...
if ( ! class_exists( 'VXT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class VXT_WebFont_Loader {
		...
		public function __construct( $url = '' ) {
			...
			$this->base_path = VXT_UPLOAD_DIR . 'assets/';

			$this->base_url = VXT_UPLOAD_URL . 'assets/';

			// Add a cleanup routine.
			$this->schedule_cleanup();
			add_action( 'vxt_ultimate_gutenberg_blocks_delete_fonts_folder', array( $this, 'delete_fonts_folder' ) );
		}
		...
		public function get_local_files_from_css() {
			...
			$stored         = get_site_option( 'vxt_ultimate_gutenberg_blocks_downloaded_font_files', array() );
			$stored_preload = get_site_option( 'vxt_ultimate_gutenberg_blocks_preload_font_files', array() );
			...
			if ( $change ) {
				...
				update_site_option( 'vxt_ultimate_gutenberg_blocks_downloaded_font_files', $stored );
				update_site_option( 'vxt_ultimate_gutenberg_blocks_preload_font_files', $stored_preload );
			}
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'vxt_ultimate_gutenberg_blocks_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'vxt_ultimate_gutenberg_blocks_get_local_fonts_base_url', content_url() . '/' );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'vxt_ultimate_gutenberg_blocks_get_local_fonts_subfolder_name', 'fonts' );
			}
		...
		public function schedule_cleanup() {
			if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
				if ( ! wp_next_scheduled( 'vxt_ultimate_gutenberg_blocks_delete_fonts_folder' ) && ! wp_installing() ) {
					wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'vxt_ultimate_gutenberg_blocks_delete_fonts_folder' );
				}
			}
		}
		...
		public function get_preload_local_fonts() {
			...
			// Get an array of locally-hosted files.
			$local_fonts = get_site_option( 'vxt_ultimate_gutenberg_blocks_preload_font_files', array() );
...
if ( ! function_exists( 'vxt_ultimate_gutenberg_blocks_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function vxt_ultimate_gutenberg_blocks_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new VXT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'vxt_ultimate_gutenberg_blocks_get_webfont_remote_styles' ) ) {
	/**
	 * Get styles for a webfont.
	 *
	 * This will get the CSS from the remote API,
	 ...
	 * @return string Returns the CSS.
	 */
	function vxt_ultimate_gutenberg_blocks_get_webfont_remote_styles( $url, $format = 'woff2' ) {
		$font = new VXT_WebFont_Loader( $url );
		$font->set_font_format( $format );
...
if ( ! function_exists( 'vxt_ultimate_gutenberg_blocks_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function vxt_ultimate_gutenberg_blocks_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new VXT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
...
if ( ! function_exists( 'vxt_ultimate_gutenberg_blocks_get_preload_local_fonts' ) ) {
	/**
	 * Get a stylesheet URL for a webfont.
	 *
	 * @since 1.1.0
	 ...
	 * @return string Returns the CSS.
	 */
	function vxt_ultimate_gutenberg_blocks_get_preload_local_fonts( $url, $format = 'woff2' ) {
		$font = new VXT_WebFont_Loader( $url );
		$font->set_font_format( $format );
