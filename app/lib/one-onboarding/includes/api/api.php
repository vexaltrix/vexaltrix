	...
	class Api {
		...
		public function get_plugin_status( \WP_REST_Request $request ) {
			...
			if ( empty( $plugin_slug ) ) {
				return new \WP_Error(
					'missing_plugin',
					__( 'Plugin slug is required', 'vexaltrix' ),
					array( 'status' => 400 )
				);
			}
		...
		public function install_plugin( \WP_REST_Request $request ) {
			...
			if ( $plugin_file ) {
				return rest_ensure_response(
					array(
						'success'     => true,
						'message'     => __( 'Plugin is already installed', 'vexaltrix' ),
						'plugin_file' => $plugin_file,
					)
				);
			}
			...
			if ( is_wp_error( $api ) ) {
				return new \WP_Error(
					...
					sprintf( __( 'Plugin %s not found in repository', 'vexaltrix' ), $plugin_slug ),
					array( 'status' => 404 )
				);
			}
			...
			return rest_ensure_response(
				array(
					'success'     => true,
					'message'     => __( 'Plugin installed successfully', 'vexaltrix' ),
					'plugin_file' => $plugin_file,
				)
			);
		}
		...
		public function activate_plugin( \WP_REST_Request $request ) {
			...
			if ( ! $plugin_file ) {
				return new \WP_Error(
					...
					__( 'Plugin is not installed', 'vexaltrix' ),
					array( 'status' => 404 )
				);
			}
			...
			if ( is_plugin_active( $plugin_file ) ) {
				return rest_ensure_response(
					array(
						'success' => true,
						'message' => __( 'Plugin is already active', 'vexaltrix' ),
					)
				);
			}
			...
			return rest_ensure_response(
				array(
					'success' => true,
					'message' => __( 'Plugin activated successfully', 'vexaltrix' ),
				)
			);
		}
		...
		public function save_onboarding_state( \WP_REST_Request $request ) {
			...
			if ( empty( $onboarding_state ) ) {
				return new \WP_Error(
					...
					__( 'Onboarding state is required', 'vexaltrix' ),
					array( 'status' => 400 )
				);
			}
