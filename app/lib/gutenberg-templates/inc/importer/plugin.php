...
class Plugin {
	...
	public function api_request() {
		...
		if ( ! current_user_can( 'manage_ast_block_templates' ) ) {
			wp_send_json_error( __( 'You are not allowed to perform this action', 'vexaltrix' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			wp_send_json_error( __( 'Something went wrong', 'vexaltrix' ) );
		}
		...
		foreach ( $blocks as $i => &$block ) {
			...
			if ( is_array( $block ) ) {
				...
				switch ( $block['blockName'] ) {
					case 'vexaltrix/container':
						$block = BlockEditor::instance()->parse_vexaltrix_container( $block );
						break;

					case 'vexaltrix/image':
						$block = BlockEditor::instance()->parse_vexaltrix_image( $block );
						break;

					case 'vexaltrix/image-gallery':
						$block = BlockEditor::instance()->parse_vexaltrix_gallery( $block );
						break;

					case 'vexaltrix/info-box':
						$block = BlockEditor::instance()->parse_vexaltrix_infobox( $block );
						break;

					case 'vexaltrix/google-map':
						$block = BlockEditor::instance()->parse_vexaltrix_google_map( $block );
						break;

					case 'vexaltrix/forms':
						$block = BlockEditor::instance()->parse_vexaltrix_form( $block );
						break;

					case 'vexaltrix/icon-list':
						$block = BlockEditor::instance()->parse_vexaltrix_social_icons( $block );
						break;
				}
	...
	public function template_importer() {
		...
		// Early return.
		if ( '' == $api_uri ) {
			wp_send_json_error( __( 'Something wrong', 'vexaltrix' ) );
		}
	...
	public function template_assets() {
		...
		wp_enqueue_script( 'ast-block-templates', VXT_BLOCK_TEMPLATES_URI . 'dist/main.js', array( 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor', 'masonry', 'imagesloaded', 'updates', 'media-upload', 'wp-util' ), AST_BLOCK_TEMPLATES_VER, true );
		wp_add_inline_script( 'ast-block-templates', 'window.lodash = _.noConflict();', 'after' );
		wp_enqueue_media();

		wp_enqueue_style( 'ast-block-templates', VXT_BLOCK_TEMPLATES_URI . 'dist/style.css', array(), AST_BLOCK_TEMPLATES_VER, 'all' );

		wp_enqueue_script(
			'ast-block-templates-plugin',
			VXT_BLOCK_TEMPLATES_URI . 'admin-assets/js/plugin.js',
		...
		wp_localize_script(
			...
			apply_filters(
				...
				array(
					'popup_class'             => defined( 'VXT_PLUGIN_SHORT_NAME' ) ? 'uag-block-templates-lightbox' : 'ast-block-templates-lightbox',
					'ajax_url'                => admin_url( 'admin-ajax.php' ),
					'uri'                     => VXT_BLOCK_TEMPLATES_URI,
					'wpforms_status'          => $this->get_plugin_status( 'wpforms-lite/wpforms.php' ),
					'vexaltrix_status'          => $this->get_plugin_status( 'vexaltrix/vexaltrix.php' ),
					'vexaltrix_pro_status'      => $this->get_plugin_status( 'vexaltrix-pro/vexaltrix-pro.php' ),
					...
					'button_text'             => esc_html__( 'Design Library', 'vexaltrix' ),
					'display_button_logo'     => true,
					'popup_logo_uri'          => VXT_BLOCK_TEMPLATES_URI . 'dist/vexaltrix-logo.svg',
					'button_logo'             => VXT_BLOCK_TEMPLATES_URI . 'dist/vexaltrix.svg',
					'st_button_logo'           => VXT_BLOCK_TEMPLATES_URI . 'dist/st.svg',
					'button_class'            => '',
					...
					'vexaltrix_common_styles'   => preg_replace( '/(?<!-)(\\bbody\\b)(?!-)/i', '.st-block-container', $common_css_content ) . ' .st-block-container .vxt-button__wrapper a { text-decoration: none; }',
					...
					'skip_zip_ai_onboarding_nonce'             => wp_create_nonce( 'skip-vexaltrix-pro-onboarding-nonce' ),
					...
					'images' => VXT_BLOCK_TEMPLATES_URI . 'admin-assets/images/',
	...
	/**
	 * Regenerate Vexaltrix CSS.
	 *
	 * @since 2.0.0
	 *
	 * @return void
	 */
	public function regenerate_vexaltrix_css() {

		if ( ! defined( 'VXT_FILE' ) && ! class_exists( 'VXT_Helper' ) ) {
			return;
		}
	
		if ( class_exists( 'VXT_Helper' ) && is_callable( array( 'VXT_Helper', 'allow_file_generation' ) ) ) {
			$file_generation = \VXT_Helper::allow_file_generation();
		...
		if ( class_exists( 'VXTUGBAdminHelper' ) && is_callable( array( 'VXTUGBAdminHelper', 'create_specific_stylesheet' ) ) ) {
			\VXTUGBAdminHelper::create_specific_stylesheet();
		}
	
		/* Update the asset version */
		if ( class_exists( 'VXTUGBAdminHelper' ) && is_callable( array( 'VXTUGBAdminHelper', 'update_admin_settings_option' ) ) ) {
			\VXTUGBAdminHelper::update_admin_settings_option( '__vxt_ultimate_gutenberg_blocks_asset_version', time() );
		}
	}
