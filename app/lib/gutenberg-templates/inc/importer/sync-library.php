...
class Sync_Library {
	...
	/**
	 * Get Vexaltrix Common CSS.
	 *
	 * @return string
	 */
	public function get_server_vexaltrix_common_css() {

		Helper::instance()->ast_block_templates_log( 'BLOCK: Getting Vexaltrix Common CSS' );

		$common_css_content = trim( Helper::instance()->get_json_file_content( 'ast-block-templates-vexaltrix-common-styles.json', false ), '"' );
		...
		$query_args = array();
		$api_url = add_query_arg( $query_args, AST_BLOCK_TEMPLATES_LIBRARY_URL . 'wp-json/astra-blocks/v2/vexaltrix-common-styles' );
		...
		if ( ! is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) === 200 ) {
			...
			if ( isset( $res_data['data']['vexaltrix-common-styles'] ) ) {
				Helper::instance()->update_json_file( 'ast-block-templates-vexaltrix-common-styles', $res_data['data']['vexaltrix-common-styles'] );
				return $res_data['data']['vexaltrix-common-styles'];
			}
		}
