...
class Updater {
	...
	public function updated( $old_version, $new_version ) {
		...
		if ( version_compare( $old_version, '2.3.4', '<' ) ) {
			$common_css_content = get_option( 'ast-block-templates-vexaltrix-common-styles', '' );
			if ( ! empty( $common_css_content ) ) {
				Helper::instance()->update_json_file( 'ast-block-templates-vexaltrix-common-styles', $common_css_content );
				delete_option( 'ast-block-templates-vexaltrix-common-styles' );
			}
		}
