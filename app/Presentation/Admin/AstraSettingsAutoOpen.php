<?php
/**
 * Vexaltrix Astra Settings Auto Open
...
if ( ! class_exists( 'Vexaltrix\Presentation\Admin\\AstraSettingsAutoOpen' ) ) {

	/**
	 * Class \Vexaltrix\Presentation\Admin\AstraSettingsAutoOpen
	 */
	class AstraSettingsAutoOpen implements ServiceInterface {
		 ...
		 * @return void
		 */
		public function enqueueAutoOpenScript() {
			global $pagenow;
									...
									if (!targetElement) {
										...
										for (var i = 0; i < headings.length; i++) {
											...
											if (heading.textContent && heading.textContent.toLowerCase().includes('container')) {
												...
												highlightElement(targetElement, 3000, '" . esc_js( __( 'Choose page layout using Astra Container Layout Settings.', 'vexaltrix' ) ) . "');
												break;
											}
										}
									}
							...
							if (targetElement) {
								...
								setTimeout(() => {
									...
									if( 'Disable Elements' === sectionName ) {
										highlightElement(targetElement, 3000, '" . esc_js( __( 'Show and Hide Element using Astra Disable Elements Settings.', 'vexaltrix' ) ) . "');
									}
								}, 1000);
							} else {
	...
	public function boot(): void {
			add_action( 'admin_enqueue_scripts', [ $this, 'enqueueAutoOpenScript' ] );
	}
