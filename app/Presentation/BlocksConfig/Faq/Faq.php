...
namespace Vexaltrix\Presentation\BlocksConfig\Faq;

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

if ( ! class_exists( 'Vexaltrix\Presentation\BlocksConfig\\Faq\\Faq' ) ) {

	/**
	 * Class \Vexaltrix\Presentation\BlocksConfig\Faq\Faq.
	 *
	 * @since 2.13.5
	 */
	class Faq {
		 ...
		 * Member Variable
		 *
		 * @var \Vexaltrix\Presentation\BlocksConfig\Faq\Faq
		 * @since 2.13.5
		 */
		 ...
		 * Initiator
		 *
		 * @return \Vexaltrix\Presentation\BlocksConfig\Faq\Faq
		 * @since 2.13.5
		 */
		...
		public function faqRenderIcon( $icon, $class ) {
			...
			<span class="<?php echo esc_attr( $class ); ?> vxt-faq-icon-wrap">
				<?php	
				echo \Vexaltrix\Core\Support\Helper::renderSvgHtml( $icon ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Already escaped & sanitize inside render_svg_html().
				?>
			</span>
			<?php
			return ob_get_clean();
		...
		public function renderFaqChildBlock( $attributes, $content, $block ) {
			...
			$headingTag           = \Vexaltrix\Core\Support\Helper::titleTagAllowedHtml( $attributes['headingTag'], $arrayOfAllowedHTML, 'span' );
	...
	/**
	 *  Prepare if class 'Vexaltrix\Presentation\BlocksConfig\\Faq\\Faq' exist.
