...
/**
 * \Vexaltrix\Presentation\Blocks\InitBlocks.
 *
 * @package Vexaltrix
 */
class InitBlocks implements ServiceInterface {
	...
	public function browserVisibility( $blockAttributes, $blockContent ) {
		...
		$userAgent = isset( $_SERVER['HTTP_USER_AGENT'] ) ? \Vexaltrix\Core\Support\Helper::getBrowserName( sanitize_text_field( $_SERVER['HTTP_USER_AGENT'] ) ) : '';
	...
	public function getTaxonomy() {
		...
		$postTypes = \Vexaltrix\Core\Support\Helper::getPostTypes();
	...
	public function addSvgIconAssets() {
		$localizeIconChunks = \Vexaltrix\Core\Support\Helper::backendLoadFontAwesomeIcons();
	...
	public function editorAssets() {
		...
		$localizedParams = [
			...
			'image_sizes'                             => \Vexaltrix\Core\Support\Helper::getImageSizes(),
			'post_types'                              => \Vexaltrix\Core\Support\Helper::getPostTypes(),
		...
		.my-class {text-align: center;} // my-class is a custom selector',
			...
			'docsUrl'                                 => \Vexaltrix\Presentation\Admin\AdminSettings::getProUrl( '/docs/', 'free-plugin', 'vxt-editor-page', 'vxt-plugin' ),
			'upsellModalEditor'                       => \Vexaltrix\Presentation\Admin\AdminSettings::getProUrl( '/pricing/', 'free-plugin', 'vexaltrix-editor', 'upsell-popup-view-plan' ),
		...
		// Style.
		\Vexaltrix\Core\Support\ScriptsUtils::enqueueBlocksStyles();
		// RTL Styles.
	...
	public function uagGlobalUpdateAllowedBlock() {
		...
		if ( ! empty( $_POST['defaultAllowedQuickSidebarBlocks'] ) ) {
			...
			\Vexaltrix\Presentation\Admin\AdminSettings::updateAdminSettingsOption( 'vxt_ultimate_gutenberg_blocks_quick_sidebar_allowed_blocks', $vexaltrixDefaultAllowedQuickSidebarBlocks );
			wp_send_json_success();
		}
		wp_send_json_error();
	}
	...
	public function uagGlobalBlockStyles() {
		...
		foreach ( $globalBlockStyles as $key => $style ) {
			if ( ! empty( $_POST['globalBlockStyleId'] ) && ! empty( $style['value'] ) && $style['value'] === $_POST['globalBlockStyleId'] ) {
				...
				$blockCss  = \Vexaltrix\Presentation\Blocks\BlockModule::getFrontendCss( $blockSlug, $blockAttr, $blockAttr['block_id'], true );
				...
				if ( ! empty( $globalBlockStyles[ $key ]['post_ids'] ) && is_array( $globalBlockStyles[ $key ]['post_ids'] ) ) {
					foreach ( $globalBlockStyles[ $key ]['post_ids'] as $postId ) {
						\Vexaltrix\Core\Support\Helper::deletePageAssets( $postId );
					}
				}
			}
		}
...
/**
 *  Prepare if class 'Vexaltrix\Presentation\Blocks\\InitBlocks' exist.
