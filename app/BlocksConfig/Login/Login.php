<?php
namespace Vexaltrix\BlocksConfig\Login;

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

if ( ! class_exists( 'Vexaltrix\\BlocksConfig\\Login\\Login' ) ) {

	/**
	 * Class \Vexaltrix\BlocksConfig\Login\Login.
	 */
	final class Login {
		...
		public function renderCallback( $attributes, $content ) {
			...
			$recaptchaSiteKey = \Vexaltrix\Admin\AdminSettings::get( 'uag_recaptcha_site_key_v2', '' );
		...
		public function loginFormHandler() {
			...
			if ( $recaptchaStatus ) {
				...
				$recaptchaSecret = '';
				if ( 'v2' === $recaptchaType ) {
					$recaptchaSecret = \Vexaltrix\Admin\AdminSettings::get( 'uag_recaptcha_secret_key_v2', '' );
				} else {
					$recaptchaSecret = \Vexaltrix\Admin\AdminSettings::get( 'uag_recaptcha_secret_key_v3', '' );
				}
	...
	/**
	 *  Prepare if class 'Vexaltrix\\BlocksConfig\\Login\\Login' exist.
