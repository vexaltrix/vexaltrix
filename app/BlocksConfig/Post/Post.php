...
namespace Vexaltrix\BlocksConfig\Post;

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

if ( ! class_exists( 'Vexaltrix\\BlocksConfig\\Post\\Post' ) ) {

	/**
	 * Class \Vexaltrix\BlocksConfig\Post\Post.
	 */
	class Post {
		...
		public function registerBlocks() {
			...
			if ( method_exists( 'Vexaltrix\\Core\\Blocks\\BlockHelper', 'uagGeneratePhpBorderAttribute' ) ) {

				$paginationMasonryBorderAttribute = \Vexaltrix\Core\Blocks\BlockHelper::uagGeneratePhpBorderAttribute( 'paginationMasonry' );
		...
		public function getPostAttributes() {
			...
			if ( method_exists( 'Vexaltrix\\Core\\Blocks\\BlockHelper', 'uagGeneratePhpBorderAttribute' ) ) {
				...
				$btnBorderAttribute     = \Vexaltrix\Core\Blocks\BlockHelper::uagGeneratePhpBorderAttribute( 'btn' );
				$overallBorderAttribute = \Vexaltrix\Core\Blocks\BlockHelper::uagGeneratePhpBorderAttribute( 'overall' );
		...
		public function postGridCallback( $attributes ) {
			...
			// Render query.
			$query = \Vexaltrix\Support\Helper::getQuery( $attributes, 'grid' );
		...
		public function postCarouselCallback( $attributes ) {
			...
			// Render query.
			$query = \Vexaltrix\Support\Helper::getQuery( $attributes, 'carousel' );
		...
		public function postMasonryCallback( $attributes ) {
			...
			// Render query.
			$query = \Vexaltrix\Support\Helper::getQuery( $attributes, 'masonry' );
		...
		public function renderPagination( $query, $attributes ) {
			...
			$base                = \Vexaltrix\Support\Helper::buildBaseUrl( $permalinkStructure, $base );
			$format              = \Vexaltrix\Support\Helper::pagedFormat( $permalinkStructure, $base );
			$paged               = \Vexaltrix\Support\Helper::getPaged( $query );
		...
		public function postPagination() {
			...
			if ( isset( $_POST['attributes'] ) ) {
				...
				$query = \Vexaltrix\Support\Helper::getQuery( $postAttributeArray, 'grid' );
		...
		public function masonryPagination() {
			...
			$query = \Vexaltrix\Support\Helper::getQuery( $postAttributeArray, 'masonry' );
		...
		public function renderTitle( $attributes ) {
			...
			$titleTag             = \Vexaltrix\Support\Helper::titleTagAllowedHtml( $attributes['titleTag'], $arrayOfAllowedHTML, 'h4' );
			?>
		...
		public function renderExcerpt( $attributes ) {
			...
			if ( 'full_post' === $attributes['displayPostContentRadio'] ) {
				...
				$excerptLengthFallback = \Vexaltrix\Core\Blocks\BlockHelper::getFallbackNumber( $attributes['excerptLength'], 'excerptLength', 'post-timeline' );
	...
	/**
	 *  Prepare if class 'Vexaltrix\\BlocksConfig\\Post\\Post' exist.
