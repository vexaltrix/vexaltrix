...
namespace Vexaltrix\BlocksConfig\TaxonomyList;

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

if ( ! class_exists( 'Vexaltrix\\BlocksConfig\\TaxonomyList\\TaxonomyList' ) ) {

	/**
	 * Class \Vexaltrix\BlocksConfig\TaxonomyList\TaxonomyList.
	 *
	 * @since 1.18.1
	 */
	class TaxonomyList {
		...
		public function registerBlocks() {
			...
			if ( method_exists( 'Vexaltrix\\Core\\Blocks\\BlockHelper', 'uagGeneratePhpBorderAttribute' ) ) {
				...
				$borderAttribute = \Vexaltrix\Core\Blocks\BlockHelper::uagGeneratePhpBorderAttribute( 'overall' );
		...
		public function gridHtml( $attributes ) {
			...
			if ( 'grid' === $layout ) {
				...
				$titleTag             = \Vexaltrix\Support\Helper::titleTagAllowedHtml( $titleTag, $arrayOfAllowedHTML, 'h4' );
		...
		public function listHtml( $attributes ) {
			...
			if ( 'list' === $layout ) {
				...
				$arrayOfAllowedHTML = [ 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'div' ];
				$titleTag             = \Vexaltrix\Support\Helper::titleTagAllowedHtml( $titleTag, $arrayOfAllowedHTML, 'div' );
	...
	/**
	 *  Prepare if class 'Vexaltrix\\BlocksConfig\\TaxonomyList\\TaxonomyList' exist.
