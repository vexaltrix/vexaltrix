...
$defaultEmail   = $currUser->user_email;
...
$jsAttr = [
	...
	'afterSubmitToEmail'      => isset( $attr['afterSubmitToEmail'] ) && is_string( $attr['afterSubmitToEmail'] ) && '' !== trim( $attr['afterSubmitToEmail'] ) ? sanitize_email( $attr['afterSubmitToEmail'] ) : sanitize_email( $defaultEmail ),
	...
	'afterSubmitEmailSubject' => isset( $attr['afterSubmitEmailSubject'] ) && is_string( $attr['afterSubmitEmailSubject'] ) ? sanitize_text_field( $attr['afterSubmitEmailSubject'] ) : __( 'Form Submission', 'vexaltrix' ),
...
window.addEventListener("DOMContentLoaded", function(){
	VexaltrixForms.init( <?php echo wp_json_encode( $jsAttr ); ?>, '<?php echo esc_attr( $selector ); ?>', <?php echo wp_json_encode( $currentPostId ); ?> );
});
<?php
return ob_get_clean();
?>
