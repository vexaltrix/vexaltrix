...
$slickOptions = apply_filters(
	'vxt_ultimate_gutenberg_blocks_testimonials_slick_options',
	[
...
$equalHeight  = isset( $attr['equalHeight'] ) ? $attr['equalHeight'] : '';

$baseSelector = ( isset( $attr['classMigrate'] ) && $attr['classMigrate'] ) ? '.vxt-block-' : '#vxt-testimonial-';
$selector      = $baseSelector . $id;
ob_start();
?>
jQuery( document ).ready( function() {
	if( jQuery( '<?php echo esc_html( $selector ); ?>' ).length > 0 ){
	jQuery( '<?php echo esc_html( $selector ); ?>' ).find( ".is-carousel" ).slick( <?php echo wp_json_encode( $slickOptions ); ?> );
	}
	var $scope = jQuery('.vxt-block-<?php echo esc_html( $id ); ?>');
	var enableEqualHeight = ( '<?php echo esc_html( $equalHeight ); ?>' );
			if( enableEqualHeight ){
				$scope.imagesLoaded( function() {
					VexaltrixTestimonialCarousel._setHeight( $scope );
				});
